...
\begin{document}
  ...
  \begin{frame}{Notação e Tipos de Grafos}
    \begin{block}{Definição básica}
      ...
      \textit{adjacency matrix} $\mathbf{A} \in \{0,1\}^{n \times n}$ codifica $E$.
      Atributos de nó vivem em $\mathbf{X} \in \mathbb{R}^{n \times d}$.
    \end{block}
    \vspace{0.3em}
    \begin{itemize}
      \item \textbf{Dirigido vs não-dirigido}: $\mathbf{A}$ simétrica ou não.
    ...
    \note{
      \begin{itemize}
        ...
        \item Mostrar que $\mathbf{X}$ existe é importante: GNNs se diferenciam de embeddings
  ...
  \begin{frame}{Exemplo: Matriz de Adjacência}
    \begin{columns}[T]
      ...
      \begin{column}{0.55\textwidth}
        ...
        \begin{itemize}
          ...
          \item Não-dirigido $\Rightarrow \mathbf{A}$ \textbf{simétrica}.
    ...
    \note{
      Aproveite para apontar: a diagonal de $\mathbf{A}$ é zero (sem auto-laços por
  ...
  \begin{frame}{O Que Queremos Aprender}
    \begin{itemize}
      \item Representações vetoriais $\mathbf{h}_v \in \mathbb{R}^{d'}$ para cada nó $v$.
  ...
  \begin{frame}{Sem Ordem Canônica}
    \begin{columns}[T]
      \begin{column}{0.55\textwidth}
        Um mesmo grafo $G$ pode ser representado por matrizes de adjacência $\mathbf{A}$
        \textbf{diferentes}, dependendo de como rotulamos os nós.

        \vspace{0.4em}
        Se $\mathbf{P}$ é qualquer permutação dos nós, então $\mathbf{A}$ e $\mathbf{P} \mathbf{A} \mathbf{P}^\top$ descrevem o
        \textbf{mesmo grafo}, apenas com etiquetas trocadas.

        \vspace{0.5em}
        \begin{block}{Implicação}
          Qualquer modelo $f(\mathbf{A}, \mathbf{X})$ que veja $G$ deve ser indiferente a essa
          permutação: $f(\mathbf{P} \mathbf{A} \mathbf{P}^\top, \mathbf{P} \mathbf{X}) = f(\mathbf{A}, \mathbf{X})$ (no caso invariante).
        \end{block}
      \end{column}
      \begin{column}{0.42\textwidth}
        \centering
        ...
        \end{tikzpicture}}
        \vspace{0.2em}\\
        \scriptsize Mesmo grafo, duas rotulagens, duas matrizes $\mathbf{A}$ distintas.
      \end{column}
    \end{columns}
    \note{
  ...
  \begin{frame}{Invariância e Equivariância à Permutação}
    Seja $\mathbf{P}$ uma matriz de permutação $n \times n$.
    \begin{itemize}
  ...
  \begin{frame}{Três Tarefas Canônicas}
    ...
    \begin{itemize}
      \item Nó: usar diretamente $\mathbf{h}_v$.
      \item Aresta: combinar $\mathbf{h}_u$ e $\mathbf{h}_v$ (concatenação, produto interno, MLP).
      \item Grafo: agregação (\textit{pooling}) sobre todos os $\mathbf{h}_v$ (soma, média, máx, atenção).
    \end{itemize}
    \note{
  ...
  \begin{frame}{As Três Tarefas, Visualmente}
    \begin{columns}[T]
      \begin{column}{0.32\textwidth}
        ...
        \scriptsize \textit{readout}: usar $\mathbf{h}_v$ direto
      \end{column}
      \begin{column}{0.32\textwidth}
        \centering
        ...
        \scriptsize \textit{readout}: combinar $\mathbf{h}_u$ e $\mathbf{h}_v$
      \end{column}
      \begin{column}{0.32\textwidth}
        \centering
        ...
        \begin{tikzpicture}[scale=0.85,
          ...
          \node[font=\scriptsize, NodeGreen] at (0,-1.05)
            {$\{\mathbf{h}_v\} \xrightarrow{\;\mathrm{pooling}\;} \mathbf{h}_G$};
        \end{tikzpicture}\\[0.35em]
        \small O \textcolor{NodeGreen}{grafo inteiro} é tóxico?\\[0.2em]
        \scriptsize \textit{readout}: pooling sobre $\{\mathbf{h}_v\}$
      \end{column}
    \end{columns}
    \note{
  ...
  \begin{frame}{node2vec e o Limite dos Embeddings Pré-GNN}
      ...
      \begin{column}{0.42\textwidth}
        \begin{alertblock}{Limites fundamentais}
          \begin{itemize}
            ...
            \item \textbf{Sem atributos}: ignora $\mathbf{X}$.
    ...
    \begin{block}{Para onde queremos ir}
      Um modelo que (i) seja \textbf{indutivo}, (ii) use atributos $\mathbf{X}$, (iii) treine
    ...
    \begin{block}{Para cada camada $k = 1, \ldots, K$ e cada nó $v$:}
      \begin{enumerate}
        \item \textbf{Message}: cada vizinho $u$ envia uma mensagem
          \[
            \mathbf{m}^{(k)}_{u \to v} = M^{(k)}\!\bigl(\mathbf{h}^{(k-1)}_u,\, \mathbf{h}^{(k-1)}_v,\, e_{uv}\bigr)
          \]
        \item \textbf{Aggregate}: $v$ combina mensagens dos vizinhos $\mathcal{N}(v)$
          \[
            \mathbf{a}^{(k)}_v = \mathrm{AGG}\!\bigl(\{\mathbf{m}^{(k)}_{u \to v} : u \in \mathcal{N}(v)\}\bigr)
          \]
        \item \textbf{Update}: $v$ atualiza seu próprio embedding
          \[
            \mathbf{h}^{(k)}_v = U^{(k)}\!\bigl(\mathbf{h}^{(k-1)}_v,\, \mathbf{a}^{(k)}_v\bigr)
          \]
      \end{enumerate}
    \end{block}
    {\footnotesize Convenções: $\mathbf{h}^{(0)}_v = \mathbf{x}_v$ (atributos iniciais do nó), $\mathcal{N}(v)$ é a vizinhança de $v$ e $e_{uv}$ é o atributo (opcional) da aresta.}
    \note{
  ...
  \begin{frame}{Uma Camada de Message Passing}
    ...
    \begin{tikzpicture}[
      ...
      \visible<2->{
        \draw[msg] (u1) to[bend right=10] node[midway,above,font=\scriptsize,NodeBlue]{$\mathbf{m}_{u_1\to v}$} (v);
        \draw[msg] (u2) to[bend left=10]  node[midway,above,font=\scriptsize,NodeBlue]{$\mathbf{m}_{u_2\to v}$} (v);
        \draw[msg] (u3) to[bend left=10]  node[midway,below,font=\scriptsize,NodeBlue]{$\mathbf{m}_{u_3\to v}$} (v);
        \draw[msg] (u4) to[bend right=10] node[midway,below,font=\scriptsize,NodeBlue]{$\mathbf{m}_{u_4\to v}$} (v);
      }
      % Aggregate + update (passos 2 e 3, overlay 3)
      \visible<3->{
        \node[font=\small, NodeRed, right=2.6cm of v] (agg)
          {$\mathrm{AGG}\to \mathbf{a}_v \to U \to \mathbf{h}_v^{(k)}$};
    ...
    \begin{itemize}
      ...
      \item<3-> Após $K$ camadas, $\mathbf{h}_v$ contém informação a até $K$ saltos:
  ...
  \begin{frame}{GCN}{Intuição\footfullcite{kipf2017gcn}}
    ...
    \begin{block}{Forma por nó}
      \[
        \mathbf{h}^{(k)}_v = \sigma\!\left(
          \mathbf{W}^{(k)} \cdot
          \sum_{u \in \mathcal{N}(v) \cup \{v\}}
            \frac{1}{\sqrt{\tilde d_v\, \tilde d_u}}\, \mathbf{h}^{(k-1)}_u
        \right)
      \]
    ...
    \vspace{0.2em}
    \begin{itemize}
      \item Mensagem $=$ identidade ($\mathbf{h}_u$).
      \item Agregação $=$ soma normalizada simétrica.
      \item Update $=$ projeção linear $\mathbf{W}^{(k)}$ + não-linearidade $\sigma$.
    \end{itemize}
  ...
  \begin{frame}{GCN}{Forma Matricial}
    Definindo $\tilde{\mathbf{A}} = \mathbf{A} + \mathbf{I}$ (auto-laços, para $v$ manter a própria informação) e $\tilde{\mathbf{D}} = \mathrm{diag}(\tilde{\mathbf{A}} \mathbf{1})$:

    \vspace{0.4em}
    \begin{block}{Camada GCN inteira de uma vez}
      \[
        \mathbf{H}^{(k)} = \sigma\!\left(
          \tilde{\mathbf{D}}^{-1/2} \tilde{\mathbf{A}} \tilde{\mathbf{D}}^{-1/2}\, \mathbf{H}^{(k-1)}\, \mathbf{W}^{(k)}
        \right)
      \]
    \end{block}
    ...
    \vspace{0.5em}
    \begin{itemize}
      \item $\mathbf{H}^{(k)} \in \mathbb{R}^{n \times d_k}$: matriz de embeddings por nó.
      \item $\tilde{\mathbf{D}}^{-1/2} \tilde{\mathbf{A}} \tilde{\mathbf{D}}^{-1/2}$: \textbf{depende apenas
  ...
  \begin{frame}{Exemplo Numérico: GCN em 4 Nós (1/2)}
    Mesmo grafo da Seção 1. Atributos iniciais 1D: $\mathbf{h}^{(0)} = (1, 2, 3, 4)^\top$.
    ...
    \begin{columns}[T]
      ...
      \begin{column}{0.40\textwidth}
        Graus em $\tilde{\mathbf{A}}$:
        \[
          \tilde{\mathbf{d}} = (2,\, 4,\, 3,\, 3)
        \]
        \[
    ...
    \end{columns}
    \vspace{0.5em}
    Próximo passo: aplicar $\tilde{\mathbf{D}}^{-1/2} \tilde{\mathbf{A}}\, \tilde{\mathbf{D}}^{-1/2}$ a $\mathbf{h}^{(0)}$.
    \note{
  ...
  \begin{frame}{Exemplo Numérico: GCN em 4 Nós (2/2)}
    Matriz normalizada $\hat{\mathbf{A}} = \tilde{\mathbf{D}}^{-1/2} \tilde{\mathbf{A}}\, \tilde{\mathbf{D}}^{-1/2}$
    ...
    \[
      ...
      \end{bmatrix}
    \]
    Aplicando a $\mathbf{h}^{(0)} = (1, 2, 3, 4)^\top$, por exemplo
    $h^{(1)}_1 = 0.50 \cdot 1 + 0.354 \cdot 2 \approx 1.21$:
    \[
      \mathbf{h}^{(1)} = \hat{\mathbf{A}}\, \mathbf{h}^{(0)} \approx (1.21,\; 2.87,\; 2.91,\; 2.91)^\top
    \]
    ...
    \note{
      ...
      o nó $1$ tem só dois vizinhos em $\tilde{\mathbf{A}}$ (ele mesmo e o nó $2$),
  ...
  \begin{frame}{GraphSAGE}{Motivação\footfullcite{hamilton2017graphsage}}
    ...
    \vspace{0.4em}
    \begin{itemize}
      \item \textbf{Transdutivo}: precisa do grafo todo para computar $\mathbf{H}^{(k)}$.
      \item \textbf{Custo de memória}: $\tilde{\mathbf{A}}$ esparsa de \texttt{ogbn-papers100M}
    ...
    \begin{columns}[T]
      \begin{column}{0.42\textwidth}
        ...
        \begin{itemize}
          ...
          \item \textbf{Pool}: max-pool de MLP$(\mathbf{h}_u)$.
        ...
        \begin{alertblock}{Concatenação no update}
          GraphSAGE \textbf{concatena} $\mathbf{h}_v$ com a agregação, em vez de fundi-los
          como GCN. Isso preserva melhor a identidade do nó central.
        \end{alertblock}
      \end{column}
      \begin{column}{0.55\textwidth}
        \begin{algorithmic}[1]
          \Require Grafo $G$, atributos $\mathbf{X}$, $K$ camadas, tamanhos $S_1,\ldots,S_K$
          \State $\mathbf{h}^{(0)}_v \gets \mathbf{x}_v$ para todo $v$
          \For{$k = 1, \ldots, K$}
            \For{cada nó $v$ no minibatch}
              \State $S_k(v) \gets$ amostra de $S_k$ vizinhos de $\mathcal{N}(v)$
              \State $\mathbf{a}^{(k)}_v \gets \mathrm{AGG}(\{\mathbf{h}^{(k-1)}_u : u \in S_k(v)\})$
              \State $\mathbf{h}^{(k)}_v \gets \sigma\!\bigl(\mathbf{W}^{(k)}\,[\mathbf{h}^{(k-1)}_v \,\Vert\, \mathbf{a}^{(k)}_v]\bigr)$
              \State $\mathbf{h}^{(k)}_v \gets \mathbf{h}^{(k)}_v / \|\mathbf{h}^{(k)}_v\|_2$
            \EndFor
          \EndFor
          \State \Return $\{\mathbf{h}^{(K)}_v\}$
        \end{algorithmic}
      \end{column}
    \end{columns}
    \note{
    ...
    \begin{block}{Coeficientes de atenção}
      \[
        e_{vu} = \mathrm{LeakyReLU}\!\left(
          \mathbf{a}^{\top}\, [\,\mathbf{W} \mathbf{h}_v \,\Vert\, \mathbf{W} \mathbf{h}_u\,]
        \right)
      \]
      \[
      ...
      \]
      \[
        \mathbf{h}^{(k)}_v = \sigma\!\left(\sum_{u \in \mathcal{N}(v)} \alpha_{vu}\, \mathbf{W} \mathbf{h}^{(k-1)}_u\right)
      \]
    \end{block}
  ...
  \begin{frame}{Multi-Head e Ligação com Transformers}
    \begin{columns}[T]
      \begin{column}{0.55\textwidth}
        ...
        \[
          \mathbf{h}^{(k)}_v = \big\Vert_{i=1}^{H}\, \sigma\!\left(
            \sum_{u \in \mathcal{N}(v)} \alpha^{(i)}_{vu}\, \mathbf{W}^{(i)} \mathbf{h}^{(k-1)}_u
          \right)
        \]
  ...
  \begin{frame}{Exemplo Numérico: Coeficientes de Atenção em GAT}
    Nó $v$ com 3 vizinhos. Após projeção, $\mathbf{W} \mathbf{h}_v = (1, 0)$ e
    \[
      \mathbf{W} \mathbf{h}_{u_1} = (1, 0), \qquad
      \mathbf{W} \mathbf{h}_{u_2} = (0, 1), \qquad
      \mathbf{W} \mathbf{h}_{u_3} = (0.5,\, 0.5).
    \]
    ...
    \begin{columns}[T]
      \begin{column}{0.50\textwidth}
        Scores $e_{v,u_k} = \mathbf{a}^\top [\mathbf{W} \mathbf{h}_v \,\Vert\, \mathbf{W} \mathbf{h}_{u_k}]$
      ...
      \begin{column}{0.45\textwidth}
        \begin{block}{Embedding final de $v$}
          \[
            \mathbf{h}_v^{\text{novo}} = \sum_{k} \alpha_{v, u_k}\, \mathbf{W} \mathbf{h}_{u_k}
          \]
          \[
  ...
  \begin{frame}{Mitigações}
    \begin{itemize}
      \item \textbf{Skip connections} (estilo ResNet): $\mathbf{h}^{(k)}_v = \mathbf{h}^{(k-1)}_v +
            \mathrm{GNN}^{(k)}(\ldots)$.
  ...
  \begin{frame}{GIN}{Tão Poderoso Quanto WL\footfullcite{xu2019gin}}
    ...
    \begin{block}{Camada GIN}
      \[
        \mathbf{h}^{(k)}_v = \mathrm{MLP}^{(k)}\!\Bigl(
          (1 + \epsilon^{(k)})\, \mathbf{h}^{(k-1)}_v +
          \sum_{u \in \mathcal{N}(v)} \mathbf{h}^{(k-1)}_u
        \Bigr)
      \]
  ...
  \begin{frame}{Positional Encodings em Grafos}
    ...
    \begin{itemize}
      \item \textbf{Laplacian PE}: usar os $k$ primeiros autovetores do
            Laplaciano $\mathbf{L} = \mathbf{D} - \mathbf{A}$ como coordenadas espectrais. Capturam
