...
\begin{columns}
    \begin{column}{.5\linewidth}
        \begin{itemize}\small
            ...
            \begin{itemize}
                \item Uma rede que processa o \textbf{contexto} $y_{<t}$ produzindo um vetor $h$.
                \item Uma camada de saída que aproxima a distribuição do \textbf{próximo \textit{token}} a partir de $h$.
            \end{itemize}
    ...
    \begin{column}{.5\linewidth}
        ...
        \begin{tikzpicture}[every node/.style={font=\tiny}]
            ...
            % h vector
            \node[draw, rounded corners, fill=PastelPurple, minimum width=0.7cm, minimum height=0.5cm] (h) at (1.75, 1.7) {$h$};
            \draw[->] (nn) -- (h);
            % linear/softmax
...
\begin{itemize}\footnotesize
    \item Numa RNN, o estado oculto $h_t$ resume todo o contexto $y_{<t}$ e é atualizado a cada \textit{token}.
    \item A cada passo a rede prevê o \textbf{próximo \textit{token}} a partir de $h_t$.
\end{itemize}
\vspace{0.2em}
\centering
\begin{tikzpicture}[every node/.style={font=\tiny}]
...
\begin{columns}
    \begin{column}{.64\linewidth}
        \begin{itemize}\small
            \item A partir do vetor de contexto $h \in \mathbb{R}^d$ aplicamos uma transformação para a dimensão do vocabulário $|V|$.
            \begin{equation*}
                \text{logits} = W h + b, \qquad W \in \mathbb{R}^{|V|\times d}
            \end{equation*}
            ...
            \end{equation*}
            \item Cada \textbf{linha} de $W$ é um \textit{output word embedding}, a representação do \textit{token} de saída.
        \end{itemize}
    \end{column}
    \begin{column}{.36\linewidth}
        ...
        \begin{tikzpicture}[every node/.style={font=\tiny}]
            ...
            \node[font=\scriptsize] at (0.48, 0.45) {$W$};
            ...
            \node[font=\scriptsize] at (2.03, 0.45) {$h$};
            % equals logits
...
\begin{itemize}
    ...
    \begin{equation*}
        \mathcal{L}(y, \hat{y}) = -\sum_{i=1}^{|V|} y_i \, \ln\bigl(p(\hat{y}_i)\bigr) = -\ln\bigl(p(y_t \mid y_{<t})\bigr)
    \end{equation*}
    \item O alvo $y$ é um \textit{one-hot} para o \textit{token} correto, de modo que a soma colapsa para o termo do alvo.
\end{itemize}
\end{frame}
...
\end{itemize}
\begin{equation*}
    \frac{\exp(w_i^{\top} h)}{\sum_{w_j \in V} \exp(w_j^{\top} h)} \quad\longrightarrow\quad \frac{\exp\!\left(\frac{w_i^{\top} h}{\tau}\right)}{\sum_{w_j \in V} \exp\!\left(\frac{w_j^{\top} h}{\tau}\right)}
\end{equation*}
\centering
\begin{tikzpicture}[every node/.style={font=\tiny}]
...
\begin{itemize}
    ...
    \end{itemize}
    \item Notação: $s_k$ são os estados do \textit{encoder} e $h_t$ o estado do \textit{decoder} no passo $t$.
\end{itemize}
\centering
\begin{tikzpicture}[every node/.style={font=\tiny}]
    % ---- encoder (step 1) ----
...
\begin{itemize}
    ...
    \item O \textit{encoder} bidirecional produz os estados ocultos $s_1, \dots, s_8$ e o \textit{decoder} parte do estado oculto final para gerar a tradução.
\end{itemize}
\centering
\mttrainstep{8}{}{}{0}{0}{}
\end{frame}
...
\begin{itemize}
    ...
    \begin{enumerate}
        \item Recebe o estado oculto do \textit{decoder} $\textcolor{decInk}{h_t}$ e todos os estados do \textit{encoder} $\textcolor{encInk}{s_1}, \dots, \textcolor{encInk}{s_m}$.
        \item Computa um \textit{score} $\text{score}(\textcolor{decInk}{h_t}, \textcolor{encInk}{s_k})$ para cada $k = 1, \dots, m$.
        \item Aplica \textcolor{smInk}{\textit{softmax}} sobre os \textit{scores} para obter os \textbf{pesos de atenção} $\textcolor{wInk}{a_k^{(t)}}$.
        \item Calcula o \textit{context vector} $\textcolor{ctxInk}{c^{(t)}}$ como soma ponderada dos $\textcolor{encInk}{s_k}$ pelos pesos.
        \item O contexto é passado para o \textit{decoder} para a previsão do próximo \textit{token}.
    \end{enumerate}
\end{itemize}
\begin{equation*}
    \textcolor{wInk}{a_k^{(t)}} = \frac{\exp(\text{score}(\textcolor{decInk}{h_t}, \textcolor{encInk}{s_k}))}{\sum_{i=1}^{m} \exp(\text{score}(\textcolor{decInk}{h_t}, \textcolor{encInk}{s_i}))}, \qquad \textcolor{ctxInk}{c^{(t)}} = \sum_{k=1}^{m} \textcolor{wInk}{a_k^{(t)}}\, \textcolor{encInk}{s_k}
\end{equation*}
\end{frame}
...
\begin{tikzpicture}[every node/.style={font=\tiny}]
    ...
    \node[font=\scriptsize, ForestGreen] at (2.375, -1.45) {\textbf{Encoder} ($s_k$, bidirecional)};

    % ---------------- (2) decoder ----------------
    \onslide<2->{
        \node[deccell, minimum width=0.65cm, minimum height=0.8cm] (dt) at (7.0, 0) {};
        \node[font=\tiny] at (7.0, -0.55) {<bos>};
        \node[font=\tiny, decInk] at (7.0, 1.0) {$h_t$};
    ...
    \onslide<3->{
        ...
        \node[font=\tiny, red!70!black] at (5.8, 1.95) {$\text{score}=h_t^\top s_k$};
    }
    ...
    \onslide<5->{
        \node[draw, rounded corners, fill=PastelPurple, minimum width=0.5cm, minimum height=0.9cm] (cv) at (3.9, 3.4) {$c^{(t)}$};
...
\onslide<3->{\textcolor{decInk}{\textbf{Scores:}} empilhando os $\textcolor{encInk}{s_k}$ como colunas de $\textcolor{encInk}{S}$, um único produto $\textcolor{decInk}{h_t^\top}\textcolor{encInk}{S}$:\\[0.15em]
$\textcolor{decInk}{(2\;\;1)}\,\textcolor{encInk}{\begin{pmatrix}1 & 0 & 0.5 & {-}1 & {-}1 & 0\\[-0.2em] 0 & 1 & {-}1 & 1 & 2 & {-}1\end{pmatrix}}=(\,2\;\;1\;\;0\;\;{-}1\;\;0\;\;{-}1\,)$}\\[0.2em]
\onslide<4->{\textcolor{smInk}{\textbf{Pesos de Atenção:}} $\textcolor{wInk}{a^{(t)}}=\operatorname{softmax}(\textcolor{decInk}{\text{scores}_t})=[\,0.58;\,0.21;\,0.08;\,0.03;\,0.08;\,0.03\,]$}\\[0.15em]
\onslide<5->{\textcolor{ctxInk}{\textbf{Contexto:}} $\textcolor{ctxInk}{c^{(t)}}=\sum_k \textcolor{wInk}{a_k^{(t)}}\, \textcolor{encInk}{s_k} \approx (0.51;\,0.29)$}
\par
\begin{itemize}\footnotesize
...
\begin{itemize}
    \item Existem várias formas de calcular $\text{score}(h_t, s_k)$. As três mais clássicas\footfullcite{luong2015effective}, vistas como produtos de matrizes:
\end{itemize}
\vspace{0.2em}
\centering
\begin{tikzpicture}[every node/.style={font=\scriptsize}, scalar/.style={fill=PastelYellow}]
    ...
    \node[font=\tiny] at (5.4,-0.62) {$[h_t, s_k]$};
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Capítulo 10 de \textit{Dive into Deep Learning}\footfullcite{zhang2023dive}.
