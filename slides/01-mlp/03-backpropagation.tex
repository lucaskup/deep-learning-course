...
\begin{document}
      ...
      \begin{column}{.35\linewidth}
        ...
        \begin{tikzpicture}
          \begin{axis}[
            ...
            xlabel={$\mlinstance_{1}$},
            ylabel={$\mlinstance_{2}$},
            xmin=-1.5,
            xmax=1.5,
            ymin=-1.5,
            ymax=1.5,
  ...
  \begin{frame}{Treinando um MLP}{Notação e Definições}
    \begin{itemize}
      ...
      \item $\X$ é a entrada do MLP (também pode ser chamado de $a^{(0)}$)

      \item $\y$ é a variável alvo

      \item $\thetav_{ij}^{(l)}$ é o peso da camada $l$ que conecta a entrada $j$
        a saída $i$

      \item $z^{(l)}$ é a \textbf{\alert{pré-ativação}} da unidade, $z^{(l)}=
        a^{(l-1)}\thetav^{(l)^\top}$

      \item $a^{(l)}$ é a \textbf{\alert{ativação}} (saída) da unidade
        $a^{(l)}=\varphi(z^{(l)})$
    \end{itemize}
  \end{frame}
  ...
  \begin{frame}{Treinando um MLP}{A função de Ativação}
    \begin{columns}
      ...
      \begin{column}{.6\linewidth}
        \resizebox{\linewidth}{!}{
        \input{slides/img/tikz/activationfunction/sinalgraph.tikz}
        \input{slides/img/tikz/activationfunction/sinalgraphprime.tikz}
        }
      \end{column}
    \end{columns}
  \end{frame}
  ...
  \begin{frame}{Treinando um MLP}{A função de Ativação}
    \begin{columns}
      ...
      \begin{column}{.7\linewidth}
        ...
        \input{slides/img/tikz/activationfunction/sigmoidgraphprime.tikz}
        }
      \end{column}
    \end{columns}
  \end{frame}
  ...
  \begin{frame}{Forward Pass}
    \centering
    \vspace{5pt}
    \input{slides/img/tikz/simplemlp.tikz.tex}
    \vspace{10pt}
  ...
  \begin{frame}[fragile,containsverbatim]
    ...
    \begin{columns}
      \begin{column}{0.4\textwidth}
        \begin{align*}
          \frac{\partial J}{\partial \thetav^{(1)}} & =-0.01318894 \\
          \frac{\partial J}{\partial \thetav^{(2)}} & =-0.04237412 \\
          \theta_{t}^{(2)}                          & = 0.5042374  \\
          \theta_{t}^{(1)}                          & = -0.4986811
        \end{align*}
      \end{column}
  ...
  \begin{frame}{Generalizando o Treinamento}
    ...
    \begin{itemize}
      ...
      \item \textbf{Ativação da Camada Anterior:} $a^{(l-1)}$
    \end{itemize}
    \vfill
    ...
    \begin{enumerate}
      ...
      \item \textbf{Propagação do Erro:}
        $\delta^{(l)}=(\delta^{(l+1)}\theta^{(l+1)})\odot a^{\prime(l)}$
        \vfill
    ...
    \begin{columns}
        ...
        \begin{column}{.4\textwidth}
            \begin{figure}
                \centering
                \resizebox{\linewidth}{!}{
                \input{slides/img/tikz/mlp_vectorized}}
            \end{figure}
        \end{column}
    \end{columns}
...
\begin{frame}{Exemplo Vetorizado: \textit{Backward Pass}}
      ...
      \begin{bmatrix}
        ...
        -0.04676
      \end{bmatrix}\\
      \delta^{(1)} &= (\delta^{(2)}\cdot \theta^{(2)}) \odot \sigma'(Z^{(1)})\\
      &= \left(
      \begin{bmatrix}
        0.00131 \\
        -0.04676
...
\end{frame}
 ...
 \begin{frame}{Exemplo Vetorizado: \textit{Backward Pass}}
    \begin{columns}
        ...
        \small
        \begin{align*}
              \frac{\partial J}{\partial \theta^{(2)}}&={\delta^{(2)}}^{T}\cdot A^{(1)}\\
              &=
      \begin{bmatrix}
        0.00131 & -0.04676
      \end{bmatrix}
      ...
      \end{bmatrix}\\
      \frac{\partial J}{\partial \theta^{(1)}}&={\delta^{(1)}}^{T}\cdot X \\
            &=
      \begin{bmatrix}
        0.00022 & -0.00809 \\
        -0.00003 & 0.00117 \\
