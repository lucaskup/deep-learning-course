...
\begin{document}
    ...
    % \begin{columns}[T]
      % \begin{column}{0.5\textwidth}
        ...
        \begin{itemize}
          ...
          \item Resultado: uma matriz $\mathbf{E} \in \mathbb{R}^{|V| \times d}$
        \end{itemize}
      % \end{column}
      % \begin{column}{0.5\textwidth}
    ...
    \begin{columns}
      \begin{column}{0.5\textwidth}
        ...
        \begin{enumerate}
          ...
          \item Adicione uma pequena cabeça linear \textcolor{DarkBlue}{$\mathbf{W} \in \mathbb{R}^{d \times C}$}
          \item $\text{logits}_i = \mathbf{W}^\top \mathbf{h}_i$
  ...
  \begin{frame}{Tarefa: \textit{Named Entity Recognition} (NER)}
    ...
    \begin{itemize}
      ...
      \item Cabeça linear $\mathbf{W} \in \mathbb{R}^{d \times C}$ com $C = 2k+1$ (um B- e um I- por tipo, mais O), onde $k$ é o número de categorias presentes no NER
  ...
  \begin{frame}{Tarefa: \textit{Extractive Question Answering} (SQuAD)\footfullcite{rajpurkar2016squad}}
    ...
    Dado uma frase no seguinte formato:  \texttt{[CLS]}\textcolor{DarkRed}{\textbf{Pergunta}}\texttt{[SEP]}\textcolor{DarkBlue}{\textbf{Contexto}}\texttt{[SEP]}, identificar o \textbf{trecho} do parágrafo que responde a pergunta.
    
      \begin{exampleblock}{Exemplo}
      ``\texttt{[CLS]}\texttt{\textcolor{DarkRed}{Quando a PUCRS foi fundada?}}\texttt{[SEP]}\texttt{\textcolor{DarkBlue}{A PUCRS foi fundada em 1948 na cidade de Porto Alegre.}}\texttt{[SEP]}'' \\
      \textbf{Resposta}: \textcolor{DarkBlue}{\textbf{1948}} $\to$ \textit{span} de um único \textit{token}: contando o \texttt{[CLS]} como posição 0 e separando por palavras, início e fim na posição 13
      \end{exampleblock}
  ...
  \begin{frame}{\textit{In-Batch Negatives}: Exemplo Visual}
    ...
    \begin{columns}[T]
      ...
      \begin{column}{0.63\textwidth}
        ...
        \begin{itemize}
          \item Matriz $\mathbf{S}$: $S_{ij} = \text{sim}(f(q_i), f(d_j)) / \tau$
  ...
  \begin{frame}{Resultados do SBERT}
    ...
    \begin{tabular}{lc}
      ...
      \midrule
      GloVe (média) & $58.02$ \\
      BERT \texttt{[CLS]} \textit{vanilla} & $16.50$ \\
