...
\begin{document}
  ...
  \begin{frame}{Máscara Causal: Exemplo Numérico}
    Sequência de 3 tokens; scores $S = \dfrac{QK^\top}{\sqrt{d_k}}$, máscara $M$ somada antes do softmax (por linha):
    \vspace{0.4em}
    \begin{center}
    \resizebox{0.94\linewidth}{!}{%
      $\underbrace{\begin{bmatrix} 2 & 1 & 3 \\ 0 & 2 & 1 \\ 1 & 0 & 2 \end{bmatrix}}_{S}
      \;+\;
      \underbrace{\begin{bmatrix} 0 & \textcolor{DarkRed}{-\infty} & \textcolor{DarkRed}{-\infty} \\ 0 & 0 & \textcolor{DarkRed}{-\infty} \\ 0 & 0 & 0 \end{bmatrix}}_{M}
      \;=\;
  ...
  \begin{frame}{LM Head: do Vetor aos Logits}
    \begin{columns}[T]
      \begin{column}{0.52\textwidth}
        Após os $N$ blocos, cada posição $t$ carrega um vetor $h_t \in \mathbb{R}^{d_\text{model}}$.\\[0.4em]
        \begin{block}{Projeção no vocabulário}
          $z = h_t\, W_\text{LM}, \qquad W_\text{LM} \in \mathbb{R}^{d_\text{model} \times |V|}$
        \end{block}
        \vspace{0.3em}
        Cada logit é um \textbf{produto interno}, $z_v = h_t \cdot w_v$, onde $w_v$ é a coluna de $W_\text{LM}$ associada ao token $v$: mede o alinhamento entre o estado atual e cada token do vocabulário.
        \vspace{0.3em}
        \begin{itemize}
          \item \textbf{Weight tying}: muitos modelos reutilizam a matriz de embedding ($W_\text{LM} = E^\top$)
        \end{itemize}
      \end{column}
      \begin{column}{0.44\textwidth}
        \footnotesize\textbf{Exemplo}: $d_\text{model}=3$, $|V|=5$, uma coluna por token:\\[0.5em]
        \resizebox{\linewidth}{!}{$
          \underbrace{\begin{bmatrix}1.0 & 0.5 & -0.5\end{bmatrix}}_{h_t}
          \underbrace{\begin{bmatrix}
            2.0 & 1.5 & 0.8 & 0.3 & -0.3\\
            1.6 & 1.0 & 0.2 & 0.6 & 0.0\\
            -0.4 & -0.2 & -0.2 & 0.2 & 0.4
          \end{bmatrix}}_{W_\text{LM}}
        $}\\[0.6em]
    ...
    \begin{columns}[T]
      \begin{column}{0.52\textwidth}
        ...
        \begin{block}{Da pontuação à probabilidade}
          $P(v \mid x_{\le t}) = \text{softmax}(z)_v = \dfrac{e^{z_v}}{\sum_{u \in V} e^{z_u}}$
        \end{block}
        \vspace{0.3em}
        \begin{itemize}
          \item $z$: \textbf{logits}, um score por token do vocabulário
  ...
  \begin{frame}{FFN Padrão: Exemplo Numérico (ReLU)}
    \begin{center}
      $\text{FFN}(x) = \text{ReLU}(x W_1)\, W_2 = \max(0,\, x W_1)\, W_2$
    \end{center}
    \begin{columns}[T]
      ...
      \begin{column}{0.44\textwidth}
        ...
        {\setlength{\fboxsep}{3pt}%
        \colorbox{CellYellow}{$[\,\cdots\,]W_2\;\approx\;y = [\,0.9,\;{-0.1},\;0.3\,]$}
        }
      \end{column}
    \end{columns}
  \end{frame}
  ...
  \begin{frame}{Decodificação Guiada: Máscara sobre os Logits em $q_1$}
    ...
    \[
      P(v \mid x_{\le t}) = \text{softmax}\bigl(z + M_{q_1}\bigr)_v,
      \qquad
  ...
  \begin{frame}{Multi-Query Attention (MQA)\footnotemark[1]}
    \begin{columns}[T]
      \begin{column}{0.52\textwidth}
        Reduza K e V para \textbf{uma única cabeça} compartilhada; mantenha $H$ cabeças de Q.
        \[
    ...
    \begin{columns}[T]
      \begin{column}{0.52\textwidth}
        ...
        \begin{itemize}
          ...
          \item FlashAttention computa a atenção \textbf{por blocos} na SRAM (cache rápido), com \textit{online softmax}, sem nunca materializar a matriz completa
        \end{itemize}
        \vspace{0.2em}
        {\scriptsize \textit{Online softmax}: acumula o softmax bloco a bloco, guardando apenas o máximo e a soma parciais dos scores, corrigidos a cada novo bloco.}
      \end{column}
      \begin{column}{0.44\textwidth}
        \begin{block}{Propriedades}
          \begin{itemize}
            ...
            \item FlashAttention-2 \cite{dao2023flashattention2}: padrão de fato atual
          \end{itemize}
        \end{block}
        \vspace{0.3em}
  ...
  \begin{frame}{Top-$k$ Routing: Exemplo Numérico}
    ...
    \begin{columns}[T]
      \begin{column}{0.52\textwidth}
        {\setlength{\fboxsep}{3pt}%
        \colorbox{CellYellow}{$s = x W_g = [\,1.2,\;\mathbf{3.1},\;0.4,\;\mathbf{2.6}\,]$}\\[0.45em]
        \colorbox{CellBlue}{$\text{TopK}(s, 2) = [\,-\infty,\;3.1,\;-\infty,\;2.6\,]$}\\[0.45em]
        \colorbox{CellGreen}{$g = \text{softmax} = [\,0,\;\mathbf{0.62},\;0,\;\mathbf{0.38}\,]$}\\[0.45em]
        }%
        {\footnotesize $g_2 = \dfrac{e^{3.1}}{e^{3.1}+e^{2.6}} = \dfrac{22.2}{35.7} = 0.62$}\\[0.5em]
        {\setlength{\fboxsep}{3pt}%
        \colorbox{PastelOrange}{$y = 0.62\,\text{FFN}_2(x) + 0.38\,\text{FFN}_4(x)$}
        }
      \end{column}
      \begin{column}{0.44\textwidth}
        \begin{exampleblock}{O que foi economizado}
          $\text{FFN}_1$ e $\text{FFN}_3$ \textbf{nem são computadas} para este token: com $E=4$ e $k=2$, metade do compute da camada é evitado.
        \end{exampleblock}
        \vspace{0.3em}
        \begin{itemize}
