...
\begin{document}
  ...
  \begin{frame}{Treinamento: Todas as Posições em Paralelo}
    ...
    \begin{tabular}{lccccc}
      ...
      \textbf{Perda}   & $-\ln P(\texttt{O})$ & $-\ln P(\texttt{rato})$ & $-\ln P(\texttt{roeu})$ & $-\ln P(\texttt{a})$ & $-\ln P(\texttt{rolha})$ \\
      \bottomrule
    \end{tabular}}
    \end{center}
    \vspace{0.4em}
  ...
  \begin{frame}{GPT-1: Pré-Treino + Fine-Tuning\footnotemark[1]}
    \begin{columns}[T]
      \begin{column}{0.52\textwidth}
        ...
        \[
          \mathcal{L} = -\sum_{t} \ln P(x_t \mid x_1,\ldots,x_{t-1};\,\Theta)
        \]
      \end{column}
      \begin{column}{0.44\textwidth}
    ...
    \begin{columns}[T]
      ...
      \begin{column}{0.46\textwidth}
        \begin{block}{Sem cache (naïve)}
          ...
          Custo por token: $\mathcal{O}(n d^2)$\\
          Custo total: $\mathcal{O}(n^2 d^2)$
        \end{block}
        \vspace{0.3em}
    ...
    \begin{columns}[T]
      ...
      \begin{column}{0.46\textwidth}
        ...
        \begin{exampleblock}{Mixtral}
          \begin{itemize}
            \item Perda auxiliar de balanceamento (o valor $\alpha=0.01$ vem do Switch Transformer)
  ...
  \begin{frame}{Perplexidade: A Métrica Mais Simples}
    ...
    \[
      \mathrm{PPL}(w_1,\ldots,w_N)
        = \exp\!\left(-\frac{1}{N}\sum_{i=1}^{N}\ln P(w_i \mid w_1,\ldots,w_{i-1})\right)
    \]
    \begin{itemize}\small
      \item Menor PPL $\to$ modelo estima melhor a distribuição do corpus
      \item $\mathrm{PPL} \in [1, \infty)$: vale $1$ na certeza absoluta e $|V|$ no modelo aleatório (uniforme sobre o vocabulário)
      \item $|V|$ é referencial, não teto: um modelo ruim e \textbf{confiante} fica arbitrariamente pior, e $P \to 0$ em um \textit{token} observado dá $\mathrm{PPL} \to \infty$
  ...
  \begin{frame}{Perplexidade: Por que Não é Suficiente?}
    \begin{alertblock}{Limitações da perplexidade como única métrica}
      \begin{itemize}
        ...
        \item \textbf{Comparação entre modelos}: o valor depende do tokenizador, então PPL de modelos com vocabulários diferentes não é comparável
      \end{itemize}
    \end{alertblock}
    \vspace{0.5em}
