...
\begin{document}
  ...
  \begin{frame}{\textit{Flash Attention}: O Gargalo de Memória \cite{dao2022flashattention}}
    \begin{columns}[T]
      \begin{column}{0.52\textwidth}
        ...
        $S = QK^\top \in \mathbb{R}^{N\times N}$ inteira na HBM:
        \[
          \text{Attn}(Q,K,V) =
  ...
  \note{
    ...
    \begin{itemize}
      \item \textbf{Atenção padrão:} (1) lê Q, K da HBM; (2) computa $S=QK^\top$ e escreve $S$ na HBM; (3) lê $S$ para softmax, escreve $P=\text{softmax}(S)$ na HBM; (4) lê $P$ e $V$ para calcular saída, escreve $O$ na HBM. Cada round-trip à HBM ($\sim$2 TB/s) é caro.
      \item Para $N=4096$: $S$ em fp16 = $32\text{MB}$. Com $\sim$2 TB/s: $32\text{MB}/2\text{TB/s} = 16\,\mu\text{s}$ por leitura/escrita. Multiplicando por cabeças de atenção, camadas e passos de treino: segundos de tempo desperdiçado em IO puro.
  ...
  \begin{frame}{\textit{Flash Attention}: Tiling na SRAM}
    \begin{columns}[T]
      \begin{column}{0.50\textwidth}
        \textbf{Ideia central:} dividir Q, K, V em \textbf{blocos}
        que cabem na SRAM e processar atenção bloco a bloco,
        sem materializar $S = QK^\top$ na HBM.
        ...
        \textcolor{DarkGreen}{\small \textbf{Resultado:} memória $O(N^2)\!\to\!O(N)$;
  ...
  \note{
    ...
    \begin{itemize}
      \item \textbf{Loop externo:} itera sobre blocos de Q. Para cada $Q_i$: carrega na SRAM.
      \item \textbf{Loop interno:} itera sobre blocos de K e V. Para cada $(Q_i, K_j)$: (1) computa $s_{ij} = Q_i K_j^\top / \sqrt{d}$ na SRAM; (2) atualiza softmax online ($m_i, \ell_i$); (3) acumula $O_i$.
      \item \textbf{Online softmax:} ao encontrar um score maior em um bloco novo, recalibra o acumulador $O_i$ multiplicando pelo fator de correção $e^{m_i^{\text{old}} - m_i^{\text{new}}}$. Matematicamente idêntico ao softmax global, sem revisitar blocos anteriores.
      \item \textbf{Resultado:} apenas Q, K, V e O são transferidos HBM$\leftrightarrow$SRAM. A matriz $S$ nunca existe na HBM. IO reduzido de $O(N^2)$ para $O(N)$.
    \end{itemize}
  }
  \end{frame}
  ...
  \begin{frame}{Online Softmax: Exemplo Numérico}
    \begin{columns}[T,onlytextwidth]
      \begin{column}{0.56\textwidth}
        \small
        \textbf{Scores:} $s = [3,\,1,\,2,\,4]$ em dois blocos:
        $B_1 = [3,\,1]$, $B_2 = [2,\,4]$
  ...
  \note{
    ...
    \begin{itemize}
      ...
      \item \textbf{Contextos longos:} FA tornou praticável treinar com $N = 32$k, 128k ou até 1M tokens. Sem FA, o custo $O(N^2)$ em memória limita $N$ a poucos milhares.
