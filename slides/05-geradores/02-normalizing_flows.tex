...
\begin{document}
  ...
  \begin{frame}{Loss -- \textit{Maximum Likelihood}}
    ...
    \[
      ...
      \left[\prod_{i=1}^{N} P\!\left(\mathbf{x}^{(i)} \mid \thetav\right)\right]
    \]
    \vspace{0.2em}
    \begin{itemize}
  ...
  \begin{frame}{Loss -- NLL passo a passo (1/2)}
    ...
    \begin{align*}
      \hat{\thetav}
      &= \argmax_{\thetav}\left[\prod_{i} P(\mathbf{x}^{(i)}\mid\thetav)\right]
      \\[0.2em]
      &\visible<2->{= \argmin_{\thetav}
        \left[\sum_{i=1}^{N} -\log P\!\left(\mathbf{x}^{(i)} \mid \thetav\right)\right]}
    \end{align*}
    \vspace{0.3em}
    ...
    \only<2->{
      ...
      \vspace{-0.5em}
        ...
        \]
      \end{block}
      \emph{Vamos substituir $P(\mathbf{x}^{(i)}\mid \thetav)$ na expressão
      acima.}
    }
    \note{
  ...
  \begin{frame}{Loss -- NLL passo a passo (2/2)}
    ...
    \begin{align*}
      ...
      &\visible<1->{= \argmin_{\thetav}
        \left[\sum_{i=1}^{N} -\log\!\left(P(\mathbf{z}^{(i)})\left|\det \tfrac{\partial f(\mathbf{x}^{(i)},\thetav)}{\partial \mathbf{x}^{(i)}}\right|\right)\right]} \\[0.2em]
      &\visible<2->{= \argmin_{\thetav}
        \left[\sum_{i=1}^{N}\!\left[-\log\!\left|\det \tfrac{\partial f(\mathbf{x}^{(i)},\thetav)}{\partial \mathbf{x}^{(i)}}\right| - \log P(\mathbf{z}^{(i)})\right]\right]}
    \end{align*}
    \vspace{0.3em}
    \visible<2->{
      \begin{alertblock}{Dois termos, dois papéis}
        \begin{itemize}
          ...
          \item \textcolor{DecBlue}{$-\log P(\mathbf{z})$}: a distribuição base é
  ...
  \begin{frame}{\textit{Flows} -- Caminho direto e inverso}
    ...
    \resizebox{0.95\linewidth}{!}{%
      \begin{tikzpicture}[
        ...
        \node[font=\scriptsize] at (0,1.35)  {base $p(\mathbf{z})$};
        \node[font=\scriptsize] at (12,1.35) {$p(\mathbf{x})$};
  ...
  \begin{frame}{\textit{Flows} -- Jacobiana da composição}
    ...
    \vspace{0.3em}
    \[
      \left|\frac{\partial f[\mathbf{z},\thetav]}{\partial \mathbf{z}}\right|
      ...
      \cdots
      \left|\frac{\partial f_{1}[\mathbf{z},\thetav_1]}{\partial \mathbf{z}}\right|
    \]
    \vspace{0.3em}
    \begin{itemize}
  ...
  \begin{frame}{\textit{Linear Flow}}
    ...
    \vspace{0.3em}
    \[
      f(\mathbf{x}) = \mathbf{\Theta}\mathbf{x} + \mathbf{b}
      \qquad
      f^{-1}(\mathbf{z}) = \mathbf{\Theta}^{-1}(\mathbf{z} - \mathbf{b})
    \]
    \vspace{0.3em}
  ...
  \begin{frame}{\textit{Coupling Flows} -- Inversa}
    ...
    \resizebox{0.84\linewidth}{!}{%
      \begin{tikzpicture}[
        ...
        % caixas
        \node[box] (cn) at (6,2.05) {Coupling\\Network $\thetav(\mathbf{z}^{A})$};
  ...
  \begin{frame}{\textit{Affine Coupling Flows}\footfullcite{dinh2017realnvp} -- \textit{Forward}}
    ...
    \vspace{0.3em}
    \begin{itemize}
      \item A \emph{coupling network} $\thetav(\mathbf{x}^{A})$ produz
    ...
    \begin{center}
      \resizebox{!}{0.50\textheight}{%
        \begin{tikzpicture}[
          ...
          % --- caixas ---
          \node[box] (cn) at (6.2,2.05) {Coupling\\Network $\thetav(\mathbf{y}^{A})$};
  ...
  \begin{frame}[fragile,containsverbatim]{\textit{Affine Coupling Flows} -- PyTorch}
    ...
    \vspace{0.3em}
    \tiny \textbf{Ideias-chave:} split em $A/B$; rede prediz $(\mathbf{s},\mathbf{t})$ a partir
