...
\begin{itemize}
    \item Uma função $f$ aplicada a uma imagem $x$ é dita \textbf{invariante} a uma transformação $t$ se:
\end{itemize}
\begin{equation*}
    f\bigl[t[x]\bigr] = f[x]
\end{equation*}
...
\begin{columns}[T]
    \begin{column}{.5\linewidth}
        \begin{itemize}
            \item Uma função $f$ aplicada a uma imagem $x$ é dita \textbf{equivariante} a uma transformação $t$ se:
        \end{itemize}
        \begin{equation*}
            f\bigl[t[x]\bigr] = t\bigl[f[x]\bigr]
        \end{equation*}
        \begin{itemize}
...
\begin{columns}[T]
    \begin{column}{.5\linewidth}
        \begin{itemize}
            \item A convolução transforma uma imagem de entrada $x$ em uma imagem de saída $z$. Cada posição $(i,j)$ resulta de uma combinação linear de uma janela $3\times 3$ ao redor:
        \end{itemize}
        \begin{equation*}
...
\vspace{0.6em}
\begin{itemize}
    \item Na camada totalmente conectada, $x$ é a imagem achatada em um vetor de $D$ elementos.
...
\end{itemize}
\begin{equation*}
    w \in \mathbb{R}^{C_i \times C_o \times K \times K}, \qquad b \in \mathbb{R}^{C_o}
\end{equation*}
\begin{block}{Exemplo}
...
\begin{columns}
    ...
    \begin{column}{.45\linewidth}
        ...
        \begin{tikzpicture}[every node/.style={font=\small}]
            ...
            \draw[->, thick] (-2.0,0.7) node[left]{$x$} -- (conv);
            \draw[->, thick] (-2.0,0) node[left]{$w$} -- (conv);
            \draw[->, thick] (-2.0,-0.7) node[left]{$b$} -- (conv);
            \draw[->, thick] (conv) -- (1.8,0) node[right]{$y$};
            \node[BrickRed, font=\scriptsize] at (-1.6,0.95) {$\partial x$};
            \node[BrickRed, font=\scriptsize] at (-1.6,0.25) {$\partial w$};
            \node[BrickRed, font=\scriptsize] at (-1.6,-0.45) {$\partial b$};
            \node[BrickRed, font=\scriptsize] at (1.4,0.25) {$\partial y$};
        \end{tikzpicture}
    \end{column}
\end{columns}
\end{frame}
...
\begin{tikzpicture}[every node/.style={font=\scriptsize}, scale=0.62,
    ...
    \node at (3.5,1.0) {$2$}; \node at (4.5,1.0) {$0$}; \node at (5.5,1.0) {$1$};
    \node at (3.5,0.0) {$0$}; \node at (4.5,0.0) {$2$}; \node at (5.5,0.0) {$1$};
    \node[font=\tiny, anchor=south] at (4.5,2.55) {kernel $\mathbf{w}$};
...
\begin{itemize}
    \item Definimos a entrada $X$ ($3\times 3$) e o filtro $w$ ($2\times 2$):
\end{itemize}
\vspace{0.4em}
\centering
...
\begin{itemize}
    \item Modo \textit{full}: $(X_h + w_h - 1)\times(X_w + w_w - 1)$. Para $X$ $3\times 3$ e $w$ $2\times 2$:
\end{itemize}
\begin{equation*}
...
\begin{itemize}
    \item Para cada linha do filtro paddado, construímos uma matriz de Toeplitz $F_k$ (tamanho saída$_h \times$ entrada$_w = 4\times 3$). Começamos pela última linha.
\end{itemize}
\vspace{0.3em}
\centering
\begin{tikzpicture}[every node/.style={font=\tiny}, scale=0.55]
    % F0 azul (da linha 4 do padded: w_{2,1}, w_{2,2}, 0, 0)
    \node[anchor=south, font=\scriptsize] at (1.5,4.2) {$F_0$};
    ...
    \node[anchor=south, font=\scriptsize] at (5.5,4.2) {$F_1$};
    ...
    \node[anchor=south, font=\scriptsize] at (9.5,4.2) {$F_2$};
    ...
    \node[anchor=south, font=\scriptsize] at (13.5,4.2) {$F_3$};
...
\vspace{0.3em}
\begin{itemize}
    \item $F_0$ vem da última linha do filtro paddado, $F_1$ da penúltima, e assim por diante.
\end{itemize}
\end{frame}
...
\begin{columns}[T]
    \begin{column}{.45\linewidth}
        ...
        \text{Bloco Toeplitz} =
        \begin{bmatrix}
        \textcolor{PastelBlue2!60!black}{F_0} & 0 & 0 \\
        \textcolor{black!50}{F_1} & \textcolor{PastelBlue2!60!black}{F_0} & 0 \\
        \textcolor{orange!70!black}{F_2} & \textcolor{black!50}{F_1} & \textcolor{PastelBlue2!60!black}{F_0} \\
        \textcolor{green!50!black}{F_3} & \textcolor{orange!70!black}{F_2} & \textcolor{black!50}{F_1}
        \end{bmatrix}
        \end{equation*}
    \end{column}
    \begin{column}{.55\linewidth}
...
\begin{itemize}
    \item Vetorizamos $X$ concatenando as linhas \textbf{de baixo para cima}:
\end{itemize}
\vspace{0.4em}
\centering
...
\end{itemize}
\begin{equation*}
    \underbrace{T}_{16 \times 9} \cdot \underbrace{\vec{X}}_{9 \times 1} = \underbrace{\vec{y}}_{16 \times 1}
    \;\;\xrightarrow{\;\text{reshape}\;}\;\;
    \underbrace{Y}_{4 \times 4}
\end{equation*}
\begin{itemize}
    \item Cada linha do produto $T\vec{X}$ já contém o resultado de uma posição da convolução; basta reorganizar em uma matriz $4\times 4$.
