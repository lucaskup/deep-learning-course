...
\end{enumerate}
    ...
    \item Dada uma imagem de entrada de $32\times32$ pixels, e duas camadas convolucionais, qual o campo receptivo na imagem original que é representado por um único pixel na saída final? Considere: Kernel para as duas camadas ($K_1 = K_2$) tem tamanho $5\times5$; Stride = 1; Dillation = 2.
...
\item Resolução Input 3x3 % resolução questão 8 VIII
    ...
    $$
    $$
    C_1 = (7\cdot1 + 8\cdot0 + 9\cdot-1) + (6\cdot1 + 5\cdot0 + 4\cdot-1) + (1\cdot1 + 2\cdot0 + 3\cdot-1)
    $$
    $$
    C_1 = (-2) + (2) + (-2) = -2
    $$
    $$
    C_1 = \begin{bmatrix} -2 \end{bmatrix}
    $$

    Camada 2 (Conv2)
    Input $1\times1$, Kernel $3\times3$, $p=1$, $s=1 \implies$ Saída $1\times1$.
    Primeiro, aplicamos padding na saída de $C_1$:
    $$
    C_{1, padded} = \begin{bmatrix}
    0 & 0 & 0 \\
    0 & -2 & 0 \\
    0 & 0 & 0
    \end{bmatrix}
