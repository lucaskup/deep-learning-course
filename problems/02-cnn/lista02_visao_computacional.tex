...
\item Nas duas tarefas o modelo produz classe e \textit{bounding box}: a localização é classificação $+$ localização de um objeto, e a detecção faz o mesmo para cada objeto encontrado. A diferença está no \textbf{número de objetos}. Na localização supomos um único objeto de interesse, então a saída tem tamanho fixo e uma cabeça densa com 4 unidades para a caixa resolve o problema. Na detecção a imagem pode conter vários objetos e não sabemos de antemão quantos, então a saída tem tamanho variável, o que é o desafio central da tarefa e o motivo de precisarmos de mecanismos como propostas de região, âncoras ou consultas aprendidas, além de supressão de detecções redundantes.
