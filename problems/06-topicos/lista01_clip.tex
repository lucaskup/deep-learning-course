...
\paragraph{Instruções:} Responda às questões de múltipla escolha assinalando apenas uma alternativa. Nas questões dissertativas, apresente o raciocínio passo a passo, incluindo fórmulas e cálculos intermediários quando solicitado. Mantenha a notação $\mathbf{S}$ para a matriz de similaridades, $\tau$ para a temperatura e $\mathcal{L}$ para a perda, conforme utilizadas em aula.
...
\begin{enumerate}[I)]
    ...
    \item Em um \textit{batch} de $N$ pares (imagem, texto) durante o treino do CLIP, quantos pares são tratados como positivos e quantos como negativos na matriz $\mathbf{S} \in \mathbb{R}^{N \times N}$?
    \begin{enumerate}[(a)]
    ...
    \item Antes de compor a matriz $\mathbf{S}$ usada na \textit{InfoNCE}, o CLIP projeta as saídas dos \textit{encoders} em um espaço comum e normaliza os vetores. Qual operação de similaridade entre um vetor de imagem $\mathbf{u}$ e um vetor de texto $\mathbf{v}$ é efetivamente computada após essa normalização?
    \begin{enumerate}[(a)]
    ...
    \item Em uma linha da matriz $\mathbf{S}$ obtém-se os logits $(0.96,\, 0.64)$ associados ao par positivo (coluna 1) e a um negativo (coluna 2). Aplicando $\mathrm{softmax}$ a $\mathbf{S}/\tau$, qual coluna recebe maior probabilidade quando $\tau = 0.1$ em comparação com $\tau = 1.0$?
    \begin{enumerate}[(a)]
    ...
    \[
        \mathbf{S}/\tau = \begin{pmatrix} 9.6 & 6.4 \\ 3.6 & 9.6 \end{pmatrix}.
    \]
    ...
    \begin{enumerate}[(a)]
        \item Garantir que a matriz $\mathbf{S}$ seja simétrica em todos os passos de treino.
    ...
    \begin{enumerate}[(a)]
        ...
        \item LoRA, que adiciona produtos $\mathbf{A}\mathbf{B}^{\top}$ de baixo posto às matrizes de pesos congeladas.
        %d
    \end{enumerate}
...
\begin{enumerate}[I)]
    ...
    \[
        \mathbf{S} = \begin{pmatrix} 0.96 & 0.64 \\ 0.36 & 0.96 \end{pmatrix}.
    \]
    \begin{enumerate}[(a)]
        \item Escreva a expressão de $\mathcal{L}_{i \to t}$ em função de $\mathbf{S}$ e $\tau$ e explique, em uma frase, qual é o papel da temperatura.
        \item Calcule $\mathcal{L}_{i \to t}$ para $\tau = 1.0$ e para $\tau = 0.1$. Apresente os valores das duas linhas de $\mathrm{softmax}(\mathbf{S}/\tau)$ e a perda final em cada caso.
...
\begin{enumerate}[I)]
    ...
    \item \textbf{(a)} A diagonal contém os $N$ pares verdadeiros, enquanto as $N^2 - N = N(N-1)$ entradas fora da diagonal funcionam como negativos ``de graça'' obtidos do próprio \textit{batch}.
    ...
    \item \textbf{(d)} Os \textit{encoders} são independentes (uma CNN ou ViT para imagens, um \textit{transformer} para texto), treinados conjuntamente, e só interagem no espaço de projeção comum onde se calcula a similaridade. Não há compartilhamento de pesos.

    \item \textbf{(a)} O procedimento padrão é codificar a imagem, codificar as $K$ descrições de classe como texto, calcular as similaridades e tomar $\arg\max$. Esse é o pipeline mostrado no exemplo trabalhado III da aula.
    ...
    \item \textbf{(d)} LoRA insere produtos de baixo posto $\mathbf{A}\mathbf{B}^{\top}$ em camadas de atenção ou \textit{feed-forward}, treinando apenas $\mathbf{A}$ e $\mathbf{B}$ e mantendo o resto congelado. O \textit{linear probe} treina apenas a camada final, \textit{adapters} adicionam blocos não lineares completos, e \textit{full fine-tuning} ajusta todos os pesos.

    \item \textbf{(d)} Contagem precisa de objetos é uma falha conhecida do CLIP, junto com leitura de texto em imagens, tarefas de granularidade fina e raciocínio espacial. Recuperação semântica e classificação \textit{zero-shot} em classes amplas estão entre seus pontos fortes.
\end{enumerate}
