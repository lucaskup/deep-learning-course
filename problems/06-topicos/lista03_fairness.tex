...
\begin{enumerate}[I)]
    \item \textbf{(c)} O problema está no rótulo: ``bom pagador'' foi definido a partir de decisões passadas já enviesadas (poucos bairros recebiam crédito), então o alvo aprendido não mede a capacidade real de pagamento, e sim o padrão histórico. Isso é viés histórico. Não é amostragem (não se trata de número de exemplos por grupo), nem medição (a renda não foi medida com instrumento distinto), nem agregação (não há um único modelo forçado sobre subpopulações com relações distintas; o defeito está na variável-alvo).

    \item \textbf{(b)} Remover o atributo protegido (\textit{fairness through unawareness}) não garante \textit{fairness}: atributos correlacionados (clubes, cursos) funcionam como \textit{proxies} e reconstroem a informação removida. A alternativa (a) está errada porque a disparidade é justamente esperada via \textit{proxies}; (c) inverte o que a remoção garante, pois ela não garante nenhuma das duas noções; (d) calibração é outro critério e nada nos números informados a sustenta.

    \item \textbf{(d)} $TPR = \frac{TP}{TP+FN} = \frac{70}{70+30} = \frac{70}{100} = 0{,}70$. A alternativa (c) traz $\frac{TP}{TP+FP}=\frac{70}{150}\approx 0{,}47$, que é o $PPV$, fácil de confundir com o $TPR$; (a) é $\frac{TP}{P_{\text{previstos}}}$ aproximado e (b) corresponde a outra razão da matriz, ambos cálculos com o denominador errado.
    ...
    \item \textbf{(c)} $TPR_0 = \frac{160}{160+40} = \frac{160}{200} = 0{,}80$ e $TPR_1 = \frac{70}{70+30} = \frac{70}{100} = 0{,}70$. A lacuna é $|0{,}80 - 0{,}70| = 0{,}10$. O valor $0{,}25$ em (b) é o $FPR$ comum aos dois grupos, medido no lugar do $TPR$; (a) e (d) resultam de usar as células erradas da matriz.

    \item \textbf{(b)} A igualdade de chances exige $TPR$ \textbf{e} $FPR$ iguais. Os $FPR$ coincidem: $FPR_0 = \frac{50}{50+150} = 0{,}25$ e $FPR_1 = \frac{25}{25+75} = 0{,}25$. Mas os $TPR$ diferem: $0{,}80$ contra $0{,}70$. Como basta uma das duas igualdades falhar, o critério está violado. A alternativa (a) afirma falsamente que as duas taxas coincidem; (c) inverte qual taxa difere; (d) confunde \textit{equalized odds} com paridade demográfica.

    \item \textbf{(a)} $TPR = 1 - FNR = 1 - 0{,}28 = 0{,}72$. A alternativa (c) repete o $FPR$, e (d) repete o $FNR$, ambos confundindo qual taxa foi pedida.

    \item \textbf{(d)} A igualdade de $PPV$ entre grupos é exatamente a versão binarizada da calibração ($P(y{=}1\mid \hat{y}{=}1)$), aqui $62\%$ contra $61\%$, aproximadamente satisfeita. A fração de bons pagadores marcados como alto risco que difere é o $FNR$ (entre $y{=}1$, os que recebem $\hat{y}{=}0$). As demais alternativas trocam o critério satisfeito ($TPR$, paridade, $FPR$), que não corresponde a $P(y{=}1\mid\hat{y}{=}1)$.

    \item \textbf{(c)} Com $PPV$ e $FNR$ iguais nos dois grupos, na identidade só varia o fator $\frac{p}{1-p}$, que cresce com $p$. Como $p_1 > p_0$, o grupo $a{=}1$ tem $\frac{p}{1-p}$ maior e, portanto, $FPR$ maior; os $FPR$ não podem coincidir. (a) e (b) ignoram a dependência em $p$; (d) inverte o papel do classificador perfeito, que é justamente a exceção em que o teorema não se aplica.

    \item \textbf{(b)} Sem acesso aos pesos nem ao treino e com prazo curto, só os métodos inferenciais se aplicam: atuam na inferência (\textit{prompting}, edição de saída) sem alterar parâmetros e servem modelos de terceiros via API. Distribucionais (a) e \textit{one-step} (c) exigem retreinar; \textit{two-step} (d) exige acesso aos pesos para a segunda etapa de treino, indisponível aqui.

    \item \textbf{(d)} O \textit{two-step-training} parte do modelo já treinado e adiciona uma etapa barata (\textit{fine-tuning}, adapters), viável em foundation models (mesma economia do PEFT) e mais persistente que correções só na inferência. Distribucional (a) e \textit{one-step} (b) implicam retreinar do zero, proibitivo na escala dada; a edição na inferência (c) é justamente o tipo de correção reversível que o enunciado quer evitar.
