...
\paragraph{Instruções:} Responda às questões de múltipla escolha assinalando apenas uma alternativa. Nas questões dissertativas, apresente o raciocínio passo a passo, incluindo fórmulas e cálculos intermediários quando solicitado. Mantenha a notação utilizada em aula: $p_X(\mathbf{x})$ para a densidade modelada, $p_Z(\mathbf{z})$ para a distribuição base, $f$ para a transformação inversível com parâmetros $\thetav$, e $J = \partial f / \partial \mathbf{z}$ para a Jacobiana. Em camadas \textit{coupling}, $\mathbf{x}^A$ é a metade copiada e $\mathbf{x}^B$ é a metade transformada. Questões marcadas com \emoji{skull-and-crossbones} são consideradas mais difíceis.
...
\begin{enumerate}[I)]
    ...
    % --------------------------------------------------------------
    \item Você precisa de um modelo gerador que, para qualquer ponto $\mathbf{x}$ no espaço de dados, devolva o valor exato de $\log p_X(\mathbf{x})$ (e não apenas um limite inferior ou um sinal de discriminador). Considerando as famílias VAE, GAN e \textit{Normalizing Flow}, qual afirmação descreve corretamente o que cada uma oferece nesse aspecto?
    \begin{enumerate}[(a)]
        \item VAE fornece $\log p_X(\mathbf{x})$ exato pelo \textit{decoder}; GAN fornece um limite inferior; NF, apenas amostras.
        \item GAN fornece $\log p_X(\mathbf{x})$ exato via discriminador; VAE fornece apenas amostras; NF, um limite inferior.
        \item NF fornece $\log p_X(\mathbf{x})$ exato via mudança de variável; VAE fornece um limite inferior; GAN, apenas amostras.
        \item As três famílias fornecem $\log p_X(\mathbf{x})$ exato, diferindo apenas na qualidade visual das amostras.
        %c
    \end{enumerate}
    ...
    % --------------------------------------------------------------
    \item Em uma camada \textit{affine coupling} aplicada a $\mathbf{x} = (\mathbf{x}^A, \mathbf{x}^B) = (2,\; 3)$, a \textit{coupling network} produz $s(\mathbf{x}^A) = -0.5$ e $t(\mathbf{x}^A) = 1.0$. Aplicando a regra \emph{forward} $\mathbf{y}^B = \mathbf{x}^B \odot \exp(\mathbf{s}) + \mathbf{t}$, $\mathbf{y}^A = \mathbf{x}^A$, qual é o valor de $\mathbf{y} = (y^A, y^B)$? Use $e^{-0.5} \approx 0.607$.
    \begin{enumerate}[(a)]
        \item $(2,\; 2.82)$.
        \item $(2,\; 1.82)$.
        \item $(2,\; 5.95)$.
    ...
    \[
        \mathcal{L}(\thetav) = \sum_{i=1}^{N}\left[\log\left|\det \tfrac{\partial f(\mathbf{z}^{(i)},\thetav)}{\partial \mathbf{z}^{(i)}}\right| - \log p_Z(\mathbf{z}^{(i)})\right],
    \]
    onde $p_Z$ é a distribuição base (gaussiana padrão) e $\mathbf{z}^{(i)} = f^{-1}(\mathbf{x}^{(i)}, \thetav)$. Sobre os dois termos, qual afirmação é correta?
    \begin{enumerate}[(a)]
        \item O termo $\log|\det J|$ depende de $\thetav$; o termo $-\log p_Z(\mathbf{z}^{(i)})$ é constante em $\thetav$ e pode ser omitido do gradiente sem alterar o treino.
        \item Ambos os termos dependem de $\thetav$: o primeiro pela Jacobiana da rede e o segundo pelo ponto de avaliação $\mathbf{z}^{(i)} = f^{-1}(\mathbf{x}^{(i)}, \thetav)$.
        \item O termo $-\log p_Z(\mathbf{z}^{(i)})$ depende de $\thetav$ pela parametrização da média e variância da gaussiana base; $\log|\det J|$ permanece constante.
        \item Nenhum dos dois termos depende de $\thetav$, pois ambos são avaliados em pontos $\mathbf{z}^{(i)}$ fixados antes do treinamento começar.
        %b
    \end{enumerate}
    ...
    % --------------------------------------------------------------
    \item Em um \textit{linear flow} $f(\mathbf{x}) = \mathbf{\Theta}\mathbf{x} + \mathbf{b}$ com $\mathbf{x} \in \mathbb{R}^d$ e $\mathbf{\Theta}$ densa, calcular $\det(\mathbf{\Theta})$ e $\mathbf{\Theta}^{-1}$ a cada passo de treino custa $\mathcal{O}(d^3)$. Estruturas como decomposição LU ou convoluções $1{\times}1$ (Glow) são adotadas porque:
    \begin{enumerate}[(a)]
...
\begin{enumerate}[I)]
    ...
    \[
        \hat{\thetav} = \argmax_{\thetav}\prod_{i=1}^{N} p_X\!\left(\mathbf{x}^{(i)} \mid \thetav\right),
    \]
    ...
    \begin{enumerate}[(a)]
        ...
        \item Substitua $p_X(\mathbf{x}^{(i)} \mid \thetav)$ usando a fórmula de mudança de variável $p_X(\mathbf{x}) = p_Z(\mathbf{z})\left|\partial f / \partial \mathbf{z}\right|^{-1}$, com $\mathbf{z} = f^{-1}(\mathbf{x}, \thetav)$.
...
\begin{enumerate}[I)]
    ...
    \item \textbf{(c)} \textit{Normalizing flows} fornecem $\log p_X(\mathbf{x})$ exato porque a mudança de variável devolve uma fórmula fechada em função de $p_Z(\mathbf{z})$ e do log-determinante. VAEs maximizam o ELBO, que é apenas um limite inferior de $\log p_X(\mathbf{x})$, não o valor exato. GANs treinam por jogo entre gerador e discriminador e não dão acesso a uma densidade.
    ...
    \item \textbf{(b)} Ambos os termos recebem gradiente. O $\log|\det J|$ depende de $\thetav$ diretamente, pela Jacobiana da rede, e penaliza camadas que contraem volume de forma exagerada para "trapacear" empurrando toda a massa para uma região pequena. O termo $-\log p_Z(\mathbf{z})$ tem forma fechada fixa (gaussiana padrão), mas o ponto de avaliação $\mathbf{z}^{(i)} = f^{-1}(\mathbf{x}^{(i)}, \thetav)$ se move quando $\thetav$ muda; é esse termo que puxa os latentes para a região de alta densidade da base. A (a) é o erro clássico: se o termo da base fosse omitido do gradiente, a rede minimizaria a perda apenas contraindo volume, sem ajustar a distribuição aos dados. A (c) inverte os papéis, a base é fixa e a Jacobiana não é constante. A (d) é falsa, $\mathbf{z}^{(i)}$ é recalculado a cada passo de treino.
...
\begin{enumerate}[I)]
    \item
    \begin{enumerate}[(a)]
        \item Tomando o logaritmo e trocando o sinal, $\hat{\thetav} = \argmin_{\thetav}\sum_{i=1}^{N}\left[-\log p_X(\mathbf{x}^{(i)} \mid \thetav)\right]$. A vantagem é numérica: o produto de $N$ probabilidades pequenas sofre \textit{underflow} rapidamente em ponto flutuante; somar logaritmos é estável e produz gradientes bem condicionados.
        \item Pela mudança de variável, $p_X(\mathbf{x}) = p_Z(f^{-1}(\mathbf{x}, \thetav))\left|\partial f / \partial \mathbf{z}\right|^{-1}$. Aplicando $-\log$, obtém-se $-\log p_X(\mathbf{x}) = -\log p_Z(\mathbf{z}) + \log|\partial f / \partial \mathbf{z}|$, com $\mathbf{z} = f^{-1}(\mathbf{x}, \thetav)$.
        \item Soma final:
        \[
            \mathcal{L}(\thetav) = \sum_{i=1}^{N}\left[\log\left|\tfrac{\partial f(\mathbf{z}^{(i)},\thetav)}{\partial \mathbf{z}^{(i)}}\right| - \log p_Z(\mathbf{z}^{(i)})\right].
        \]
        O termo $\log|\det J|$ depende de $\thetav$ diretamente (via a Jacobiana da rede $f$) e penaliza configurações que comprimem volume excessivamente, que de outra forma deixariam a verossimilhança artificialmente alta. O termo $-\log p_Z(\mathbf{z})$ tem densidade em forma fechada e fixa (gaussiana padrão, em geral), mas também depende de $\thetav$, através do ponto de avaliação $\mathbf{z} = f^{-1}(\mathbf{x}, \thetav)$; é ele que puxa os latentes para a região de alta densidade da base.
    \end{enumerate}
    ...
    \begin{enumerate}[(a)]
        ...
        \item \textbf{VAE}: (i) avaliação aproximada via ELBO (limite inferior de $\log p_X(\mathbf{x})$, não o valor exato); (ii) amostragem rápida (uma passagem pelo \textit{decoder}); (iii) qualidade visual em geral inferior à de GANs e modelos difusores, com tendência a amostras borradas devido à perda de reconstrução pixel a pixel.
        \item \textbf{Normalizing Flow}: (i) avaliação exata de $\log p_X(\mathbf{x})$ pela fórmula de mudança de variável; (ii) amostragem rápida (uma passagem direta pelo \textit{flow}); (iii) qualidade visual inferior a GANs e modelos difusores em \textit{datasets} grandes como ImageNet/FFHQ, em parte pela restrição arquitetural (inversibilidade) que limita a expressividade por parâmetro.

        Conclusão: NFs ocupam o canto \emph{velocidade $+$ cobertura} (com avaliação exata) do trilema. Aplicações em que esse canto é preferível: detecção de anomalias (precisamos de $\log p_X(\mathbf{x})$ para definir limiar), inferência variacional (substituir $q(\mathbf{z}\mid \mathbf{x})$ por uma distribuição expressiva e ainda tratável), física computacional e \textit{Boltzmann generators} (amostragem em equilíbrio com densidade exata para reponderação).
    \end{enumerate}
