...
\paragraph{Instruções:} Responda às questões de múltipla escolha assinalando apenas uma alternativa. Nas dissertativas, apresente o raciocínio passo a passo, incluindo fórmulas e cálculos intermediários quando solicitado. Mantenha a notação dos slides: $\beta_t$ para o \textit{noise schedule}, $\alpha_t = \prod_{s=1}^{t}(1 - \beta_s)$ para o produto acumulado, $z_t$ para o estado em $t$, $\epsilon$ para o ruído gaussiano e $g_t[z_t,\theta_t]$ para a rede que estima o ruído. Questões marcadas com \emoji{skull-and-crossbones} são consideradas difíceis.
...
% ------------------------------------------------------------------
\item Considere o kernel de difusão $z_t = \sqrt{\alpha_t}\, x + \sqrt{1 - \alpha_t}\, \epsilon$, com $\epsilon \sim \mathcal{N}(0, \mathbf{I})$. Dado $\alpha_t = 0.64$, $x = 2$ e $\epsilon = 0.5$, qual é o valor amostrado de $z_t$?
\begin{enumerate}[(a)]
    \item $1.90$.
    \item $1.60$.
    \item $1.46$.
...
% ------------------------------------------------------------------
\item No processo de difusão, $q(z_t \mid x) = \mathcal{N}_{z_t}[\mu,\, \Sigma]$. Para $\alpha_t = 0.49$ e $x = 4$, quais são, respectivamente, a média $\mu$ e a variância $\Sigma$ (esta última como múltiplo de $\mathbf{I}$)?
\begin{enumerate}[(a)]
    \item $\mu = 2.80$ e $\Sigma = 0.51\,\mathbf{I}$.
    \item $\mu = 1.96$ e $\Sigma = 0.49\,\mathbf{I}$.
    \item $\mu = 2.80$ e $\Sigma = 0.71\,\mathbf{I}$.
    \item $\mu = 2.04$ e $\Sigma = 0.51\,\mathbf{I}$.
\end{enumerate}
% R: a
...
% ------------------------------------------------------------------
\item Após a reparametrização que faz a rede $g_t[z_t,\theta_t]$ predizer o ruído, a função de custo do modelo difusor (excluindo o termo de reconstrução) assume qual forma?
\begin{enumerate}[(a)]
    \item $\sum_{i,t} \big|g_t[z_t,\theta_t] - x^{(i)}\big|^{2}$.
    \item $\sum_{i,t} \big|g_t[z_t,\theta_t] - z_{t-1}\big|^{2}$.
    \item $\sum_{i,t} \big|g_t[z_t,\theta_t] - \epsilon_t\big|^{2}$.
    \item $\sum_{i,t} -\log g_t[z_t,\theta_t]\,\epsilon_t$.
\end{enumerate}
% R: c
...
\begin{enumerate}[(a)]
    \item A transição $q(z_t \mid z_{t-1})$ depende explicitamente de $x$ e de todos os estados anteriores.
    \item A cadeia é markoviana, com $q(z_t \mid z_{t-1}, x) = q(z_t \mid z_{t-1})$, e a variância da transição é $\beta_t\,\mathbf{I}$.
    \item A transição é determinística e a aleatoriedade entra somente em $z_T \sim \mathcal{N}(0,\mathbf{I})$.
    \item A média de $q(z_t \mid z_{t-1})$ é $\sqrt{\alpha_t}\, z_{t-1}$, com variância $(1 - \alpha_t)\,\mathbf{I}$.
\end{enumerate}
% R: b
...
% ------------------------------------------------------------------
\item Nos slides, a rede $g_t$ é treinada para predizer o ruído $\epsilon$ adicionado, e não $x$ ou $z_{t-1}$ diretamente. Qual a justificativa apresentada para essa escolha?
\begin{enumerate}[(a)]
    ...
    \item Predizer o ruído é uma observação empírica: na prática produz melhores resultados que predizer $x$ diretamente.
...
% ------------------------------------------------------------------
\item A distribuição condicional $q(z_{t-1} \mid z_t, x)$, derivada no curso, é uma gaussiana cuja média é uma combinação linear de $z_t$ e $x$. Para $\beta_t = 0.2$, $\alpha_{t-1} = 0.9$ e $\alpha_t = 0.72$, qual é, respectivamente, o coeficiente de $z_t$ e o coeficiente de $x$ na média?
\begin{enumerate}[(a)]
    \item Coef. $z_t \approx 0.319$; coef. $x \approx 0.678$.
    \item Coef. $z_t \approx 0.894$; coef. $x \approx 0.200$.
    \item Coef. $z_t \approx 0.357$; coef. $x \approx 0.678$.
    \item Coef. $z_t \approx 0.849$; coef. $x \approx 0.300$.
\end{enumerate}
% R: a
...
\begin{enumerate}[I)]
    ...
    \item Como $q(z_t \mid x) = \mathcal{N}[\sqrt{\alpha_t}\,x,\,(1 - \alpha_t)\,\mathbf{I}]$, temos $\mu = \sqrt{0.49}\cdot 4 = 0.7\cdot 4 = 2.80$ e $\Sigma = (1 - 0.49)\,\mathbf{I} = 0.51\,\mathbf{I}$. A (b) usa $\alpha_t$ em vez de $\sqrt{\alpha_t}$ na média. A (c) confunde a variância com o desvio padrão $\sqrt{1 - \alpha_t} \approx 0.71$. A (d) usa $(1 - \alpha_t)\,x = 0.51 \cdot 4 = 2.04$ na média, trocando o papel de $\alpha_t$ com o de $1 - \alpha_t$, embora acerte a variância.
    \item Substituindo a reparametrização da rede no termo quadrático da \textit{loss}, todos os termos em $z_t$ e $x$ se cancelam, restando $|g_t[z_t,\theta_t] - \epsilon_t|^{2}$, conforme caixa final no slide ``Reparametrizando a rede, Loss final''. A (a) ignora a reparametrização. A (b) corresponde ao alvo da formulação anterior. A (d) tem forma de log-verossimilhança e não corresponde a um MSE.
    \item Pela definição $q(z_t \mid z_{t-1}) = \mathcal{N}[\sqrt{1 - \beta_t}\,z_{t-1},\, \beta_t\,\mathbf{I}]$, vale a propriedade de Markov com variância $\beta_t\,\mathbf{I}$. A (a) viola a propriedade de Markov. A (c) descreve um processo determinístico, o que não é o caso. A (d) confunde a média da transição passo a passo (que usa $\sqrt{1 - \beta_t}$) com a do kernel acumulado (que usa $\sqrt{\alpha_t}$).
    ...
    \item No slide ``Porém\ldots\ um detalhe empírico'' fica explícito: a escolha de predizer $\epsilon$ é uma observação empírica de que treinar a rede para estimar o ruído produz melhores resultados que estimar $x$ diretamente. As alternativas (a), (b) e (d) descrevem consequências falsas dessa reparametrização.
    \item Pelo slide ``Distribuição condicional, Produto de gaussianas'', a média é $\tfrac{(1 - \alpha_{t-1})\sqrt{1 - \beta_t}}{1 - \alpha_t}\, z_t + \tfrac{\sqrt{\alpha_{t-1}}\,\beta_t}{1 - \alpha_t}\, x$. Substituindo os valores dados, o coeficiente de $z_t$ é $\tfrac{(1 - 0.9)\sqrt{1 - 0.2}}{1 - 0.72} = \tfrac{0.1\cdot 0.8944}{0.28} \approx 0.319$, e o coeficiente de $x$ é $\tfrac{\sqrt{0.9}\cdot 0.2}{0.28} = \tfrac{0.9487\cdot 0.2}{0.28} \approx 0.678$. A (b) confunde o coeficiente de $z_t$ com $\sqrt{1 - \beta_t}$ e o de $x$ com $\beta_t$ puro (esquece a normalização por $1 - \alpha_t$). A (c) inflaciona o coeficiente de $z_t$ ao usar $1 - \beta_t$ no lugar de $\sqrt{1 - \beta_t}$. A (d) confunde com a média de $q(z_{t-1} \mid x)$ derivada do kernel direto, sem a correção bayesiana introduzida pela condição em $z_t$.
...
% ------------------------------------------------------------------
\item Seja o \textit{noise schedule} $\boldsymbol{\beta} = (0.1,\, 0.3)$ com $T = 2$ e $x = 1$.
\begin{enumerate}[(a)]
    \item Calcule $\alpha_1$ e $\alpha_2$.
    \item Usando o kernel de difusão, escreva $q(z_2 \mid x)$ explicitamente (média e variância).
    \item Para $\epsilon = -1.0$, calcule o valor amostrado de $z_2$ via kernel.
    \item Compare $z_2$ com a média da distribuição $q(z_2 \mid x)$. Em uma frase, interprete o resultado.
\end{enumerate}

% ------------------------------------------------------------------
% D2 — Hard (derivation of kernel for T=2)
% ------------------------------------------------------------------
\item \emoji{skull-and-crossbones} Partindo das definições passo a passo $z_1 = \sqrt{1 - \beta_1}\,x + \sqrt{\beta_1}\,\epsilon_1$ e $z_2 = \sqrt{1 - \beta_2}\,z_1 + \sqrt{\beta_2}\,\epsilon_2$, com $\epsilon_1, \epsilon_2 \sim \mathcal{N}(0,\mathbf{I})$ independentes, mostre algebricamente que existe um único ruído $\epsilon \sim \mathcal{N}(0,\mathbf{I})$ tal que $z_2 = \sqrt{\alpha_2}\,x + \sqrt{1 - \alpha_2}\,\epsilon$, onde $\alpha_2 = (1 - \beta_1)(1 - \beta_2)$. Indique claramente onde a propriedade ``soma de gaussianas independentes'' é usada.
...
% ------------------------------------------------------------------
\item \emoji{skull-and-crossbones} Em \textit{classifier-free guidance}, treina-se uma única rede que pode operar em modo condicional, isto é, $g_t[z_t, c]$, ou incondicional, $g_t[z_t, \varnothing]$. Na amostragem, define-se um estimador guiado $\tilde{g}_t = (1 + w)\, g_t[z_t, c] - w\, g_t[z_t, \varnothing]$, com $w \geq 0$.
\begin{enumerate}[(a)]
...
\begin{enumerate}[I)]
    ...
    \textbf{(b)} Pelo kernel, $q(z_2 \mid x) = \mathcal{N}_{z_2}[\sqrt{\alpha_2}\, x,\, (1 - \alpha_2)\,\mathbf{I}] = \mathcal{N}_{z_2}[\sqrt{0.63}\cdot 1,\, 0.37\,\mathbf{I}] \approx \mathcal{N}_{z_2}[0.794,\, 0.37\,\mathbf{I}]$.

    \textbf{(c)} $z_2 = \sqrt{0.63}\cdot 1 + \sqrt{0.37}\cdot (-1.0) \approx 0.794 + 0.608\cdot (-1.0) \approx 0.794 - 0.608 \approx 0.186$.

    \textbf{(d)} O valor amostrado $z_2 \approx 0.186$ está abaixo da média $\approx 0.794$ por aproximadamente um desvio-padrão ($\sqrt{0.37} \approx 0.608$), o que reflete a contribuição de um ruído $\epsilon = -1.0$ no kernel. Em uma população grande de amostras com o mesmo $x$, a média empírica de $z_2$ convergiria para $0.794$.

    \item Partindo de $z_2 = \sqrt{1 - \beta_2}\,z_1 + \sqrt{\beta_2}\,\epsilon_2$ e substituindo $z_1$:
    \[
    ...
    \]
    Como $\epsilon_1$ e $\epsilon_2$ são gaussianas independentes de média zero e variância $1$, a soma $\sqrt{(1 - \beta_2)\beta_1}\,\epsilon_1 + \sqrt{\beta_2}\,\epsilon_2$ é uma gaussiana de média zero e variância
    \[
    ...
    \[
      z_2 = \sqrt{\alpha_2}\,x + \sqrt{1 - \alpha_2}\,\epsilon, \qquad \alpha_2 = (1 - \beta_1)(1 - \beta_2). \qed
    \]
    A propriedade ``soma de gaussianas independentes é gaussiana com variâncias somadas'' é usada exatamente no passo onde combinamos $\epsilon_1$ e $\epsilon_2$ em um único $\epsilon$.
