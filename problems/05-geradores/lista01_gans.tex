...
% ------------------------------------------------------------------
\item Para uma única amostra \textbf{real} ($y = 1$), o discriminador devolve $\sigma(f[x,\phi]) = 0.20$. Qual é a contribuição dessa amostra à \emph{loss} de entropia cruzada binária do discriminador?
\begin{enumerate}[(a)]
    \item $0.200$.
    \item $\approx 0.223$.
    \item $\approx 1.609$.
...
% ------------------------------------------------------------------
\item Seja $d = f[g[z,\theta],\phi]$ o logit do discriminador na amostra gerada. No ponto $d = -3$, quais são, respectivamente, as magnitudes dos gradientes da \emph{loss} original do gerador $L_{\text{orig}} = \ln[1-\sigma(d)]$ e da \emph{loss} \textit{non-saturating} $L_{\text{NS}} = -\ln[\sigma(d)]$ em relação a $d$? Use $\sigma(-3) \approx 0.047$.
\begin{enumerate}[(a)]
    \item $0.953$ e $0.047$.
    \item $0.047$ e $0.953$.
    \item $0.500$ e $0.500$.
...
% ------------------------------------------------------------------
\item Em \textit{WGAN-GP}, o termo de penalidade adicionado à \emph{loss} é $\lambda\,(\|\nabla_{\hat{x}} f[\hat{x},\phi]\|_{2} - 1)^{2}$ avaliado em um ponto interpolado $\hat{x}$. Para $\lambda = 10$ e $\|\nabla_{\hat{x}} f[\hat{x},\phi]\|_{2} = 3$, qual é a contribuição desse ponto à \emph{loss}?
\begin{enumerate}[(a)]
    \item $4$.
    \item $90$.
    \item $40$.
...
\begin{enumerate}[I)]
    ...
    \item Como o rótulo é $y = 1$ (real), a contribuição é $-\ln[\sigma(f[x,\phi])] = -\ln(0.20) \approx 1.609$ (alternativa c). A (a) reporta $\sigma$ diretamente, sem aplicar o logaritmo. A (b) usa $-\ln(1 - \sigma) = -\ln(0.80) \approx 0.223$, que seria correto se a amostra fosse gerada ($y = 0$). A (d) reporta $1 - \sigma$, também sem logaritmo.
...
\[
  L[\phi] = \mathbb{E}_{x^{*}}\!\left[\ln[1 - \sigma(f[x^{*},\phi])]\right]
          + \mathbb{E}_{x}\!\left[\ln[\sigma(f[x,\phi])]\right],
\]
mostre, em passos algébricos, que com o discriminador ótimo $D^{*}(x) = \tfrac{P(x)}{P(x) + P^{*}(x)}$ a \emph{loss} se reduz a
\[
  L[\phi] = 2\,D_{JS}(P^{*}\|P) - 2\ln 2.
\]
Indique explicitamente: (a) onde a hipótese de iguais prioris ($P(\text{real}) = P(\text{gen}) = \tfrac{1}{2}$) entra; (b) onde aparece o ``truque do fator 2''; (c) por que a constante $-2\ln 2$ não afeta o $\argmax$ em relação a $\phi$ nem o $\argmin$ em relação a $\theta$ (lembre que, escrita sem o sinal de menos, essa é a forma que o discriminador maximiza e o gerador minimiza).

% ------------------------------------------------------------------
% D3 — Hard (non-saturating loss gradient analysis)
% ------------------------------------------------------------------
\item \emoji{skull-and-crossbones} Seja $d = f[g[z,\theta],\phi]$ o logit do discriminador na amostra gerada. Defina
\[
  L_{\text{orig}}[\theta] = \ln[1 - \sigma(d)] \qquad\text{e}\qquad L_{\text{NS}}[\theta] = -\ln[\sigma(d)],
\]
...
\begin{enumerate}[(a)]
    ...
    \item Suponha que, em alguma região $R$, $P^{*}(x) \ll P(x)$ (o gerador ignora uma moda). Mostre que, nessa região, o integrando do termo de \emph{cobertura} é aproximadamente $P(x)\ln 2$, ou seja, fica \textbf{limitado e pouco sensível} a pequenos aumentos de $P^{*}(x)$.
...
\begin{enumerate}[I)]
    ...
    \[
      L[\phi] = \mathbb{E}_{x^{*}}\!\left[\ln[1 - \sigma(f[x^{*},\phi])]\right]
              + \mathbb{E}_{x}\!\left[\ln[\sigma(f[x,\phi])]\right]
              = \int P^{*}(x)\ln[1 - \sigma(f)]\,dx + \int P(x)\ln[\sigma(f)]\,dx.
    \]
    O discriminador ótimo é obtido pela regra de Bayes assumindo $P(\text{real}) = P(\text{gen}) = \tfrac{1}{2}$ \textbf{(item (a))}, o que cancela os \emph{priors} e produz $\sigma(f[x,\phi]) = \tfrac{P(x)}{P(x) + P^{*}(x)}$, com complemento $1 - \sigma = \tfrac{P^{*}(x)}{P(x) + P^{*}(x)}$. Substituindo,
    \[
      L = \int P^{*}(x)\ln\!\left[\tfrac{P^{*}(x)}{P(x) + P^{*}(x)}\right]dx + \int P(x)\ln\!\left[\tfrac{P(x)}{P(x) + P^{*}(x)}\right]dx.
    \]
    Introduzimos a mistura $M(x) = \tfrac{1}{2}(P(x) + P^{*}(x))$, de modo que $P(x) + P^{*}(x) = 2M(x)$. Multiplicando e dividindo por $2$ dentro de cada logaritmo \textbf{(item (b))}, cada razão se reescreve como $\tfrac{2P^{*}(x)}{P(x) + P^{*}(x)} = \tfrac{P^{*}(x)}{M(x)}$ (e analogamente para $P(x)$):
    \[
      L = \int P^{*}(x)\!\left(\ln\!\left[\tfrac{P^{*}(x)}{M(x)}\right] - \ln 2\right)dx
        + \int P(x)\!\left(\ln\!\left[\tfrac{P(x)}{M(x)}\right] - \ln 2\right)dx.
    \]
    ...
    \]
    \textbf{Item (c)}: $-2\ln 2$ é constante em $\phi$ e em $\theta$. Como essa forma da \emph{loss} (sem o sinal de menos) é a log-verossimilhança que o discriminador \emph{maximiza} e que o gerador \emph{minimiza} (minimizar $L$ equivale a minimizar $D_{JS}$), a constante não altera $\argmax_{\phi}$ nem $\argmin_{\theta}$.
    ...
    \textbf{(b)} Em região $R$ onde $P^{*}(x) \ll P(x)$, temos $M(x) = \tfrac{P(x) + P^{*}(x)}{2} \approx \tfrac{P(x)}{2}$. Logo $\tfrac{P(x)}{M(x)} \approx 2$, e o integrando da cobertura fica
    \[
      P(x)\ln\!\tfrac{P(x)}{M(x)} \approx P(x)\ln 2,
    \]
    \textbf{limitado} (cresce apenas com $P(x)$, não com a discrepância em si) e \textbf{pouco sensível} a pequenas perturbações em $P^{*}(x)$ (a derivada em $P^{*}(x)$ envolve $\tfrac{1}{P(x) + P^{*}(x)}$, que é pequena quando $P(x)$ domina).

    \textbf{(c)} O termo de qualidade ($D_{KL}(P^{*}\|M)$) explode se o gerador colocar massa em pontos onde $P$ é baixa: o gradiente pune \textbf{fortemente} amostras implausíveis. O termo de cobertura, pelo argumento de (b), é \textbf{quase insensível} a aumentos pequenos de $P^{*}(x)$ em modas ignoradas. Diante dessa assimetria, o caminho de menor custo durante o treinamento é \emph{melhorar a qualidade nas modas já cobertas} (penalidade alta e gradiente forte) em vez de \emph{cobrir novas modas} (penalidade baixa e gradiente fraco), produzindo \textit{mode collapse}.
\end{enumerate}
