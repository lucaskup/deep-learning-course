...
$\mathbf{\Theta}_\bullet \in \mathbb{R}^{hs \times (hs + is)}$ e \textit{bias}
...
\begin{enumerate}[I)]
    ...
    \item \textbf{c.} Para manter $\mathbf{c}_{t-1}$ e bloquear a contribuição nova é preciso
    ...
    \item \textbf{d.} $h_t = 0.6 \cdot \tanh(0.5) = 0.6 \cdot 0.4621 \approx 0.2773$.
    A alternativa (a) $0.3000$ esquece o $\tanh$; (b) $0.2913$ aplica $\tanh$ \emph{depois}
    do produto, $\tanh(0.3)$; (c) $0.4621$ esquece de multiplicar por $o_t$.
    \item \textbf{b.} Com $\mathbf{z}_t \to 0$, $\mathbf{h}_t = 1 \cdot \mathbf{h}_{t-1} + 0 \cdot \tilde{\mathbf{h}}_t = \mathbf{h}_{t-1}$:
    o estado anterior é copiado. A alternativa (a) corresponde a $\mathbf{z}_t \to 1$; (c) e (d) não
...
\begin{enumerate}[(a)]
    ...
    $\prod_k \theta_{hh}\,(1 - \tanh^2(z_k))$, escrito no caso escalar.
...
\begin{enumerate}[(a)]
    ...
    \textit{vanilla}, cada passo multiplica por $\theta_{hh}\,(1 - \tanh^2(z_k))$ no caso escalar; como
    $1 - \tanh^2 \le 1$, o produto tende a $0$ (\textit{vanishing}) ou, com $|\theta_{hh}|$
    grande, explode. No caso vetorial o que importa é o raio espectral de $\mathbf{\Theta}_{hh}$: abaixo
    de $1$ o produto se dissipa, acima de $1$ ele explode.
