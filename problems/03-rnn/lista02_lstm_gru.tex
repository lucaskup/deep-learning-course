...
$\theta_\bullet \in \mathbb{R}^{(hs + is) \times hs}$ e \textit{bias}
$b_\bullet \in \mathbb{R}^{hs}$. Quantos parâmetros treináveis (pesos e \textit{bias})
...
% ------------------------------------------------------------------
\item Considere a atualização da memória $c_t = f_t \odot c_{t-1} + i_t \odot \tilde{c}_t$.
...
\begin{enumerate}[a)]
    \item $f_t \to 1$ e $i_t \to 0$.
    \item $f_t \to 0$ e $i_t \to 1$.
    \item $f_t \to 1$ e $i_t \to 1$.
    \item $f_t \to 0$ e $i_t \to 0$.
\end{enumerate}
% R: a
...
$h_t = (1 - z_t) \odot h_{t-1} + z_t \odot \tilde{h}_t$. Quando o \textit{update gate}
satisfaz $z_t \to 0$, o que ocorre com o estado oculto?

\begin{enumerate}[a)]
    \item $h_t \approx h_{t-1}$.
    \item $h_t \approx \tilde{h}_t$.
    \item $h_t \approx 0$.
    \item $h_t \approx r_t \odot h_{t-1}$.
\end{enumerate}
% R: a
...
\begin{enumerate}[I)]
    ...
    \item \textbf{a.} Para manter $c_{t-1}$ e bloquear a contribuição nova é preciso
    $f_t \to 1$ (preserva a memória) e $i_t \to 0$ (zera $i_t \odot \tilde{c}_t$). As demais
    ou apagam a memória ($f_t \to 0$) ou deixam a entrada atualizar o estado ($i_t \to 1$).
    ...
    \item \textbf{a.} Com $z_t \to 0$, $h_t = 1 \cdot h_{t-1} + 0 \cdot \tilde{h}_t = h_{t-1}$:
    o estado anterior é copiado. A alternativa (b) corresponde a $z_t \to 1$; (c) e (d) não
...
\begin{enumerate}[(a)]
    \item A partir da atualização $c_t = f_t \odot c_{t-1} + i_t \odot \tilde{c}_t$, mostre que,
    pelo \emph{caminho direto} no tempo, $\dfrac{\partial c_t}{\partial c_{t-1}} = f_t$.
    ...
    $\prod_k \theta_{hh}\,(1 - \tanh^2(z_k))$.
    \item \emoji{skull-and-crossbones} A GRU não mantém um estado de memória $c_t$ separado;
    o estado é atualizado por $h_t = (1 - z_t)\, h_{t-1} + z_t\, \tilde{h}_t$. Identifique qual
    termo desempenha, na GRU, o papel análogo ao do \textit{forget gate} da LSTM na preservação
    do gradiente e diga sob qual condição do \textit{update gate} $z_t$ o gradiente é carregado
    quase sem atenuação.
\end{enumerate}
...
\begin{enumerate}[(a)]
    \item Derivando $c_t = f_t \odot c_{t-1} + i_t \odot \tilde{c}_t$ em relação a $c_{t-1}$,
    o termo direto $f_t \odot c_{t-1}$ contribui com $f_t$; os fatores $f_t$, $i_t$ e
    $\tilde{c}_t$ dependem de $c_{t-1}$ apenas indiretamente, através de $h_{t-1}$, formando
    caminhos secundários menores. Logo, pelo caminho direto,
    $\partial c_t / \partial c_{t-1} = f_t$. Ao longo de $T$ passos esse caminho acumula
    $\prod_k f_k$: são multiplicações por portas aprendidas e \textbf{somas}, sem $\tanh'$
    repetido. Quando o \textit{forget gate} aprende $f_k \approx 1$, o produto permanece
    $\approx 1$ e o gradiente é preservado (\textit{constant error carousel}). Na RNN
    \textit{vanilla}, cada passo multiplica por $\theta_{hh}\,(1 - \tanh^2(z_k))$; como
    $1 - \tanh^2 \le 1$, o produto tende a $0$ (\textit{vanishing}) ou, com $|\theta_{hh}|$
    grande, explode.
    \item Na GRU, o fator $(1 - z_t)$ multiplica $h_{t-1}$ no caminho direto, então
    $\partial h_t / \partial h_{t-1} = (1 - z_t)$ por esse caminho, papel análogo ao $f_t$ da
    LSTM. Quando $z_t \approx 0$ (\textit{update gate} ``fechado''), tem-se $(1 - z_t) \approx 1$
