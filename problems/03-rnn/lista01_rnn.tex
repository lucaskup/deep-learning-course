...
\subsection*{Redes Recorrentes}
    ...
    \item \textit{(Adaptado de CS224N Stanford 2017)} Considere que todos os parâmetros da rede recorrente ilustrada na figura abaixo são escalares ($\theta_{hh}, \theta_{ih}, b_{h}, \theta_{ho}$ e  $b_{o}$). A função de ativação $f[.]$ é a função degrau definida abaixo:
...
\]
    ...
    \item 
        ...
        \begin{enumerate}[a)]
            ...
            \item Este processo de geração de palavras é autorregressivo. Descreva o que serve como input ($\mathbf{x}_t$) e o que serve como estado oculto anterior ($\mathbf{h}_{t-1}$) para a RNN nos seguintes passos de tempo:
                \begin{enumerate}[i.)]
                    \item Para gerar a primeira palavra ($y_1$) no tempo $t=1$.
                    \item Para gerar a segunda palavra ($y_2$) no tempo $t=2$.
                \end{enumerate}
            
            \item A CNN (Encoder) processa a imagem uma única vez para gerar um vetor de contexto $\mathbf{v}$. Qual é a origem e o formato (shape) mais comum deste vetor $\mathbf{v}$? Ele é a saída da última camada convolucional (um tensor 3D), ou é a saída que foi "achatada" (flattened), ou é a saída já processada de uma camada totalmente conectada (FC)?

            \item Digamos que não queremos perder a informação espacial de cada frame ao passá-la para a RNN. Ou seja, queremos que o \textit{input} $\mathbf{x}_t$ e o \textit{estado oculto} $\mathbf{h}_t$ sejam eles mesmos mapas de características 3D (ex: [Altura, Largura, Canais]), e não vetores 1D. É possível utilizar operações de convolução \emph{dentro} da célula da RNN (para calcular $\mathbf{h}_t$ a partir de $\mathbf{h}_{t-1}$ e $\mathbf{x}_t$) em vez das multiplicações de matrizes (camadas densas) tradicionais? Explique brevemente o conceito.
        \end{enumerate}
...
\begin{enumerate}[I)]
    ...
    \item 
        \begin{enumerate}[a)]
            \item Sua função é processar a imagem, analisando e extraindo as características visuais importantes, comprimindo toda a informação em um único vetor ($\mathbf{v}$). Uma CNN como a VGG-16 não consegue, sozinha, gerar uma saída sequencial. A tarefa de decoder sequencial (gerar uma frase) requer uma arquitetura diferente, como uma RNN ou um Transformer.
            ...
            \item O processo é chamado de autorregressivo porque a saída de um passo de tempo é usada como entrada para o próximo passo.
                        \begin{enumerate}[i.]
                            \item \textbf{Para gerar $y_1$ (Tempo $t=1$):}
                                \begin{itemize}
                                    \item \textbf{Estado Oculto Anterior ($\mathbf{h}_{t-1}$):} É o $\mathbf{h}_0$. Uma opção comum é inicializá-lo com o vetor da imagem $\mathbf{v}$ fornecido pela CNN, que é a convenção adotada aqui; alimentar $\mathbf{v}$ como primeira entrada $\mathbf{x}_0$ e partir de $\mathbf{h}_0 = \mathbf{0}$ é igualmente comum.

                                    \item \textbf{Input ($\mathbf{x}_1$):} Não temos uma palavra anterior. Portanto, usamos um \textit{token} especial de início de sentença (ex: \texttt{<START>}).

                                    \item \textbf{Cálculo:} $\mathbf{h}_1 = f(\mathbf{x}_1\,\mathbf{\Theta}_{ih}^{T} + \mathbf{v}\,\mathbf{\Theta}_{hh}^{T} + \mathbf{b}_h)$ e $\hat{\mathbf{y}}_1 = g(\mathbf{h}_1\,\mathbf{\Theta}_{ho}^{T} + \mathbf{b}_o)$, em que $\hat{\mathbf{y}}_1$ é uma distribuição sobre o vocabulário.
                                \end{itemize}
                        
                            \item \textbf{Para gerar $y_2$ (Tempo $t=2$):}
                                \begin{itemize}
                                    \item \textbf{Estado Oculto Anterior ($\mathbf{h}_{t-1}$):} É o $\mathbf{h}_1$, o estado oculto que acabou de ser calculado no passo anterior.

                                    \item \textbf{Input ($\mathbf{x}_2$):} É a palavra gerada no passo anterior, mas atenção ao que de fato entra na rede: $\hat{\mathbf{y}}_1$ é uma distribuição sobre o vocabulário, não uma palavra. Amostramos (ou tomamos o \textit{argmax} de) $\hat{\mathbf{y}}_1$ para obter um \textit{token} $w_1$ e usamos a \textit{embedding} dele, $\mathbf{x}_2 = \text{emb}(w_1)$. Em treino com \textit{teacher forcing} usa-se a \textit{embedding} da palavra correta da legenda em vez da palavra gerada, e é justamente essa diferença entre treino e geração que dá origem ao \textit{exposure bias}.

                                    \item \textbf{Cálculo:} $\mathbf{h}_2 = f(\mathbf{x}_2\,\mathbf{\Theta}_{ih}^{T} + \mathbf{h}_1\,\mathbf{\Theta}_{hh}^{T} + \mathbf{b}_h)$ e $\hat{\mathbf{y}}_2 = g(\mathbf{h}_2\,\mathbf{\Theta}_{ho}^{T} + \mathbf{b}_o)$.
                                \end{itemize}
                        \end{enumerate}

            \item A abordagem clássica, usada no \textit{Show and Tell} de Vinyals et al. (2015), é tomar a saída de uma camada totalmente conectada e projetá-la linearmente para a dimensão da RNN.
                \begin{enumerate}[i.]
                    \item Processo: a imagem passa pela CNN e, na VGG-16, a saída da penúltima camada densa é um vetor de $4096$ dimensões. Uma camada linear aprendida projeta esse vetor para a dimensão do estado oculto da RNN, produzindo $\mathbf{v}$.
                    \item Formato: $\mathbf{v}$ é um vetor 1-D com a dimensão do estado oculto da RNN (por exemplo $512$), e não com a dimensão da saída da CNN.
                    \item Uso: $\mathbf{v}$ representa a imagem inteira e serve para condicionar o \textit{decoder}. Duas formas são comuns: usá-lo como estado oculto inicial $\mathbf{h}_0$, ou apresentá-lo como primeira entrada $\mathbf{x}_0$ da sequência, que é o que o \textit{Show and Tell} faz.
                    \item As outras duas opções do enunciado também aparecem na literatura, mas exigem cuidado. O \textit{feature map} 3-D não pode ser consumido diretamente por uma RNN densa, e a versão achatada ($7 \times 7 \times 512 = 25088$ dimensões na VGG-16) é grande demais para virar $\mathbf{h}_0$ diretamente: seria preciso uma camada linear para casar a dimensão, e nesse caso a projeção é justamente o que a abordagem clássica já faz a partir do vetor menor da camada densa. Manter o mapa 3-D sem achatar é o caminho de arquiteturas com atenção visual, em que o \textit{decoder} consulta as $49$ posições espaciais a cada passo em vez de receber um único vetor.
                \end{enumerate}
            ...
            \item Sim, é possível, e esta arquitetura é conhecida como \textbf{Convolutional RNN} (ou ConvLSTM/ConvGRU).
                \begin{enumerate}[i.]
                    ...
                    $$
                    \mathbf{h}_t = \tanh(\mathbf{x}_t\,\mathbf{\Theta}_{ih}^{T} + \mathbf{h}_{t-1}\,\mathbf{\Theta}_{hh}^{T} + \mathbf{b}_h)
                    $$
                    Exigindo que $\mathbf{h}_t$, $\mathbf{h}_{t-1}$ e $\mathbf{x}_t$ sejam vetores 1D.

                    \item Em uma ConvRNN: As multiplicações de matrizes por $\mathbf{\Theta}_{ih}$ e $\mathbf{\Theta}_{hh}$ são substituídas por operações de convolução. Neste caso, o \textit{input} $\mathbf{x}_t$ é o próprio mapa de características 3D da CNN (ex: $7 \times 7 \times 512$). É necessário que sejam preservadas as dimensões espaciais, permitindo que a RNN "lembre" não apenas o que viu nos frames anteriores, mas onde (espacialmente) viu.
                \end{enumerate}
