...
\begin{enumerate}[I)]
    ...
    \item 
        \begin{enumerate}[a)]
            \item Sua função é processar a imagem, analisando e extraindo as características visuais importantes, comprimindo toda a informação em um único vetor ($\mathbf{v}$). Usando uma VGG-16 de encoder (como a VGG) não é possível, sozinha, gerar uma saída sequencial. A tarefa de decoder sequencial (gerar uma frase) requer uma arquitetura diferente, como uma RNN ou um Transformer.
            ...
            \item O processo é chamado de autorregressivo porque a saída de um passo de tempo é usada como entrada para o próximo passo.
                        \begin{enumerate}[i.]
                            \item \textbf{Para gerar $y_1$ (Tempo $t=1$):}
                                \begin{itemize}
                                    \item \textbf{Estado Oculto Anterior ($\mathbf{h}_{t-1}$):} É o $\mathbf{h}_0$, que é o vetor da imagem $\mathbf{v}$ fornecido pela CNN.
                                    ...
                                    \item \textbf{Cálculo:} $\mathbf{h}_1 = f(\mathbf{x}_1\,\mathbf{\Theta}_{ih}^{T} + \mathbf{v}\,\mathbf{\Theta}_{hh}^{T} + \mathbf{b}_h)$ e $\mathbf{y}_1 = g(\mathbf{h}_1\,\mathbf{\Theta}_{ho}^{T} + \mathbf{b}_o)$.
                                \end{itemize}
                            ...
                            \item \textbf{Para gerar $y_2$ (Tempo $t=2$):}
                                \begin{itemize}
                                    ...
                                    \item \textbf{Input ($\mathbf{x}_2$):} É a palavra que foi gerada no passo anterior, ou seja, $y_1$.

                                    \item \textbf{Cálculo} $\mathbf{h}_2 = f(\mathbf{y}_1\,\mathbf{\Theta}_{ih}^{T} + \mathbf{h}_1\,\mathbf{\Theta}_{hh}^{T} + \mathbf{b}_h)$ e $\mathbf{y}_2 = g(\mathbf{h}_2\,\mathbf{\Theta}_{ho}^{T} + \mathbf{b}_o)$.
                                \end{itemize}
                        \end{enumerate}

            \item A abordagem mais comum e direta \textbf{é} usar a saída `achatada' (flattened) da última camada convolucional.
                \begin{enumerate}[i.]
                    \item Processo: A imagem passa pelas camadas convolucionais (ex: VGG-16), resultando em um \textit{feature map} 3D (ex: $7 \times 7 \times 512$). Achatamos (flattening) para um vetor 1D (ex: $25088 \times 1$). A saída é usada como o vetor $\mathbf{v}$.
                    \item Formato (Shape): O vetor $\mathbf{v}$ é um vetor 1D (ex: `($25088$,)').
                    \item Uso: Este vetor 1D representa a imagem inteira, é usado para inicializar o primeiro estado oculto ($\mathbf{h}_0$) da RNN, `preparando' o decoder com o que ele deve descrever.
                \end{enumerate}
