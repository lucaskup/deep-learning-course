...
% ------------------------------------------------------------------
\item A camada de saída de um LM calcula $\text{logits} = \mathbf{W} \mathbf{h} + \mathbf{b}$ a partir de um vetor de
contexto $\mathbf{h} \in \mathbb{R}^{d}$, com $\mathbf{W} \in \mathbb{R}^{|V| \times d}$. Para $d = 8$ e
...
Para $p = 0.85$, quantos \textit{tokens} compõem o núcleo do qual se amostra?
...
\textit{encoder} são $\text{score}(\mathbf{h}_t, \mathbf{s}_k) = [\,3,\; 1,\; 0\,]$. Usando
$a_k^{(t)} = \dfrac{\exp(\text{score}(\mathbf{h}_t, \mathbf{s}_k))}{\sum_i \exp(\text{score}(\mathbf{h}_t, \mathbf{s}_i))}$, qual é o
...
% ------------------------------------------------------------------
\item Os pesos de atenção de um passo são $\mathbf{a}^{(t)} = [\,0.6,\; 0.3,\; 0.1\,]$ e os estados do
\textit{encoder} são $\mathbf{s}_1 = (2;\, 0)$, $\mathbf{s}_2 = (0;\, 4)$ e $\mathbf{s}_3 = ({-}2;\, {-}2)$. Qual é o
\textit{context vector} $\mathbf{c}^{(t)} = \sum_k a_k^{(t)}\, \mathbf{s}_k$?
...
$n = 4$ \textit{tokens} e $\sum_{t} \log P(y_t \mid y_{<t}) = -1.2$. Com $\alpha = 0.5$, qual é o
...
\color{blue}
\begin{enumerate}[I)]
    \item \textbf{c.} Pela regra da cadeia, $P = 0.5 \cdot 0.4 \cdot 0.3 = 0.06$. A
    alternativa (a) $0.12$ esquece o primeiro fator $P(\text{The})$; (b) $1.20$ soma as
    probabilidades em vez de multiplicar; (d) $0.20$ usa apenas os dois primeiros fatores.
    \item \textbf{a.} A matriz $\mathbf{W}$ tem $|V| \cdot d = 50 \cdot 8 = 400$ pesos e o \textit{bias}
    $\mathbf{b} \in \mathbb{R}^{|V|}$ acrescenta $50$, totalizando $450$. A alternativa (b) $400$ ignora o
    \textit{bias}; (c) $408$ usa um \textit{bias} de dimensão $d$ (entrada) em vez de $|V|$
    (saída); (d) $58$ soma $|V| + d$ em vez de multiplicar.
    \item \textbf{d.} $\mathcal{L} = -\ln(0.25) \approx 1.386$. A alternativa (a) $2$ usa
    $-\log_2(0.25)$ em vez de logaritmo natural; (b) $-1.386$ esquece o sinal negativo da
    \textit{cross-entropy}; (c) $0.25$ devolve a própria probabilidade sem aplicar $-\ln$.
    \item \textbf{b.} Com $b = 2$ sobrevivem \texttt{nice} e \texttt{dog}. As melhores expansões
    dão $P(\texttt{The dog has}) = 0.3 \cdot 0.9 = 0.27$ e
    $P(\texttt{The nice woman}) = 0.6 \cdot 0.4 = 0.24$, logo \texttt{The dog has} vence. A
    alternativa (a) escolhe pelo \textit{token} localmente mais provável (\texttt{nice}), erro do
    ...
    \item \textbf{c.} As probabilidades acumuladas são $0.50,\; 0.75,\; 0.90,\; 0.97,\;
    1.00$. O menor conjunto cuja massa acumulada atinge $0.85$ é $\{w_1, w_2, w_3\}$, isto é,
    $3$ \textit{tokens}. A alternativa (a) $2$ para em $0.75$, antes de atingir $p$; (b) $4$ e
    (d) $5$ incluem \textit{tokens} além do necessário, porque a massa já passa de $0.85$ no
    terceiro.
    \item \textbf{b.} $\log_2 p = [\,-1,\; -1,\; -3\,]$, então $L = -5$ e
    $-\frac{1}{m} L = \frac{5}{3} \approx 1.667$; logo
    $\text{Perplexidade} = 2^{1.667} \approx 3.17$. A alternativa (a) $5.00$ devolve
    ...
    \item \textbf{d.} $\exp(\text{scores}) = [\,20.09;\; 2.72;\; 1.00\,]$, com soma
    $\approx 23.80$; então $a_1^{(t)} = 20.09 / 23.80 \approx 0.844$. A alternativa (a)
    $0.750$ normaliza os \textit{scores} linearmente ($3/(3+1+0)$) em vez de aplicar
    \textit{softmax}; (b) $0.114$ é o peso $a_2^{(t)}$ do segundo estado; (c) $0.333$ supõe
    pesos uniformes.
    \item \textbf{c.}
    $\mathbf{c}^{(t)} = 0.6 (2;\, 0) + 0.3 (0;\, 4) + 0.1 ({-}2;\, {-}2)
    = (1.2 - 0.2;\; 1.2 - 0.2) = (1.0;\; 1.0)$. A alternativa (a) $(1.2;\, 1.2)$
    ignora a contribuição de $\mathbf{s}_3$; (b) $(0.0;\, 0.67)$ faz a média simples dos vetores sem
    os pesos; (d) $(0.0;\, 2.0)$ soma os vetores sem ponderar.
    \item \textbf{b.} $\frac{1}{n^{\alpha}} \sum_t \log P = \frac{1}{4^{0.5}} (-1.2) =
    \frac{-1.2}{2} = -0.60$. A alternativa (a) $-0.30$ usa $\alpha = 1$, dividindo por $n = 4$
    em vez de $\sqrt{n}$; (c) $-1.20$ não aplica normalização alguma; (d) $-2.40$ multiplica por
    $n^{\alpha}$ em vez de dividir. O efeito da normalização aparece na comparação com uma
    sequência curta $A$ ($n = 2$, $\sum \log P = -1.0$): sem normalizar, $A$ vence
    ($-1.0 > -1.2$); com $\alpha = 0.5$, o \textit{score} de $A$ é
    $-1.0/\sqrt{2} \approx -0.71$ e o de $B$ é $-0.60$, então $B$ passa à frente.
\end{enumerate}
\color{black}
...
% ------------------------------------------------------------------
\item Em um mecanismo de atenção com \textit{score} do tipo \textit{dot-product}, o estado do \textit{decoder} é
$\mathbf{h}_t = (1;\, 2)$ e há três estados do \textit{encoder}: $\mathbf{s}_1 = (2;\, 0)$, $\mathbf{s}_2 = (0;\, 2)$ e
$\mathbf{s}_3 = ({-}1;\, 1)$. Use $\text{score}(\mathbf{h}_t, \mathbf{s}_k) = \mathbf{h}_t^{\top} \mathbf{s}_k$ e
$a_k^{(t)} = \dfrac{\exp(\text{score}(\mathbf{h}_t, \mathbf{s}_k))}{\sum_i \exp(\text{score}(\mathbf{h}_t, \mathbf{s}_i))}$.
Calcule, passo a passo (use 4 casas decimais quando necessário):
\begin{enumerate}[(a)]
    \item os três \textit{scores} $\text{score}(\mathbf{h}_t, \mathbf{s}_k)$;
    \item os pesos de atenção $a_k^{(t)}$;
    \item o \textit{context vector} $\mathbf{c}^{(t)} = \sum_k a_k^{(t)}\, \mathbf{s}_k$.
\end{enumerate}

% ------------------------------------------------------------------
% D2 — Medium/Hard (perplexidade: cálculo, interpretação, ausência de teto)
% ------------------------------------------------------------------
...
\begin{enumerate}[(a)]
    ...
    exatamente igual a $|V|$, qualquer que seja a sequência avaliada.
    \item \emoji{skull-and-crossbones} O item anterior costuma ser lido como se $|V|$ fosse um
    limite superior da perplexidade. Construa um contraexemplo: escolha $|V|$, uma distribuição do
    modelo e uma sequência avaliada tais que a perplexidade seja maior que $|V|$. Explique, em uma
    frase, por que o argumento ``a uniforme é a distribuição de maior incerteza'' não impede isso.
\end{enumerate}
...
\begin{enumerate}[(a)]
    \item \textit{Scores} (produto interno $\mathbf{h}_t^{\top} \mathbf{s}_k$ com $\mathbf{h}_t = (1;\, 2)$):
    \begin{align*}
        \text{score}(\mathbf{h}_t, \mathbf{s}_1) &= 1 \cdot 2 + 2 \cdot 0 = 2, \\
        \text{score}(\mathbf{h}_t, \mathbf{s}_2) &= 1 \cdot 0 + 2 \cdot 2 = 4, \\
        \text{score}(\mathbf{h}_t, \mathbf{s}_3) &= 1 \cdot ({-}1) + 2 \cdot 1 = 1.
    \end{align*}
    \item Exponenciais: $\exp(2) = 7.3891$, $\exp(4) = 54.5982$, $\exp(1) = 2.7183$, com
    soma $64.7056$. Logo
    \[
        \mathbf{a}^{(t)} = \left[\frac{7.3891}{64.7056};\; \frac{54.5982}{64.7056};\;
        \frac{2.7183}{64.7056}\right] \approx [\,0.1142;\; 0.8438;\; 0.0420\,].
    \]
    \item \textit{Context vector}:
    \begin{align*}
        \mathbf{c}^{(t)} &= 0.1142 (2;\, 0) + 0.8438 (0;\, 2) + 0.0420 ({-}1;\, 1) \\
                &= (0.2284 - 0.0420;\; 1.6876 + 0.0420) \approx (0.186;\; 1.730).
    \end{align*}
    O peso domina em $\mathbf{s}_2$, o estado de maior \textit{score}, e o contexto é puxado nessa direção.
\end{enumerate}
...
\begin{enumerate}[(a)]
    ...
    \]
    logo $\text{Perplexidade} = 2^{\log_2 |V|} = |V|$. Note que o resultado não depende de quais
    \textit{tokens} foram observados: o modelo uniforme tem perplexidade $|V|$ em qualquer
    sequência.
    \item Com $V = \{\texttt{A}, \texttt{B}\}$, portanto $|V| = 2$, considere o modelo que prevê
    $p(\texttt{A}) = 0.99$ e $p(\texttt{B}) = 0.01$ em todo passo, avaliado na sequência
    \texttt{B B B B}. Então
    \[
        -\frac{1}{m} L = -\log_2(0.01) \approx 6.644
        \qquad \Rightarrow \qquad
        \text{Perplexidade} = 2^{6.644} \approx 100 \gg |V| = 2.
    \]
    O argumento da máxima incerteza limita a \textbf{entropia do modelo},
    $H(p) \le \log_2 |V|$, mas a perplexidade é $2$ elevado à \textbf{entropia cruzada} entre os
    dados e o modelo, $H(\hat{q}, p) = H(\hat{q}) + D_{KL}(\hat{q} \,\|\, p)$. O termo
    $D_{KL}(\hat{q} \,\|\, p)$ não tem limite superior, então um modelo confiante e errado é pior
    que o chute uniforme. No extremo, se $p(y_t \mid y_{<t}) = 0$ para algum \textit{token}
    observado, a perplexidade é $+\infty$, que é justamente o problema motivador do
    \textit{smoothing} em modelos de \textit{n-gramas}.
\end{enumerate}
