...
\begin{enumerate}[I)]
    ...
    \item A figura abaixo ilustra o processo de atenção em uma rede recorrente. Nessa rede recorrente, temos que tanto os estados ocultos do \textit{enconder} $\mathbf{h}_i$ quanto o estado oculto do \textit{decoder} $\mathbf{s}$ possuem tamanho $2$. Qual é o valor do vetor de contexto ao computar a atenção por produto escalar do estado do \textit{decoder} $\mathbf{s}_1$ para todos os estados do \textit{encoder} $\mathbf{h}_i$?
    ...
    \begin{figure}[H]
        ...
        \end{tikzpicture}
        \caption{Diagrama contemplando os estados ocultos $\mathbf{h}_i$ de um \textit{encoder} baseado em RNNs e o estado oculto de um \textit{decoder} $\mathbf{s}_1$. A representação dos tokens e dos \textit{embeddings} dos \textit{tokens} foram removidas da figura para simplificar o diagrama.}
...
% ------------------------------------------------------------------
\item Considere dois vetores $\mathbf{s}, \mathbf{h} \in \mathbb{R}^d$ com componentes
      $s_i, h_i \sim \mathcal{N}(0, 1)$ i.i.d. Para $d = 64$, qual o valor da
      variância de $\mathbf{s} \cdot \mathbf{h}$ e por qual fator devemos dividir o produto
      escalar para que a variância dos scores resultantes seja igual a~1?

\begin{enumerate}[a)]
    \item $\mathrm{var}[\mathbf{s} \cdot \mathbf{h}] = 1$, dividir por $1$.
    \item $\mathrm{var}[\mathbf{s} \cdot \mathbf{h}] = 8$, dividir por $\sqrt{8}$.
    \item $\mathrm{var}[\mathbf{s} \cdot \mathbf{h}] = 64$, dividir por $\sqrt{64} = 8$.
    \item $\mathrm{var}[\mathbf{s} \cdot \mathbf{h}] = 64^2$, dividir por $64$.
\end{enumerate}
% R: c
...
\item As três variantes de \textit{score} apresentadas para atenção são
      \textbf{dot-product} ($\mathbf{h}^\top \mathbf{s}$), \textbf{bilinear} ($\mathbf{h}^\top \mathbf{W} \mathbf{s}$)
      e \textbf{MLP aditiva} ($\mathbf{w}_2^\top \tanh(\mathbf{W}_1 [\mathbf{h}; \mathbf{s}])$). Qual afirmação
...
\begin{enumerate}[a)]
    \item A dot-product tem um parâmetro escalar aprendido, a bilinear adiciona
          uma matriz $\mathbf{W}$, e a MLP aditiva é a mais barata das três.
    \item A dot-product não tem parâmetros aprendidos, a bilinear adiciona uma
          matriz $\mathbf{W}$, e a MLP aditiva é a mais cara das três.
    \item A dot-product e a bilinear não têm parâmetros aprendidos, e a MLP
          aditiva é a mais barata das três.
    \item As três variantes têm o mesmo número de parâmetros e o mesmo custo,
          diferindo apenas na forma algébrica do score.
\end{enumerate}
% R: b
...
% ------------------------------------------------------------------
\item Considere o token $\mathbf{x} = [1,\;2,\;3]$ e a matriz de projeção
      \[
        \mathbf{W}_q = \begin{bmatrix} 2 & 1 \\ 0 & 1 \\ 1 & 0 \end{bmatrix}.
      \]
      Com bias zero, qual o valor de $\mathbf{q} = \mathbf{x} \, \mathbf{W}_q$?
...
\item Considere os pesos da atenção (já normalizados pela softmax) e a matriz
      ...
      \[
        \mathbf{a} = [\,0.7,\; 0.2,\; 0.1\,], \qquad
        \mathbf{V} = \begin{bmatrix} 10 & 0 \\ 0 & 10 \\ 5 & 5 \end{bmatrix}.
      \]
      Qual é o vetor de contexto $\mathbf{c} = \sum_{i=1}^{3} a_i\, \mathbf{v}_i$?
...
\item Considere a versão ``crua'' de \textit{self-attention}, em que aplicamos
      diretamente
      \[
        \mathrm{score}(\mathbf{x}_t, \mathbf{x}_k) = \frac{\mathbf{x}_t^\top \mathbf{x}_k}{\sqrt{d}}, \qquad
...
\begin{enumerate}[a)]
    \item O cálculo depende apenas dos valores dos \textit{embeddings} e não
          tem parâmetros treináveis no bloco de atenção.
    \item O score $\mathbf{x}_t^\top \mathbf{x}_k$ é sempre não negativo, fazendo com que a
          softmax sature em pesos próximos a $1/m$.
    \item A normalização por $\sqrt{d}$ deixa de funcionar quando $\mathbf{x}_t$ e
          $\mathbf{x}_k$ não têm a mesma média componente a componente.
...
\begin{enumerate}[a)]
    \item As saídas são diferentes, pois o índice $i$ de cada token influencia
          o cálculo das chaves $\mathbf{k}_i$.
    \item Para cada token, a saída produzida é a mesma em $S_1$ e em $S_2$,
          a menos da permutação das posições.
...
\item Em uma camada de \textit{multi-head attention} com $H = 8$ cabeças,
      ...
      $\mathbf{A}^{(h)} = \mathbf{Q}^{(h)} (\mathbf{K}^{(h)})^\top / \sqrt{d_k}$. Para uma sequência
...
\begin{enumerate}[a)]
    ...
    \item BatchNorm computa estatísticas por coluna (\textit{feature}) sobre
          o \textit{batch}, e o número de \textit{tokens} em cada posição
          varia com o comprimento das sequências.
\end{enumerate}
% R: d
...
\item No \textit{decoder}, aplicamos uma máscara causal somando uma matriz
      $\mathbf{M}$ aos scores antes da \textit{softmax}. Para a posição $t = 2$
      (que pode atender apenas às posições $1$ e $2$), considere os scores
      \[
        \mathrm{scores}_2 = [\,1.5,\; 0.5,\; 2.0,\; 1.0\,], \qquad
        \mathbf{m}_2 = [\,0,\; 0,\; -\infty,\; -\infty\,].
      \]
      Quais são os pesos da atenção $\mathbf{a}^{(2)}$ resultantes (após \textit{softmax})?
...
\item No bloco de \textit{cross-attention} do \textit{decoder} de uma
      ...
      tradução), de onde provêm $\mathbf{Q}$, $\mathbf{K}$ e $\mathbf{V}$?

\begin{enumerate}[a)]
    \item $\mathbf{Q}$, $\mathbf{K}$ e $\mathbf{V}$ vêm todos do \textit{encoder}; o \textit{decoder}
          fornece apenas o sinal de \textit{masking} causal.
    \item $\mathbf{Q}$, $\mathbf{K}$ e $\mathbf{V}$ vêm todos do \textit{decoder}; o \textit{encoder}
          fornece apenas a sequência inicial de \textit{embeddings}.
    \item $\mathbf{Q}$ vem do \textit{decoder} e $\mathbf{K}, \mathbf{V}$ vêm do \textit{encoder};
          a \textit{query} consulta os estados da sequência de origem.
    \item $\mathbf{Q}$ vem do \textit{encoder} e $\mathbf{K}, \mathbf{V}$ vêm do \textit{decoder};
...
\item \emoji{skull-and-crossbones} A FFN do \textit{transformer} original é
      ...
      \]
      com $\mathbf{W}_1 \in \mathbb{R}^{d \times d_{ff}}$,
      $\mathbf{W}_2 \in \mathbb{R}^{d_{ff} \times d}$,
      $\mathbf{b}_1 \in \mathbb{R}^{d_{ff}}$ e $\mathbf{b}_2 \in \mathbb{R}^{d}$.
...
\item \emoji{skull-and-crossbones} Em uma camada de \textit{self-attention}
      ...
      (i) a quantidade de parâmetros das matrizes $\mathbf{W}_Q, \mathbf{W}_K, \mathbf{W}_V, \mathbf{W}_O$,
      e (ii) o número de operações multiplicar-acumular (FLOPs) para
      computar a matriz de scores $\mathbf{Q} \mathbf{K}^\top$?
...
\[
  \mathrm{var}[\mathbf{s} \cdot \mathbf{h}]
    ...
    = \sum_{i=1}^{d} 1 = d.
\]
Para $d = 64$, $\mathrm{var}[\mathbf{s} \cdot \mathbf{h}] = 64$. Para reescalar o desvio
...
A variante \textbf{dot-product} ($\mathbf{h}^\top \mathbf{s}$) não introduz parâmetros
aprendidos: o score depende apenas dos vetores que entram no cálculo. A
\textbf{bilinear} ($\mathbf{h}^\top \mathbf{W} \mathbf{s}$) adiciona uma matriz $\mathbf{W}$ aprendida que
atua como um produto interno paramétrico. A \textbf{MLP aditiva}
($\mathbf{w}_2^\top \tanh(\mathbf{W}_1 [\mathbf{h}; \mathbf{s}])$) tem dois conjuntos de pesos $\mathbf{W}_1, \mathbf{w}_2$
e ainda computa um $\tanh$ por par $(\mathbf{h}, \mathbf{s})$, sendo a mais cara das três.
A opção~(a) atribui erroneamente um parâmetro à dot-product. A opção~(c)
esquece que a bilinear tem $\mathbf{W}$. A opção~(d) ignora as diferenças óbvias
de número de parâmetros.
% ---

% --- Solução Q11 ---
\subsubsection*{XI — Resposta: (a)}
A multiplicação $\mathbf{q} = \mathbf{x}\, \mathbf{W}_q$ produz um vetor de duas componentes:
\[
  q_0 = 1\cdot 2 + 2\cdot 0 + 3\cdot 1 = 5, \qquad
  q_1 = 1\cdot 1 + 2\cdot 1 + 3\cdot 0 = 3.
\]
Logo $\mathbf{q} = [5,\;3]$.
A opção~(b) corresponde a somar as colunas de $\mathbf{W}_q$ ignorando $\mathbf{x}$:
$[2+0+1,\,1+1+0] = [3,\,2]$. A opção~(c) inverte os componentes. A
opção~(d) usa a primeira coluna de $\mathbf{W}_q$ para ambas as posições da saída
($1\cdot 2 + 2\cdot 0 + 3\cdot 1 = 5$ aplicado duas vezes).
% ---

% --- Solução Q12 ---
\subsubsection*{XII — Resposta: (c)}
O contexto é a soma das linhas de $\mathbf{V}$ ponderada por $\mathbf{a}$:
\[
  \mathbf{c} = 0.7 \cdot [10,\,0] + 0.2 \cdot [0,\,10] + 0.1 \cdot [5,\,5]
    = [7,\,0] + [0,\,2] + [0.5,\,0.5]
    = [7.5,\;2.5].
\]
A opção~(a) soma as linhas de $\mathbf{V}$ sem usar os pesos $\mathbf{a}$. A opção~(b)
distribui peso uniforme $1/3$ entre as linhas. A opção~(d) aplica apenas
o peso $a_1 = 0.7$ à primeira linha de $\mathbf{V}$ e ignora os demais
\textit{values}.
% ---

% --- Solução Q13 ---
\subsubsection*{XIII — Resposta: (a)}
Sem as projeções $\mathbf{W}_q$, $\mathbf{W}_k$, $\mathbf{W}_v$, o cálculo de scores e do contexto
...
A solução é introduzir as três projeções lineares $\mathbf{q}_t = \mathbf{x}_t \mathbf{W}_q$,
$\mathbf{k}_t = \mathbf{x}_t \mathbf{W}_k$, $\mathbf{v}_t = \mathbf{x}_t \mathbf{W}_v$, criando os parâmetros que serão
otimizados.
A opção~(b) é falsa: $\mathbf{x}_t^\top \mathbf{x}_k$ pode ser negativo, dependendo dos
...
Sem \textit{positional encoding}, os vetores $\mathbf{q}_t$, $\mathbf{k}_t$ e $\mathbf{v}_t$
dependem apenas do \textit{token} (do conteúdo de $\mathbf{x}_t$), não da posição
...
$(\mathbf{q}_i, \mathbf{k}_i, \mathbf{v}_i)$ é o mesmo. Para o \textit{token} ``comeu'' (que ocupa
...
A opção~(a) atribui a $\mathbf{k}_i$ uma dependência do índice que não existe sem
...
A opção~(a) inverte o eixo de normalização do BatchNorm. A opção~(b)
inventa um requisito de reinicialização do otimizador que não existe.
A opção~(c) atribui ao BatchNorm uma janela deslizante que não faz parte
da sua definição.
% ---

% --- Solução Q17 ---
\subsubsection*{XVII — Resposta: (a)}
...
\[
  \mathbf{a}^{(2)} = \left[\frac{4.482}{6.131},\; \frac{1.649}{6.131},\; 0,\; 0\right]
         \approx [\,0.73,\; 0.27,\; 0,\; 0\,].
\]
A opção~(b) ignora a máscara, aplicando \textit{softmax} aos quatro
...
\textit{cross-attention}. A opção~(d) inverte os papéis de $\mathbf{Q}$ e $\mathbf{K}, \mathbf{V}$.
% ---

% --- Solução Q19 ---
\subsubsection*{XIX — Resposta: (d)}
Contagem direta dos parâmetros:
\[
  |\mathbf{W}_1| = d \cdot d_{ff} = 512 \cdot 2\,048 = 1\,048\,576,
\]
\[
  |\mathbf{W}_2| = d_{ff} \cdot d = 2\,048 \cdot 512 = 1\,048\,576,
\]
\[
  |\mathbf{b}_1| = d_{ff} = 2\,048, \qquad |\mathbf{b}_2| = d = 512.
\]
Total:
\[
...
A opção~(b) usa $\mathbf{W}_2 \in \mathbb{R}^{d\times d}$ por engano:
$1\,048\,576 + 262\,144 + 2\,048 + 512 = 1\,313\,280$.
A opção~(c) duplica indevidamente o número de matrizes:
$4\cdot 512\cdot 2\,048 = 4\,194\,304$.
% ---

% --- Solução Q20 ---
\subsubsection*{XX — Resposta: (b)}
\textbf{(i)~Parâmetros das projeções.} Cada uma das matrizes $\mathbf{W}_Q$, $\mathbf{W}_K$,
$\mathbf{W}_V$ e $\mathbf{W}_O$ tem formato $d \times d$, contribuindo $d^2$ parâmetros, em
...
\textbf{(ii)~FLOPs para $\mathbf{Q} \mathbf{K}^\top$.} Com $\mathbf{Q} \in \mathbb{R}^{N\times d}$ e
$\mathbf{K}^\top \in \mathbb{R}^{d\times N}$, o resultado é uma matriz $N\times N$ na
qual cada entrada $(i, j)$ é o produto interno entre uma linha de $\mathbf{Q}$ e
uma coluna de $\mathbf{K}^\top$, custando $d$ multiplicações-adições. Total:
$N^2 \cdot d = \Theta(N^2\, d)$.

A opção~(a) subestima o custo de $\mathbf{Q}\mathbf{K}^\top$ ao ignorar a dependência
