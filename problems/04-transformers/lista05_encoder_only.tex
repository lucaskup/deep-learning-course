...
\begin{enumerate}[a)]
    \item A perda MLM fica indefinida porque \texttt{[MASK]} não faz
          parte do vocabulário usado no \textit{fine-tuning}.
    \item O modelo nunca vê \textit{tokens} reais nas posições
          selecionadas durante o pré-treino, criando um desalinhamento
          com o \textit{fine-tuning}.
...
\begin{enumerate}[a)]
    \item \textit{POS tagging} em uma única sentença, com um rótulo por token.
    \item Classificação de sentimento em uma única sentença (SST-2), com um
          rótulo por sentença.
    \item \textit{Extractive QA} (SQuAD): identificar o trecho que responde
          a uma pergunta.
    \item \textit{Named Entity Recognition} (NER) em uma única sentença, com
          um rótulo por token.
\end{enumerate}
% R: c
...
\begin{enumerate}[a)]
    ...
    \item O ELECTRA gera sinal de gradiente em \textbf{todas} as posições
          da sequência, e não apenas nos $\approx 15\%$ mascarados do MLM.
...
\item Um engenheiro extrai $\mathbf{h}_{[\texttt{CLS}]}^{(L)}$ (estado
      ...
      esse método obtém $16.50$ de correlação de Spearman no
...
\begin{enumerate}[a)]
    \item O \texttt{[CLS]} foi treinado no NSP, um objetivo binário e
          ruidoso que não organiza o espaço para similaridade cosseno;
          o espaço resultante é anisotrópico.
...
\begin{enumerate}[a)]
    \item $\approx 100\times$
    \item $\approx 1\,000\times$
    \item $\approx 100\,000\times$
    \item Nenhuma redução de latência.
\end{enumerate}
% R: c
...
\begin{enumerate}[a)]
    \item O treino fica mais lento por \textit{token}, mas cada âncora
          distingue seu positivo de mais competidores, melhorando a
          qualidade dos \textit{embeddings}.
...
o pré-treino.
A opção~(a) é falsa: \texttt{[MASK]} é um token do vocabulário do BERT.
...
\]
A opção~(a) inclui $\mathbf{h}_3 = [1,\,3]$ do \texttt{[PAD]} no numerador e usa $L = 3$:
$([2,0]+[0,2]+[1,3])/3 = [3,5]/3 = [1,\,\tfrac{5}{3}]$: resultado distorcido pela posição de \textit{padding}.
A opção~(b) aplica corretamente a máscara ao numerador, mas omite a divisão por $\sum m_i$,
produzindo a soma bruta $[2,\,2]$.
A opção~(c) exclui o \texttt{[PAD]} do numerador (correto), mas divide por $L = 3$ em vez
...
A opção~(d) supõe que a busca vetorial custa tanto quanto um \textit{forward pass}
completo por um transformer, ignorando o fator de $\sim 10^4$--$10^6$ entre um produto
escalar e uma passagem pela rede.
...
A opção~(a) ocorreria se as duas distâncias fossem iguais ($d^+ = d^-$); nesse
caso $\mathcal{L} = \alpha = 0.50$.
A opção~(c), $-1.90$, é o argumento $d^+ - d^- + \alpha$ antes do $\max(0,\cdot)$;
como ele é negativo, o truncamento por zero é justamente o que leva à resposta correta.
A opção~(b), $+1.90$, é o mesmo valor com o sinal trocado, $d^- - d^+ - \alpha$, erro de
quem inverte a ordem das distâncias no argumento. Note que a diferença das distâncias
sozinha é $d^- - d^+ = 2.40$, e não $1.90$.
