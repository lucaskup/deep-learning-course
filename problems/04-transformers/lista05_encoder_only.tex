...
\item O BERT-Base tem $L = 12$ camadas, dimensão oculta $d = 768$ e
      ...
      \[
        W^Q,\; W^K,\; W^V \in \mathbb{R}^{d \times (H \cdot d_k)},
        \qquad
...
\begin{enumerate}[a)]
    \item $589\,824$: apenas $W^Q$.
    \item $1\,769\,472$: $W^Q, W^K, W^V$ sem a projeção de saída $W^O$.
    \item $2\,359\,296$: as quatro matrizes $W^Q, W^K, W^V, W^O$.
...
% ------------------------------------------------------------------
\item A representação de entrada do BERT é $\mathbf{x}_i = E_{\text{token}}(w_i)
      + E_{\text{segmento}}(s_i) + E_{\text{posição}}(i)$, onde
      $E_{\text{segmento}}$ assume o valor $E_A$ para tokens da primeira
      sentença e $E_B$ para tokens da segunda. Um pesquisador remove
      o componente $E_{\text{segmento}}$ para simplificar o modelo.
...
\item \emoji{skull-and-crossbones} Considere três \textit{embeddings}
      ...
      \[
        \mathcal{L}_{\text{triplet}} = \max\!\bigl(0,\;
          \|f(x)-f(x^+)\|_2^2 - \|f(x)-f(x^-)\|_2^2 + \alpha\bigr).
      \]
...
\]
A opção~(a) conta apenas $W^Q$.
A opção~(b) omite a projeção de saída $W^O$, que transforma a concatenação das
...
os tokens da pergunta recebendo $E_A$ e os do contexto recebendo $E_B$. Sem
...
compartilhariam $E_A$: a remoção de $E_{\text{segmento}}$ não altera
...
\[
  \mathcal{L}_{\text{NCE}} = -\frac{1}{N}\sum_{i=1}^{N} \log
    \frac{e^{\text{sim}(f(x_i),\,f(x_i^+))/\tau}}
         {\sum_{j=1}^{N} e^{\text{sim}(f(x_i),\,f(x_j^+))/\tau}}.
\]
