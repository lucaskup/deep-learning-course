...
\begin{tikzpicture}[
  ...
  \node[kv, fill=CellGreen]   (k1) at (0.0, 1.58) {$\mathbf{k}_1$};
  \node[kv, fill=CellGreen]   (k2) at (1.7, 1.58) {$\mathbf{k}_2$};
  \node[kv, fill=CellGreen]   (k3) at (3.4, 1.58) {$\mathbf{k}_3$};
  \node[kv, fill=CellYellow]  (k4) at (5.1, 1.58) {$\mathbf{k}_4$};

  \node[kv, fill=CellGreen]   (v1) at (0.0, 0.96) {$\mathbf{v}_1$};
  \node[kv, fill=CellGreen]   (v2) at (1.7, 0.96) {$\mathbf{v}_2$};
  \node[kv, fill=CellGreen]   (v3) at (3.4, 0.96) {$\mathbf{v}_3$};
  \node[kv, fill=CellYellow]  (v4) at (5.1, 0.96) {$\mathbf{v}_4$};

  \node[font=\scriptsize, gray, left=0.1cm of k1] {$\mathbf{K}$:};
  \node[font=\scriptsize, gray, left=0.1cm of v1] {$\mathbf{V}$:};
...
\begin{enumerate}[a)]
    \item Apenas $\mathbf{k}_4$ e $\mathbf{v}_4$ são computados; $\mathbf{k}_1$--$\mathbf{k}_3$ e $\mathbf{v}_1$--$\mathbf{v}_3$
          são reutilizados do cache.
    \item Todos os $\mathbf{k}_1$--$\mathbf{k}_4$ e $\mathbf{v}_1$--$\mathbf{v}_4$ são recalculados integralmente
          a cada novo token gerado.
    \item $\mathbf{k}_4$ é computado, mas todos os $\mathbf{v}_1$--$\mathbf{v}_4$ são recalculados
          para combinar com a nova query $\mathbf{q}_4$.
...
\item O gráfico abaixo mostra a distribuição de probabilidade sobre
      quatro tokens para os mesmos logits $\mathbf{z}{=}[3,\;1,\;0,\;{-1}]$,
...
\begin{enumerate}[a)]
    ...
    \item A perplexidade independe do tokenizador, permitindo
          ...
          múltiplos passos.
    \item A perplexidade é barata e não exige dados rotulados, mas
          correlaciona-se imperfeitamente com tarefas específicas;
          o MMLU mede capacidades concretas, mas é sensível à
          contaminação do corpus.
...
\begin{tikzpicture}[
  ...
  \foreach \i/\x in {1/0.3, 2/1.0, 3/1.7, 4/2.4} {
    \node[qh] (qm\i) at (\x, 1.8) {$\mathbf{Q}_{\i}$};
    \node[kh] (km\i) at (\x, 0.8) {$\mathbf{KV}_{\i}$};
    \draw[arr] (km\i) -- (qm\i);
  }
  ...
  \foreach \i/\x in {1/4.1, 2/4.9, 3/5.7, 4/6.5} {
    \node[qh] (qg\i) at (\x, 1.8) {$\mathbf{Q}_{\i}$};
  }
  \node[kh] (kga) at (4.5, 0.8) {$\mathbf{KV}_a$};
  \node[kh] (kgb) at (6.1, 0.8) {$\mathbf{KV}_b$};
  \draw[arr] (kga) -- (qg1);
  \draw[arr] (kga) -- (qg2);
  \draw[arr] (kgb) -- (qg3);
  \draw[arr] (kgb) -- (qg4);
...
% ------------------------------------------------------------------
\item Em um modelo Mixture-of-Experts (MoE) com $N_E = 4$ especialistas
...
\begin{enumerate}[a)]
    \item \textbf{Overfitting} do router: o router memoriza os tokens de treino;
          solução: aumentar o número de especialistas $N_E$.
    ...
    \item \textbf{Expert collapse}: o especialista mais ativo recebe cada vez mais
          tokens; solução: perda auxiliar de balanceamento.
\end{enumerate}
...
\item Na FFN SwiGLU, o cálculo é:
      \[
        \mathrm{FFN}(\mathbf{x}) = \bigl(\mathrm{SiLU}(\mathbf{x}\mathbf{W}_1) \odot \mathbf{x}\mathbf{W}_3\bigr)\mathbf{W}_2,
      \]
      enquanto a ReLU-FFN clássica usa $\mathrm{FFN}(\mathbf{x}) = \max(0,\,\mathbf{x}\mathbf{W}_1)\,\mathbf{W}_2$.
      Qual é a principal vantagem do portão $\mathbf{x}\mathbf{W}_3$ em relação ao corte
      estático da ReLU?
...
\begin{enumerate}[a)]
    ...
    \item O portão $\mathbf{x}\mathbf{W}_3$ é computado a partir da entrada e determina
          dinamicamente quais dimensões do espaço oculto passam adiante.
...
\begin{enumerate}[a)]
    ...
    \item Aceitação $= 0.30$; substituto amostrado de $q$, a distribuição
          do próprio \textit{draft}.
\end{enumerate}
...
\begin{enumerate}[a)]
    \item $k = 10$ é viável: sobra espaço na janela de $2048$ tokens
          para os exemplares, a consulta e a geração.
    \item $k = 10$ não é viável: o prompt excede a janela de $2048$
          tokens, e o modelo precisaria truncar o contexto ou a geração.
...
Durante a fase de \emph{decode}, o KV-cache armazena os pares $\mathbf{k}_j, \mathbf{v}_j$ de todos
os tokens já processados. Para gerar o token na posição~4, o modelo computa $\mathbf{k}_4$
e $\mathbf{v}_4$ aplicando as projeções $\mathbf{W}^K$ e $\mathbf{W}^V$ ao embedding do novo token; os pares
$\mathbf{k}_1,\mathbf{v}_1$, $\mathbf{k}_2,\mathbf{v}_2$ e $\mathbf{k}_3,\mathbf{v}_3$ são lidos diretamente do cache sem recálculo.
Vale separar as duas contas. \emph{Por passo} de decodificação, sem cache é preciso
reprojetar os $n$ tokens já vistos, o que custa $\mathcal{O}(n\,d^2)$, mais
$\mathcal{O}(n\,d)$ da atenção propriamente dita; com cache resta apenas o
$\mathcal{O}(n\,d)$ da atenção, porque as projeções antigas não são refeitas.
Somando sobre os $n$ passos, o custo \emph{total} da geração cai de
$\mathcal{O}(n^2 d^2)$ para $\mathcal{O}(n^2 d)$. É essa economia que torna viável a
geração em sequências longas.
% ---
...
\[
  \mathcal{L}_{\text{aux}} = \alpha\, N_E \sum_{i=1}^{N_E} f_i\, P_i,
\]
...
incentiva distribuição uniforme entre especialistas (o Switch Transformer usa $\alpha=0.01$).
% ---
...
Na SwiGLU, o portão $\mathbf{x}\mathbf{W}_3$ é um vetor computado a partir da entrada $\mathbf{x}$: para
cada token diferente, $\mathbf{x}\mathbf{W}_3$ produz valores distintos, de modo que o modelo
...
treinado em ${\approx}15$T tokens, ou seja ${\approx}1875$ tokens por parâmetro e
${\approx}94 \times D^*$: do ponto de
vista de \emph{inferência}, vale gastar mais no treinamento para obter
um modelo menor e mais barato de implantar.

As opções~(a) e~(c) erram a regra ($10N$ em vez de $20N$).
A opção~(a) agrava o erro ao afirmar que a perda já saturou em $2.22$ e que a segunda
rodada não traria melhoria alguma: além de $D^*$ a perda apenas \emph{diminui mais
lentamente}, mas não para de diminuir.
A opção~(b) inverte a direção: com $D^* = 20N = 160$B, o modelo com
...
A opção~(a) ignora que o prompt de sistema ($150$) e a consulta ($200$) também ocupam a
janela: só os exemplares já somam $10 \times 200 = 2000$ dos $2048$ tokens disponíveis.
...
\texttt{seu} desnecessariamente, pois o limiar já foi satisfeito. A opção~(c) é o
