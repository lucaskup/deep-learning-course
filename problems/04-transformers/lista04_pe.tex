...
\begin{enumerate}[a)]
    ...
    \item Sim, para posições práticas ($i \ll 10^4$): as frequências
          ...
          tornando o mapeamento $i \mapsto \mathbf{PE}(i)$ injetivo nesse domínio.
    \item Não, pois a norma de $\mathbf{PE}(i)$ é constante igual a $\sqrt{d/2}$
...
Para que dois inteiros $i \neq j$ produzam o mesmo $\mathbf{PE}$, seria necessário que
...
A opção (c) erra ao afirmar norma constante: em geral $\|\mathbf{PE}(i)\| \neq \sqrt{d/2}$
...
\]
Usando $\mathbf{R}(i)^\top = \mathbf{R}(-i)$ (matrizes de rotação são ortogonais) e
a propriedade de composição $\mathbf{R}(-i)\,\mathbf{R}(j) = \mathbf{R}(j-i)$:
\[
  \tilde{\mathbf{q}}_i^\top \tilde{\mathbf{k}}_j
