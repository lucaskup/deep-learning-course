...
\begin{enumerate}[a)]
    \item Porque o score de atenção $\mathbf{q}_i^\top \mathbf{k}_j / \sqrt{d}$ não depende
          dos índices $i$ e $j$.
    \item Porque a função softmax não é diferenciável sem informação de posição.
    \item Porque a multiplicação de matrizes não é comutativa e a ordem dos
          tokens se perde.
...
\begin{enumerate}[a)]
    \item O NoPE utiliza uma dimensão de embedding maior, aumentando a capacidade
          do modelo para sequências longas.
    \item O positional encoding absoluto produz vetores fora da distribuição além
          do comprimento treinado; o NoPE não soma nada.
    \item O NoPE aplica um decaimento cosseno aos pesos de atenção, que se estende
          a qualquer comprimento.
    \item O NoPE requer um embedding aprendido para cada posição, o que é mais
          flexível que uma fórmula fixa.
\end{enumerate}

% ------------------------------------------------------------------
% Q6: Fácil
% ------------------------------------------------------------------
\item Qual estratégia de positional encoding possui um \emph{limite rígido}:
...
\begin{enumerate}[a)]
    ...
    \item ALiBi
    \item RoPE
    \item Positional encoding absoluto aprendido
\end{enumerate}
...
\begin{enumerate}[a)]
    ...
    \item $\mathbf{PE}(i + \delta)$ pode ser escrito como uma transformação linear
          fixa de $\mathbf{PE}(i)$.
...
\begin{enumerate}[a)]
    ...
    \item O PI não requer fine-tuning; o YaRN requer dois estágios de fine-tuning
          com dados longos.
    \item O PI modifica a máscara de atenção; o YaRN modifica apenas os vetores
          key antes da rotação.
    \item O PI aplica um fator de escala uniforme a todas as posições; o YaRN
          aplica fatores por dimensão de frequência.
\end{enumerate}
...
\begin{enumerate}[a)]
    \item Cada token na posição $i$ atende exatamente a $i+1$ tokens, criando um
          padrão único por linha.
    \item A máscara introduz um padrão senoidal nos pesos de atenção, equivalente
          ao PE absoluto.
    \item Mascarar tokens futuros força o modelo a memorizar as posições absolutas
          vistas no treino.
    \item A máscara triangular inferior é equivalente a subtrair as penalidades
          lineares do ALiBi.
\end{enumerate}
...
\begin{enumerate}[a)]
    \item Porque o BERT utiliza tokenização de subpalavras, que é incompatível
          com a ausência de positional encoding.
    \item Porque modelos encoder-only não possuem máscara causal, sem a qual o
          NoPE fica sem sinal posicional.
    \item Porque o BERT possui um vocabulário pequeno demais para inferir a
          posição a partir do contexto.
...
\begin{enumerate}[a)]
    ...
    \item Trocar o positional encoding do modelo de RoPE para senoidal absoluto
          e refazer o fine-tuning.
    \item Usar Retrieval-Augmented Generation (RAG) e posicionar o conteúdo
          crítico no início ou no final do prompt.
...
\item \emoji{skull-and-crossbones} Considere o RoPE com $d = 4$ e frequências $\theta_0 = 1.0$,
      $\theta_1 = 0.01$. A query é $\mathbf{q} = [2, 0, 1, 0]$ na posição $i = 2$,
      e a key é $\mathbf{k} = [0, 1, 1, 0]$ na posição $j = 5$.
      Usando $\cos(3.0) \approx -0.990$, $\sin(3.0) \approx 0.141$,
      $\cos(0.03) \approx 1.000$, $\sin(0.03) \approx 0.030$, qual é
      $\mathrm{score}(2, 5) = \tilde{\mathbf{q}}_2^\top \tilde{\mathbf{k}}_5$?
...
\begin{enumerate}[a)]
    \item O score depende apenas do offset relativo $(j - i)$, e não das posições
          absolutas $i$ ou $j$.
    \item O score é sempre zero quando $i = j$, o que impede o self-attention
          na diagonal.
    \item O score é limitado ao intervalo $[-1, +1]$ para qualquer par de query
          e key.
...
\begin{enumerate}[a)]
    \item Dimensões de $k$ alto (baixa frequência), pois não completam muitas
          rotações dentro de $L_{\mathrm{train}}$.
    \item Todas as dimensões são igualmente afetadas, pois o mesmo fator de escala
          é aplicado uniformemente.
    \item Dimensões de $k$ médio, pois suas frequências estão mais próximas da taxa
          de Nyquist do índice comprimido.
    \item Dimensões de $k$ baixo (alta frequência), pois já completam muitas
          rotações dentro de $L_{\mathrm{train}}$.
\end{enumerate}
...
\item \emoji{skull-and-crossbones} Considere o positional encoding senoidal
      $\mathbf{PE}(i) \in \mathbb{R}^d$ com $d = 512$ para posições inteiras $i \geq 0$.
      Em \textbf{aritmética exata} (ou seja, ignorando limitações de ponto flutuante),
      é possível recuperar univocamente a posição $i$ a partir do vetor $\mathbf{PE}(i)$?
%R: b

\begin{enumerate}[a)]
    \item Não, pois seno e cosseno são periódicas com período $2\pi$, e posições
          inteiras distintas cujos argumentos coincidem módulo $2\pi$ recebem o
          mesmo vetor.
    \item Sim, o mapeamento $i \mapsto \mathbf{PE}(i)$ é injetivo para todo inteiro
          $i \geq 0$, sem restrição de faixa.
    \item Não, pois a norma de $\mathbf{PE}(i)$ é a mesma para todo $i$, o que
          impede distinguir posições distintas.
    \item Sim, mas apenas para $i$ menor que o período da dimensão mais lenta,
          $\approx 2\pi \cdot 10000$; a partir daí o vetor se repete.
\end{enumerate}
...
\item \emoji{skull-and-crossbones} Um modelo com positional encoding absoluto
      ...
      O que ocorre com os tokens nas posições $512$ a $599$ (indexação a partir de $0$)?
%R: b

\begin{enumerate}[a)]
    \item O modelo extrapola os embeddings posicionais por interpolação linear
          entre $\mathbf{PE}(511)$ e $\mathbf{PE}(0)$, sem perda de desempenho.
    \item Essas posições não possuem embeddings treinados; a implementação típica
          lança um erro de índice (\textit{index out of bounds}) ou reutiliza
          vetores de posições iniciais.
    \item A camada posicional gera os embeddings faltantes pela fórmula senoidal,
          usada como \textit{fallback} padrão para índices fora da tabela.
...
\begin{enumerate}[a)]
    ...
    \item São equivalentes quando $\mathbf{W}_Q = \mathbf{W}_K = \mathbf{I}$ (projeções identidade),
          pois nesse caso adição e rotação têm o mesmo efeito sobre o produto interno.
    \item O PE senoidal é um caso especial do RoPE aplicado somente às duas
          primeiras dimensões do embedding de entrada.
    \item Compartilham o mesmo espectro, mas o PE senoidal soma posição
          \emph{absoluta} antes das projeções e o RoPE aplica rotação
          \emph{relativa} depois delas; nenhuma configuração de pesos os
          torna equivalentes.
\end{enumerate}
...
\item \emoji{skull-and-crossbones} Para o RoPE com $d = 2$ e parâmetro $\theta$,
      ...
      \[
        ...
        \end{pmatrix}.
      \]
      Dado que matrizes de rotação satisfazem $\mathbf{R}(i)^\top \mathbf{R}(j) = \mathbf{R}(j - i)$,
      e que $\tilde{\mathbf{q}}_i = \mathbf{R}(i)\,\mathbf{q}$ e
      $\tilde{\mathbf{k}}_j = \mathbf{R}(j)\,\mathbf{k}$, como o score de atenção
...
\begin{enumerate}[a)]
    ...
    \item $\mathbf{q}^\top \mathbf{R}(j-i)\,\mathbf{k}$, dependendo apenas do offset
          relativo $(j-i)$.
    \item $\mathbf{q}^\top \mathbf{R}(i+j)\,\mathbf{k}$,
...
O RoPE (Su et al., 2024) aplica uma matriz de rotação $\mathbf{R}_\theta(i)$ diretamente aos
vetores de query $\mathbf{q}_i$ e key $\mathbf{k}_j$ dentro de cada cabeça de atenção:
$\tilde{\mathbf{q}}_i = \mathbf{R}_\theta(i)\,\mathbf{q}_i$.
...
uma matriz treinável $\mathbf{E} \in \mathbb{R}^{L_{\max} \times d}$. Uma posição $i > L_{\max}$
simplesmente não tem linha em $\mathbf{E}$: o embedding não existe. O encoding senoidal possui
...
$\mathbf{PE}(i + \delta) = \mathbf{M}(\delta)\,\mathbf{PE}(i)$
para uma matriz de rotação em blocos \emph{fixa} $\mathbf{M}(\delta)$ que depende apenas de
...
(2022) reportam que um modelo com ALiBi treinado com contexto de 1024 tokens
\emph{mantém perplexidade baixa} ao ser avaliado em sequências mais longas,
igualando a perplexidade de um modelo com PE senoidal treinado diretamente com
2048 tokens, sem nenhum fine-tuning. Vale notar que essa
...
O caminho mais curto usa a propriedade relativa do RoPE: o produto interno dos vetores
rotacionados depende de $i$ e $j$ apenas por $j - i = 3$. Bloco a bloco,
\[
  \mathrm{score}(i,j) = \sum_{b}\Bigl[(q_{2b}k_{2b} + q_{2b+1}k_{2b+1})\cos\bigl((j-i)\theta_b\bigr)
  + (q_{2b+1}k_{2b} - q_{2b}k_{2b+1})\sin\bigl((j-i)\theta_b\bigr)\Bigr].
\]

\textbf{Bloco $b=0$} ($\mathbf{q} = (2,0)$, $\mathbf{k} = (0,1)$, ângulo $(j-i)\theta_0 = 3.0$):
\[
  (2\cdot0 + 0\cdot1)\cos(3.0) + (0\cdot0 - 2\cdot1)\sin(3.0)
  = 0 - 2\,(0.141) = -0.282.
\]

\textbf{Bloco $b=1$} ($\mathbf{q} = (1,0)$, $\mathbf{k} = (1,0)$, ângulo $(j-i)\theta_1 = 0.03$):
\[
  (1\cdot1 + 0\cdot0)\cos(0.03) + (0\cdot1 - 1\cdot0)\sin(0.03)
  = 1.000 + 0 = 1.000.
\]

\textbf{Total:} $\mathrm{score}(2,5) \approx -0.282 + 1.000 = 0.718$.

A alternativa (a) $+1.282$ troca o sinal do termo em seno, o que equivale a usar
$i - j$ no lugar de $j - i$; (c) $-0.282$ considera apenas o bloco $b=0$; (d) $+1.000$
considera apenas o bloco $b=1$.
% ---

% --- Solução Q20 ---
\subsection*{XIX, Resposta:(a)}
...
Basta olhar a dimensão $k = 0$, cujo argumento é o próprio $i$. Dois inteiros
$i \neq j$ produziriam o mesmo par $(\sin i, \cos i) = (\sin j, \cos j)$ apenas se
$i - j$ fosse um múltiplo inteiro não nulo de $2\pi$, o que é impossível porque
$2\pi$ é irracional. Logo a primeira dimensão já separa quaisquer duas posições
inteiras e o mapeamento $i \mapsto \mathbf{PE}(i)$ é injetivo em todo
$\mathbb{Z}_{\geq 0}$.
A opção (a) supõe que existam inteiros distintos congruentes módulo $2\pi$, o que
nunca ocorre.
A opção (c) afirma um fato verdadeiro (a norma vale $\sqrt{d/2}$ para todo $i$),
mas a norma é irrelevante: posições distintas diferem na direção do vetor, não no
comprimento.
A opção (d) inventa um limite de faixa: a dimensão mais lenta tem período
$\approx 2\pi \cdot 10000$, mas o vetor completo não se repete em nenhum inteiro,
pois a dimensão $k = 0$ nunca repete.
% ---
...
\begin{itemize}
  ...
  \item Em implementações que truncam o índice (\textit{clamping}), as posições 512
        a 599 receberiam o embedding da posição 511, repetindo-o para todos os tokens
...
\begin{itemize}
  \item \textbf{PE senoidal}: adiciona $\mathbf{PE}(i)$ ao token embedding
        \emph{antes} das projeções $\mathbf{W}_Q$, $\mathbf{W}_K$. O score de atenção contém termos
        cruzados conteúdo-posição e termos de posição absoluta.
  \item \textbf{RoPE}: aplica $\mathbf{R}_\theta(i)$ \emph{após} a projeção $\mathbf{W}_Q\mathbf{x}_i$,
        resultando em $\mathbf{q}^\top \mathbf{R}(j-i)\,\mathbf{k}$, dependência
        exclusivamente relativa, sem termos de posição absoluta no score.
\end{itemize}
Mesmo com $\mathbf{W}_Q = \mathbf{W}_K = \mathbf{I}$, os dois scores diferem: o PE senoidal ainda gera termos
cruzados $\mathbf{x}_i^\top\mathbf{PE}(j)$ que o RoPE não possui. Não existe,
...
\[
  ...
  = \bigl(\mathbf{R}(i)\,\mathbf{q}\bigr)^\top\bigl(\mathbf{R}(j)\,\mathbf{k}\bigr)
  = \mathbf{q}^\top \mathbf{R}(i)^\top \mathbf{R}(j)\,\mathbf{k}.
\]
Usando $\mathbf{R}(i)^\top = \mathbf{R}(-i)$ (rotações são ortogonais) e a
composição $\mathbf{R}(-i)\,\mathbf{R}(j) = \mathbf{R}(j-i)$:
\[
  \tilde{\mathbf{q}}_i^\top \tilde{\mathbf{k}}_j
  = \mathbf{q}^\top \mathbf{R}(j-i)\,\mathbf{k}.
\]
...
o produto $\mathbf{q}^\top \mathbf{R}(\delta)\,\mathbf{k}$ é bem definido para qualquer
...
\begin{itemize}
  \item Prova algébrica da propriedade de posição relativa do \textbf{RoPE}
        (questões XVIII, XIX e XXV);
  \item Cálculo dos \textit{slopes} e das penalidades do \textbf{ALiBi} (questão XXI);
  \item Argumento de injetividade do \textbf{positional encoding senoidal} (questão XXII);
  \item Análise das consequências da compressão uniforme no \textbf{Position Interpolation}
        (questão XX);
  \item Comportamento do PE aprendido além de $L_{\max}$ (questão XXIII);
  \item Comparação matemática entre RoPE e PE senoidal (questão XXIV).
\end{itemize}

% ------------------------------------------------------------------
% Q18: Derivação Detalhada
% ------------------------------------------------------------------
\subsection*{XVIII \emoji{skull-and-crossbones}: Cálculo Numérico do Score RoPE}

\paragraph{Enunciado.}
Considere o RoPE com $d = 4$ e frequências $\theta_0 = 1.0$, $\theta_1 = 0.01$.
A query é $\mathbf{q} = [2, 0, 1, 0]$ na posição $i = 2$, e a key é
$\mathbf{k} = [0, 1, 1, 0]$ na posição $j = 5$. Usando os valores trigonométricos
\[
  \cos(3.0) \approx -0.990,\quad \sin(3.0) \approx 0.141,\quad
  \cos(0.03) \approx 1.000,\quad \sin(0.03) \approx 0.030,
\]
calcule $\mathrm{score}(2,5) = \tilde{\mathbf{q}}_2^\top \tilde{\mathbf{k}}_5$.

\begin{respostabox}
Alternativa \textbf{(b)}: $+0.718$.
\end{respostabox}

\paragraph{Derivação pela propriedade relativa.}
Os valores fornecidos no enunciado são os do ângulo $(j-i)\theta_b$, e não os de
$i\theta_b$ ou $j\theta_b$: isso já indica o caminho. O produto interno dos vetores
rotacionados depende das posições apenas pela diferença $j - i = 3$, e se separa por
bloco de frequência:
\[
  \mathrm{score}(i,j) = \sum_{b}\Bigl[(q_{2b}k_{2b} + q_{2b+1}k_{2b+1})\cos\bigl((j-i)\theta_b\bigr)
  + (q_{2b+1}k_{2b} - q_{2b}k_{2b+1})\sin\bigl((j-i)\theta_b\bigr)\Bigr].
...
\bigskip
\textbf{Bloco $b = 0$.} Aqui $\mathbf{q}_{[0:2]} = (2, 0)$, $\mathbf{k}_{[0:2]} = (0, 1)$
e o ângulo é $(j-i)\theta_0 = 3 \times 1.0 = 3.0\;\mathrm{rad}$:
\[
  \underbrace{(2 \cdot 0 + 0 \cdot 1)}_{= 0}\cos(3.0)
  + \underbrace{(0 \cdot 0 - 2 \cdot 1)}_{= -2}\sin(3.0)
  = 0 - 2\,(0.141) = -0.282.
\]

\bigskip
\textbf{Bloco $b = 1$.} Aqui $\mathbf{q}_{[2:4]} = (1, 0)$, $\mathbf{k}_{[2:4]} = (1, 0)$
e o ângulo é $(j-i)\theta_1 = 3 \times 0.01 = 0.03\;\mathrm{rad}$:
\[
  \underbrace{(1 \cdot 1 + 0 \cdot 0)}_{= 1}\cos(0.03)
  + \underbrace{(0 \cdot 1 - 1 \cdot 0)}_{= 0}\sin(0.03)
  = 1.000 + 0 = 1.000.
\]
...
\paragraph{Verificação via rotação explícita.}
Rotacionando cada vetor pela sua própria posição, o que exige os valores
$\cos(2.0) \approx -0.416$, $\sin(2.0) \approx 0.909$, $\cos(5.0) \approx 0.284$,
$\sin(5.0) \approx -0.959$, $\cos(0.02) \approx 1.000$, $\sin(0.02) \approx 0.020$,
$\cos(0.05) \approx 0.999$ e $\sin(0.05) \approx 0.050$:
\[
  \tilde{\mathbf{q}}_{2,[0:2]}
  = \begin{pmatrix}-0.416 & -0.909 \\ 0.909 & -0.416\end{pmatrix}
    \begin{pmatrix}2\\0\end{pmatrix}
  = \begin{pmatrix}-0.832\\ 1.818\end{pmatrix},
  \qquad
  \tilde{\mathbf{q}}_{2,[2:4]}
  = \begin{pmatrix}1.000 & -0.020 \\ 0.020 & 1.000\end{pmatrix}
    \begin{pmatrix}1\\0\end{pmatrix}
  = \begin{pmatrix}1.000\\ 0.020\end{pmatrix}.
\]
\[
  \tilde{\mathbf{k}}_{5,[0:2]}
  = \begin{pmatrix}\phantom{-}0.284 & 0.959 \\ -0.959 & 0.284\end{pmatrix}
    \begin{pmatrix}0\\1\end{pmatrix}
  = \begin{pmatrix}0.959\\ 0.284\end{pmatrix},
  \qquad
  \tilde{\mathbf{k}}_{5,[2:4]}
  = \begin{pmatrix}0.999 & -0.050 \\ 0.050 & \phantom{-}0.999\end{pmatrix}
    \begin{pmatrix}1\\0\end{pmatrix}
  = \begin{pmatrix}0.999\\ 0.050\end{pmatrix}.
\]
Logo $\tilde{\mathbf{q}}_2 = [-0.832,\; 1.818,\; 1.000,\; 0.020]$ e
$\tilde{\mathbf{k}}_5 = [0.959,\; 0.284,\; 0.999,\; 0.050]$, e
\[
  \mathrm{score}(2,5)
  = \underbrace{(-0.832)(0.959)}_{\approx -0.798}
  + \underbrace{(1.818)(0.284)}_{\approx +0.516}
  + \underbrace{(1.000)(0.999)}_{\approx +0.999}
  + \underbrace{(0.020)(0.050)}_{\approx +0.001}
  \approx 0.718 \;\checkmark
\]
O caminho relativo exige quatro valores trigonométricos e duas multiplicações; o
caminho explícito exige oito valores e quatro produtos de matriz. Essa diferença é
exatamente o conteúdo da propriedade que a questão quer testar.
% ------------------------------------------------------------------
% Q19: Derivação Detalhada
% ------------------------------------------------------------------
\subsection*{XIX \emoji{skull-and-crossbones}: Expansão Algébrica do Score RoPE e Posição Relativa}
...
% ------------------------------------------------------------------
\subsection*{XX \emoji{skull-and-crossbones}: Compressão Uniforme no Position Interpolation}
...
% ------------------------------------------------------------------
% Q22: Derivação Detalhada
% ------------------------------------------------------------------
\subsection*{XXI \emoji{skull-and-crossbones}: Cálculo dos Slopes e Penalidades ALiBi}
...
% ------------------------------------------------------------------
\subsection*{XXII \emoji{skull-and-crossbones}: Injetividade do Positional Encoding Senoidal}

\paragraph{Enunciado.}
Considere o positional encoding senoidal $\mathbf{PE}(i) \in \mathbb{R}^d$ com
$d = 512$ para posições inteiras $i \geq 0$. É possível recuperar univocamente a
posição $i$ a partir do vetor $\mathbf{PE}(i)$?

\begin{respostabox}
Alternativa \textbf{(b)}: Sim, o mapeamento $i \mapsto \mathbf{PE}(i)$ é injetivo
para todo inteiro $i \geq 0$, sem restrição de faixa.
\end{respostabox}
...
\[
  \mathrm{PE}(i, 2k)   = \sin\!\left(i\,\omega_k\right),\quad
  \mathrm{PE}(i, 2k+1) = \cos\!\left(i\,\omega_k\right),\quad
  \omega_k = 10000^{-2k/d},\quad k = 0, 1, \ldots, \frac{d}{2}-1.
\]

\textbf{Condição de colisão.} Dois inteiros $i \neq j$ com $i, j \geq 0$ produzem
$\mathbf{PE}(i) = \mathbf{PE}(j)$ se e somente se, para \emph{todo} $k$:
\[
  i\,\omega_k \equiv j\,\omega_k \pmod{2\pi},
\]
ou seja, $(i - j)\,\omega_k$ é múltiplo inteiro de $2\pi$ para todo $k$.
...
O mapeamento $i \mapsto \mathbf{PE}(i)$ é \textbf{estritamente injetivo} para todo
$i, j \in \mathbb{Z}_{\geq 0}$ com $i \neq j$.

\paragraph{Observação.}
Na prática, com aritmética de ponto flutuante, posições muito distantes podem
produzir vetores numericamente próximos, mas o argumento acima é exato: a
irracionalidade de $2\pi$ garante a injetividade sobre todos os inteiros
não negativos.

\paragraph{Análise das alternativas incorretas.}

\begin{itemize}
  \item \textbf{(a) Errada.} A alternativa supõe que existam posições inteiras
        distintas congruentes módulo $2\pi$. Contudo, $2\pi$ é irracional, então
        nenhum múltiplo não nulo de $2\pi$ é inteiro. A premissa da opção (a) nunca
        é satisfeita para posições inteiras.

  \item \textbf{(c) Errada.} A norma de $\mathbf{PE}(i)$ é:
        \[
          \|\mathbf{PE}(i)\|^2 = \sum_{k=0}^{d/2-1}
          \!\left[\sin^2\!\!\left(i\,\omega_k\right)
                + \cos^2\!\!\left(i\,\omega_k\right)\right]
        ...
        de $j$ é a \emph{direção} do vetor $\mathbf{PE}(i)$, não sua magnitude.
        Dois vetores podem ter a mesma norma e ainda assim ser distintos.

  \item \textbf{(d) Errada.} A dimensão mais lenta ($k = d/2 - 1$) tem período
        $2\pi\,\omega_{d/2-1} \approx 2\pi \cdot 10000 \approx 62\,832$ posições, mas
        a injetividade do vetor completo não depende dela. A dimensão $k = 0$ já
        distingue todos os inteiros, então não existe faixa além da qual
        $\mathbf{PE}(i)$ se repita.
\end{itemize}

% ------------------------------------------------------------------
% Q24: Derivação Detalhada
% ------------------------------------------------------------------
\subsection*{XXIII \emoji{skull-and-crossbones}: PE Aprendido Além de $L_{\max}$}
...
Essa camada armazena uma matriz treinável $\mathbf{E} \in \mathbb{R}^{L_{\max} \times d}$
com uma linha para cada posição $i \in \{0, \ldots, L_{\max}-1\}$.

\textbf{Cenário 1: Erro de índice.}
Em frameworks padrão como PyTorch, consultar o índice 512 dessa tabela
(com \texttt{L\_max=512}, ela tem 512 linhas, de índice 0 a 511) produz:
\begin{center}
...
\end{center}
o que força qualquer posição $i \geq L_{\max}$ a usar o vetor treinado para
...
\textbf{Alternativas (a), (c) e (d).}
Nenhuma descreve um mecanismo existente: \texttt{nn.Embedding} não interpola entre
linhas, não possui \textit{fallback} senoidal para índices ausentes e não cria
vetores nulos sob demanda. Qualquer um desses comportamentos teria de ser
implementado explicitamente, e nenhum deles reproduziria a distribuição vista no
treino. A alternativa (b) é a única que descreve o que a tabela de consulta
realmente faz.
...
% ------------------------------------------------------------------
\subsection*{XXIV \emoji{skull-and-crossbones}: Relação Matemática entre RoPE e PE Senoidal}
...
\midrule
Ponto de aplicação & Antes de $\mathbf{W}_Q$, $\mathbf{W}_K$ (na entrada) & Após $\mathbf{W}_Q$, $\mathbf{W}_K$ \\
Operação & Adição: $\mathbf{x}_i + \mathbf{PE}(i)$ & Rotação: $\mathbf{R}(i)\,\mathbf{q}_i$ \\
Tipo de encoding & Posição absoluta & Posição relativa \\
Score de atenção & Contém termos cruzados conteúdo-posição & Apenas $\mathbf{q}^\top \mathbf{R}(j-i)\mathbf{k}$ \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Por que não são equivalentes mesmo com $\mathbf{W}_Q = \mathbf{W}_K = \mathbf{I}$?}

Com PE senoidal, o logit de atenção é:
\[
  A_{\mathrm{seno}}(i,j)
  = (\mathbf{x}_i + \mathbf{PE}(i))^\top (\mathbf{x}_j + \mathbf{PE}(j))
  = \mathbf{x}_i^\top \mathbf{x}_j
    + \mathbf{x}_i^\top \mathbf{PE}(j)
    + \mathbf{PE}(i)^\top \mathbf{x}_j
    + \mathbf{PE}(i)^\top \mathbf{PE}(j).
\]

O último termo $\mathbf{PE}(i)^\top \mathbf{PE}(j)$ se simplifica via identidades
trigonométricas:
\[
  \mathbf{PE}(i)^\top \mathbf{PE}(j)
  = \sum_k \cos\!\!\left((j-i)\,\omega_k\right),
\]
que depende apenas de $(j-i)$. Contudo, os termos cruzados
$\mathbf{x}_i^\top \mathbf{PE}(j)$ e $\mathbf{PE}(i)^\top \mathbf{x}_j$
introduzem dependência de posição absoluta misturada com conteúdo vetorial.

Com RoPE:
\[
  A_{\mathrm{RoPE}}(i,j)
  = (\mathbf{R}(i)\mathbf{x}_i)^\top (\mathbf{R}(j)\mathbf{x}_j)
  = \mathbf{x}_i^\top \mathbf{R}(j-i)\,\mathbf{x}_j.
\]
Não há termos cruzados conteúdo-posição absoluta: a posição entra exclusivamente
como offset relativo $\mathbf{R}(j-i)$.

Portanto, mesmo no caso mais favorável $\mathbf{W}_Q = \mathbf{W}_K = \mathbf{I}$, os dois logits
contêm termos estruturalmente distintos.

\paragraph{Por que a conclusão vale para \emph{qualquer} escolha de pesos.}
O caso identidade sozinho não basta para justificar a alternativa, então convém
fechar o argumento com projeções gerais. Escrevendo $\mathbf{M} = \mathbf{W}_Q^\top \mathbf{W}_K$,
o logit senoidal é
\[
  A_{\mathrm{sin}}(i,j)
  = \mathbf{x}_i^\top \mathbf{M}\, \mathbf{x}_j
  + \mathbf{x}_i^\top \mathbf{M}\, \mathbf{PE}(j)
  + \mathbf{PE}(i)^\top \mathbf{M}\, \mathbf{x}_j
  + \mathbf{PE}(i)^\top \mathbf{M}\, \mathbf{PE}(j),
\]
enquanto o do RoPE, que aplica a rotação \emph{depois} das projeções, é
\[
  A_{\mathrm{RoPE}}(i,j)
  = (\mathbf{R}(i)\mathbf{W}_Q\mathbf{x}_i)^\top (\mathbf{R}(j)\mathbf{W}_K\mathbf{x}_j)
  = \mathbf{x}_i^\top \mathbf{W}_Q^\top\, \mathbf{R}(j-i)\, \mathbf{W}_K\, \mathbf{x}_j.
\]
Considere deslocar as duas posições pela mesma quantidade $c$, mantendo o conteúdo
fixo. No RoPE, $A_{\mathrm{RoPE}}(i+c, j+c) = A_{\mathrm{RoPE}}(i,j)$ para
\textbf{toda} escolha de $\mathbf{W}_Q$ e $\mathbf{W}_K$, porque $\mathbf{R}((j+c)-(i+c)) = \mathbf{R}(j-i)$.
No senoidal, os termos cruzados $\mathbf{x}_i^\top \mathbf{M}\, \mathbf{PE}(j)$ e
$\mathbf{PE}(i)^\top \mathbf{M}\, \mathbf{x}_j$ misturam o conteúdo de um token com a posição
\emph{absoluta} do outro, e mudam sob esse deslocamento sempre que $\mathbf{M}$ não os
anula. Uma invariância que vale sempre em um método e falha no outro não pode ser
reconciliada por escolha de pesos: só haveria equivalência se $\mathbf{M}$ zerasse os
termos cruzados \emph{e} o termo $\mathbf{PE}(i)^\top \mathbf{M}\, \mathbf{PE}(j)$ reproduzisse
exatamente a rotação, o que exigiria $\mathbf{M} = \mathbf{0}$ e destruiria também o termo de
conteúdo $\mathbf{x}_i^\top \mathbf{M}\, \mathbf{x}_j$. Logo não existe configuração de pesos que
torne os dois métodos equivalentes em geral.

% ------------------------------------------------------------------
% Q26: Derivação Detalhada
% ------------------------------------------------------------------
\subsection*{XXV \emoji{skull-and-crossbones}: Prova da Propriedade de Posição Relativa do RoPE}
...
\[
  ...
  \end{pmatrix}.
\]
Dado que $\mathbf{R}(i)^\top \mathbf{R}(j) = \mathbf{R}(j-i)$ e que $\tilde{\mathbf{q}}_i = \mathbf{R}(i)\,\mathbf{q}$
e $\tilde{\mathbf{k}}_j = \mathbf{R}(j)\,\mathbf{k}$, como o score
$\tilde{\mathbf{q}}_i^\top \tilde{\mathbf{k}}_j$ pode ser reescrito?

\begin{respostabox}
Alternativa \textbf{(b)}: $\mathbf{q}^\top \mathbf{R}(j-i)\,\mathbf{k}$, mostrando que o
score depende apenas do offset relativo $(j-i)$.
\end{respostabox}

\paragraph{Prova completa em três partes.}

\textbf{Parte 1: Ortogonalidade de $\mathbf{R}(i)$.}

$\mathbf{R}(i)$ é uma matriz de rotação plana e, portanto, ortogonal:
\[
  \mathbf{R}(i)^\top \mathbf{R}(i) = \mathbf{I} \quad \Longrightarrow \quad \mathbf{R}(i)^\top = \mathbf{R}(i)^{-1} = \mathbf{R}(-i).
\]
...
\textbf{Parte 2: Propriedade de composição $\mathbf{R}(a) \mathbf{R}(b) = \mathbf{R}(a+b)$.}
...
\begin{align*}
  \tilde{\mathbf{q}}_i^\top \tilde{\mathbf{k}}_j
  &= \bigl(\mathbf{R}(i)\,\mathbf{q}\bigr)^\top \bigl(\mathbf{R}(j)\,\mathbf{k}\bigr) \\
  &= \mathbf{q}^\top \mathbf{R}(i)^\top \mathbf{R}(j)\,\mathbf{k} & &\text{(transposição de produto)} \\
  &= \mathbf{q}^\top \mathbf{R}(-i) \mathbf{R}(j)\,\mathbf{k} & &\text{(Parte 1)} \\
  &= \mathbf{q}^\top \mathbf{R}(-i + j)\,\mathbf{k} & &\text{(Parte 2 com } a = -i,\, b = j\text{)} \\
  &= \mathbf{q}^\top \mathbf{R}(j - i)\,\mathbf{k}. & &
\end{align*}

\paragraph{Interpretação teórica.}

O resultado $\mathbf{q}^\top \mathbf{R}(j-i)\,\mathbf{k}$ mostra que:

\begin{enumerate}[1.]
  \item As posições absolutas $i$ e $j$ cancelam-se durante a derivação; apenas o
        offset relativo $\delta = j - i$ permanece.
  \item A matriz $\mathbf{R}(\delta)$ é uma rotação \emph{fixa} por ângulo $\delta\theta$ que
        pondera o produto interno entre $\mathbf{q}$ e $\mathbf{k}$ de forma dependente
        da separação entre os tokens.
  \item Para $\delta = 0$ (self-attention em $i = j$): $\mathbf{R}(0) = \mathbf{I}$, e o score reduz-se
  ...
  \item O resultado generaliza para $d > 2$: o RoPE aplica blocos de rotação 2D
        independentes $\{\mathbf{R}_k(\delta)\}_{k=0}^{d/2-1}$ a cada par de dimensões,
...
\]
onde $\mathbf{R}_k(\delta) = \mathbf{R}(\delta\theta_k)$ usa a frequência $\theta_k = 10000^{-2k/d}$.
