...
\begin{enumerate}[a)]
    ...
    \item $\mathbf{PE}(i + \delta)$ pode ser escrito como uma transformação linear fixa
          (multiplicação de matriz) de $\mathbf{PE}(i)$ para qualquer offset $\delta$.
...
\begin{enumerate}[a)]
    \item O modelo extrapola suavemente os embeddings posicionais usando interpolação
          linear entre $\mathbf{PE}(511)$ e $\mathbf{PE}(0)$.
...
\begin{enumerate}[a)]
    ...
    \item São equivalentes quando $\mathbf{W}_Q = \mathbf{W}_K = \mathbf{I}$ (projeções identidade), pois
    ...
    \item Compartilham o mesmo espectro de frequências, mas diferem fundamentalmente:
          ...
          entrada (antes das projeções lineares $\mathbf{W}_Q$, $\mathbf{W}_K$), enquanto o RoPE
...
\begin{enumerate}[a)]
    ...
    \item $\mathbf{q}^\top \mathbf{R}(j-i)\,\mathbf{k}$, mostrando que o score depende apenas
