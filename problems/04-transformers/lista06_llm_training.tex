...
\begin{enumerate}[a)]
    ...
    \item O gradiente passa a incluir os tokens de instrução, que seguem
          formatos repetitivos em todos os exemplos.
\end{enumerate}
% R: d
...
\begin{enumerate}[a)]
    \item \texttt{BF16} tem $8$ bits de expoente, a mesma faixa dinâmica do
          \texttt{fp32}, enquanto \texttt{fp16} tem $5$ bits de expoente e
          satura próximo de $65\,504$.
    ...
    \item \texttt{BF16} usa o dobro de bits do \texttt{fp16} ($32$ vs.\ $16$),
          o que explica o ganho de faixa dinâmica e de estabilidade numérica.
\end{enumerate}
% R: a
...
\begin{enumerate}[a)]
    ...
    \item $262\,144$ params ($\approx 1.56\%$).
\end{enumerate}
% R: d
...
\begin{enumerate}[a)]
    \item Com $\mathbf{B} = \mathbf{0}$, temos $\Delta\mathbf{W} = \tfrac{\alpha}{r}\mathbf{B}\mathbf{A} = \mathbf{0}$ no
          passo inicial, logo o modelo adaptado começa \emph{exatamente}
          do pré-treinamento.
...
% ------------------------------------------------------------------
\item Um bloco do Flash Attention processa os escores $\mathbf{s} = [0.5,\,2,\,1,\,3]$
      em dois tiles: $\mathbf{s}^{(1)} = [0.5,\,2]$ e $\mathbf{s}^{(2)} = [1,\,3]$. A recursão do
      ...
      (Use $e^{-1} \approx 0.368$, $e^{-1.5} \approx 0.223$,
      $e^{-2} \approx 0.135$.)
...
\begin{enumerate}[a)]
    ...
    \item \textit{Catastrophic forgetting}: a LR alta por muitas épocas
          sobrescreve representações do pré-treinamento. Correção: reduzir
          a LR e limitar o número de épocas.
    \item \textit{Overfitting} nos próprios \textit{benchmarks}; a correção
          é adicionar mais \textit{benchmarks} ao conjunto de avaliação e
          reavaliar o modelo.
    \item Explosão de gradiente causada pela LR alta; a correção é aumentar
          o \textit{weight decay} para $1.0$ e manter a LR.
\end{enumerate}
% R: b
...
\item \emoji{skull-and-crossbones} Em uma iteração de GRPO para um problema
      ...
      \]
      onde $\overline{r}$ e $\sigma_r$ são a média e o desvio-padrão
      \textbf{populacional} do grupo (divisão por $G$, não por $G-1$).
      Quais os valores de $\hat{A}$ para as respostas corretas e
      incorretas?
...
\[
  \eta_{T/2} = 10^{-4} + \tfrac{10^{-3} - 10^{-4}}{2}\cdot 1
             ...
             = 5.5\times 10^{-4}.
\]
A opção~(a) ignora totalmente o termo com cosseno.
A opção~(b) omite a parcela $\eta_{\min}$ da fórmula.
A opção~(d) confunde $\cos(\pi/2) = 0$ com $\cos(\pi) = -1$, obtendo
$\eta_{\min}$, valor correto apenas no fim do treino ($t = T$).

% --- Solução Q2 ---
\subsection*{II — Resposta: (b)}
...
As opções~(a), (c) e (d) ignoram subconjuntos dos componentes: somente pesos,
...
A opção~(a) é falsa: tokens de prompt estão no vocabulário (caso contrário,
não poderiam sequer ser alimentados como entrada).
A opção~(b) confunde mascaramento de \emph{loss} com ausência do token no
vocabulário.
A opção~(c) é falsa: a normalização é por $|\mathcal{R}|$ (tokens da resposta),
...
A opção~(a) inverte o sinal da diferença, aplicando $\sigma(-1.5) \approx 0.182$
...
A opção~(a) é falsa: o PPO \emph{inclui} duas cópias adicionais frozen,
portanto sua memória é maior.
A opção~(b) ignora o estado do otimizador da política ($100$ \texttt{GB}).
A opção~(d) reduz incorretamente cada método a uma única cópia.

\textbf{Observação.} O enunciado simplifica ao contabilizar estado de otimizador
apenas para a política. Na prática o PPO também \emph{treina} a \textit{value
function}, que tem estado AdamW próprio, enquanto a referência SFT e o
\textit{reward model} ficam congelados. Com isso a memória real do PPO é
consideravelmente maior que os $156$ \texttt{GB} calculados e a economia do DPO é
maior que os $28$ \texttt{GB}. A conta acima serve para ordenar os métodos, não
para dimensionar um cluster.

% --- Solução Q7 ---
\subsection*{VII — Resposta: (d)}
...
A opção~(a) conta apenas $d \cdot r$ por projeção, esquecendo a segunda matriz.
A opção~(b) duplica erroneamente o resultado.
A opção~(c) considera apenas uma das quatro projeções.
...
\textbf{Processando $\mathbf{s}^{(1)} = [0.5,\,2]$:}
\begin{align*}
  m^{(1)} &= \max(0.5,\,2) = 2 \\
  \ell^{(1)} &= e^{0.5-2} + e^{2-2} = 0.223 + 1 = 1.223.
\end{align*}
\textbf{Processando $\mathbf{s}^{(2)} = [1,\,3]$:}
\begin{align*}
  m^{(2)} &= \max(2,\,3) = 3 \\
  \ell^{(2)} &= e^{m^{(1)}-m^{(2)}}\cdot \ell^{(1)} + e^{1-3} + e^{3-3} \\
             &= e^{-1}\cdot 1.223 + e^{-2} + e^{0} \\
             &\approx 0.368 \cdot 1.223 + 0.135 + 1 \\
             &\approx 0.450 + 0.135 + 1 = \mathbf{1.585}.
\end{align*}
Conferência (softmax global): $e^{0.5-3}+e^{2-3}+e^{1-3}+e^{3-3}
\approx 0.082 + 0.368 + 0.135 + 1 = 1.585$. \checkmark
A opção~(a) omite o fator de rescala $e^{m^{\text{old}}-m^{\text{new}}}$,
tratando $\ell^{(1)}$ como se já estivesse normalizado para $m^{(2)}$.
A opção~(b) ignora $\mathbf{s}^{(2)}$.
...
e apenas $1$–$3$ épocas.
A opção~(a) contradiz a hipótese do alinhamento superficial (LIMA):
...
A opção~(a) usa os valores do slide, que valem apenas quando \emph{metade}
do grupo acerta ($G = 4$, 2 corretos, 2 errados), situação em que
$\sigma_r = 0.5$.
A opção~(b) esquece a normalização por $\sigma_r$.
A opção~(c) substitui a \textit{baseline} correta (média) por $\max(r) = 1$.
