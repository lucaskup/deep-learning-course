...
\item Um modelo possui $N = 2 \times 10^9$ parâmetros (2B). Um
      ...
      \begin{itemize}
        ...
        \item primeiro momento Adam $m$ em \texttt{fp32} ($4$ \texttt{B}/par)
        \item segundo momento Adam $v$ em \texttt{fp32} ($4$ \texttt{B}/par)
      \end{itemize}
...
\item Um \textit{transformer} com $d_{\text{model}} = 2\,048$ possui quatro
      ...
      quatro projeções ($h = W_0 x + \tfrac{\alpha}{r}\,B A x$, com
...
% ------------------------------------------------------------------
\item No LoRA, $A$ é inicializado com distribuição normal gaussiana e
      $B$ com \textbf{zeros}. Um estudante pergunta: ``por que não
      inicializar \emph{ambos} aleatoriamente?''. Qual a justificativa
      correta para a escolha $B = 0$?

\begin{enumerate}[a)]
    \item Com $B = 0$, temos $\Delta W = \tfrac{\alpha}{r}BA = 0$ no passo
          inicial, logo o modelo adaptado começa \emph{exatamente} do
          pré-treinamento. Se $B$ fosse aleatório, $\Delta W \neq 0$
          adicionaria ruído às projeções pré-treinadas, degradando o ponto
          de partida.
    \item Inicializar $B$ aleatoriamente faria com que o adaptador LoRA
          competisse com os pesos originais, reduzindo o número de
          parâmetros treináveis efetivos.
    \item $B = 0$ é matematicamente necessário para que a decomposição
          $\Delta W = BA$ tenha \textit{rank} exatamente $r$: inicialização
          aleatória quebraria a restrição de baixo \textit{rank}.
    \item Com $B$ aleatório, o fator $\alpha/r$ causaria explosão de
...
% ------------------------------------------------------------------
\item Um bloco do Flash Attention processa os escores $s = [1,\,2,\,0,\,3]$
      em dois tiles: $B_1 = [1,\,2]$ e $B_2 = [0,\,3]$. A recursão do
...
Cada projeção LoRA adiciona $|B| + |A| = d\cdot r + r\cdot k = 2\cdot d\cdot r$
...
$\Delta W = \tfrac{\alpha}{r}BA = 0$ na inicialização. Zerar $B$ (com $A$
aleatório) satisfaz essa condição. Se \emph{ambos} fossem aleatórios,
$\Delta W \neq 0$ adicionaria ruído aos pesos congelados, destruindo o
...
A opção~(c) é falsa: o \textit{rank} de $BA$ é determinado pela forma das
...
\textbf{Processando $B_1 = [1,\,2]$:}
\begin{align*}
  m^{(1)} &= \max(1,\,2) = 2 \\
  \ell^{(1)} &= e^{1-2} + e^{2-2} = 0.368 + 1 = 1.368.
\end{align*}
\textbf{Processando $B_2 = [0,\,3]$:}
\begin{align*}
  m^{(2)} &= \max(2,\,3) = 3 \\
