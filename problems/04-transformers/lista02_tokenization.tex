...
\begin{enumerate}[a)]
    ...
    \item 256 tokens (um por valor de byte possível), pois todo texto pode ser
          codificado em UTF-8 como uma sequência de bytes.
    \item 128 tokens (caracteres ASCII), pois todo texto pode ser reduzido ao
          alfabeto latino básico sem gerar OOV.
...
\begin{enumerate}[a)]
    \item O Unigram usa merges guiados por frequência de bigramas, enquanto o BPE
          usa verossimilhança.
    \item O Unigram é top-down: parte de um vocabulário grande e itera removendo
          os tokens menos úteis.
...
\begin{enumerate}[a)]
    ...
    \item Servir como \emph{data augmentation} sobre a segmentação do texto.
...
\begin{enumerate}[a)]
    ...
    \item O token continha bytes fora do intervalo UTF-8 válido, corrompendo o
          mapeamento byte-para-unicode usado na decodificação da saída.
    \item O vocabulário foi aprendido num corpus diferente do usado para treinar
          o modelo, deixando o embedding do token subtreinado.
    \item O token era longo demais e foi truncado em bytes inválidos durante a
          codificação da entrada pelo tokenizer.
\end{enumerate}
...
O GPT-3 (davinci) reutiliza o mesmo tokenizer do GPT-2 (\texttt{r50k\_base}, 50.257 tokens), e o
\texttt{p50k\_base} (50.281 tokens) é o do Codex e dos modelos text-davinci-002/003, enquanto
