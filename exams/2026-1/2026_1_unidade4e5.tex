...
\item (1.0) Uma entrada $\mathbf{X} \in \mathbb{R}^{n \times d_{\text{model}}}$ com $n = 10$ \textit{tokens} e $d_{\text{model}} = 512$ passa por uma camada de \textit{multi-head self-attention} com $H = 8$ cabeças (logo $d_k = d_{\text{model}}/H$). Quais são, respectivamente, as dimensões de (i) $\mathbf{Q}_i$ \emph{de uma cabeça}, (ii) da matriz de \textit{scores} $\mathbf{Q}_i \mathbf{K}_i^\top$ \emph{de uma cabeça}, e (iii) da saída final da camada (após concatenar as cabeças e aplicar $\mathbf{W}_O$)?

\begin{enumerate}[a)]
    \item $\mathbf{Q}_i$: $10 \times 512$; \quad $\mathbf{Q}_i \mathbf{K}_i^\top$: $10 \times 10$; \quad saída: $10 \times 512$.
    \item $\mathbf{Q}_i$: $10 \times 64$; \quad $\mathbf{Q}_i \mathbf{K}_i^\top$: $64 \times 64$; \quad saída: $10 \times 512$.
    \item $\mathbf{Q}_i$: $10 \times 64$; \quad $\mathbf{Q}_i \mathbf{K}_i^\top$: $10 \times 10$; \quad saída: $10 \times 64$.
    \item $\mathbf{Q}_i$: $64 \times 64$; \quad $\mathbf{Q}_i \mathbf{K}_i^\top$: $10 \times 10$; \quad saída: $10 \times 512$.
    \item $\mathbf{Q}_i$: $10 \times 64$; \quad $\mathbf{Q}_i \mathbf{K}_i^\top$: $8 \times 8$; \quad saída: $10 \times 512$.
    \item $\mathbf{Q}_i$: $10 \times 64$; \quad $\mathbf{Q}_i \mathbf{K}_i^\top$: $10 \times 10$; \quad saída: $10 \times 512$.
    \item $\mathbf{Q}_i$: $10 \times 8$; \quad $\mathbf{Q}_i \mathbf{K}_i^\top$: $10 \times 10$; \quad saída: $10 \times 512$.
    \item $\mathbf{Q}_i$: $10 \times 64$; \quad $\mathbf{Q}_i \mathbf{K}_i^\top$: $10 \times 10$; \quad saída: $512 \times 512$.
\end{enumerate}

\pagebreak

%% ---------- Q2: contagem de parâmetros de um bloco Transformer ----------
\item (1.0) Considere \textbf{um} bloco Transformer (esquematizado ao lado) com $d_{\text{model}} = 64$. A atenção é \textit{multi-head} com $H = 8$ cabeças e usa as quatro projeções $\mathbf{W}_Q, \mathbf{W}_K, \mathbf{W}_V, \mathbf{W}_O$, cada uma $64 \times 64$ \textbf{com viés}. A FFN expande para $d_{\text{ff}} = 256$ (ReLU, ambas as camadas \textbf{com viés}). Há dois \textit{LayerNorm}, cada um com parâmetros $\boldsymbol{\gamma}$ e $\boldsymbol{\beta}$ de dimensão $64$. Qual o número total de parâmetros \emph{treináveis} do bloco?
...
\textbf{I.} O fator de escala $\sqrt{d}$ em $\mathrm{softmax}(\mathbf{Q}\mathbf{K}^\top/\sqrt{d})\mathbf{V}$ mantém a variância dos \textit{scores} em torno de $1$ (o produto escalar de vetores com componentes i.i.d.\ de variância $1$ tem variância $d$), evitando a saturação do \textit{softmax}.

\textbf{II.} A operação de \textit{self-attention} ``crua'' (apenas $\mathrm{softmax}(\mathbf{Q}\mathbf{K}^\top/\sqrt{d})\mathbf{V}$, sem as projeções) não possui parâmetros treináveis.
...
\item (1.0) Em um VAE com latente $D_z = 2$, o \textit{encoder} produz, para certa entrada, $\boldsymbol{\mu} = (2,\; 0)$ e $\boldsymbol{\Sigma} = \mathrm{diag}(0.5,\; 2)$. Usando a forma fechada
\[
  D_{KL}\!\left[\mathcal{N}(\boldsymbol{\mu},\boldsymbol{\Sigma})\,\|\,\mathcal{N}(\mathbf{0},\mathbf{I})\right] = \tfrac{1}{2}\!\left(\mathrm{Tr}[\boldsymbol{\Sigma}] + \boldsymbol{\mu}^{\top}\boldsymbol{\mu} - D_z - \log\det\boldsymbol{\Sigma}\right),
\]
qual o valor de $D_{KL}$? (Note que $\det\boldsymbol{\Sigma} = 1$.)
...
\textbf{I.} A ELBO se decompõe como $\mathbb{E}_{q(\mathbf{z}|\mathbf{x},\phi)}[\log p(\mathbf{x}|\mathbf{z},\thetav)] - D_{KL}[q(\mathbf{z}|\mathbf{x},\phi)\,\|\,p(\mathbf{z})]$ e é um limite \emph{inferior} de $\log p(\mathbf{x}|\thetav)$.

\textbf{II.} O \textit{reparametrization trick} ($\mathbf{z} = \boldsymbol{\mu} + \boldsymbol{\Sigma}^{1/2}\boldsymbol{\epsilon}$, com $\boldsymbol{\epsilon} \sim \mathcal{N}(\mathbf{0},\mathbf{I})$) é necessário para permitir a retropropagação \emph{através} da amostragem do latente.

\textbf{III.} No termo $\tfrac{1}{2}(\mathrm{Tr}[\boldsymbol{\Sigma}] + \boldsymbol{\mu}^{\top}\boldsymbol{\mu} - D_z - \log\det\boldsymbol{\Sigma})$, a parcela $-\log\det\boldsymbol{\Sigma}$ cresce sem limite quando a variância do posterior tende a zero, penalizando o colapso do posterior a uma massa pontual.

\textbf{IV.} Um autoencoder determinístico comum pode gerar novas amostras simplesmente amostrando $\mathbf{z} \sim \mathcal{N}(\mathbf{0},\mathbf{I})$ e passando pelo \textit{decoder}, pois seu espaço latente já é gaussiano.

\textbf{V.} A ELBO é um limite \emph{superior} de $\log p(\mathbf{x}|\thetav)$, e a diferença para $\log p(\mathbf{x}|\thetav)$ é $D_{KL}[q(\mathbf{z}|\mathbf{x},\phi)\,\|\,p(\mathbf{z}|\mathbf{x},\thetav)]$.
...
\item (1.0) Uma camada \textit{affine coupling} de um \textit{normalizing flow} recebe $\mathbf{h} = (\mathbf{h}^A, \mathbf{h}^B)$, copia $\mathbf{h}^A$ e transforma $\mathbf{h}^B$ por $\mathbf{y}^B = \mathbf{h}^B \odot \exp(\mathbf{s}) + \mathbf{t}$, onde $(\mathbf{s}, \mathbf{t})$ são produzidos por uma rede a partir de $\mathbf{h}^A$. Para $\mathbf{h}^B = (3,\; 4)$, $\mathbf{s} = (\ln 2,\; 0)$ e $\mathbf{t} = (1,\; -1)$, calcule $\mathbf{y}^B$ e o log-determinante $\log\lvert\det J\rvert$ da camada.

\begin{enumerate}[a)]
    \item $\mathbf{y}^B = (7,\; 3)$ \quad e \quad $\log\lvert\det J\rvert = 2$.
    \item $\mathbf{y}^B = (7,\; 3)$ \quad e \quad $\log\lvert\det J\rvert = 3$.
    \item $\mathbf{y}^B = (6,\; 4)$ \quad e \quad $\log\lvert\det J\rvert = \ln 2 \approx 0.693$.
    \item $\mathbf{y}^B = (7,\; 4)$ \quad e \quad $\log\lvert\det J\rvert = \ln 2 \approx 0.693$.
    \item $\mathbf{y}^B = (7,\; 3)$ \quad e \quad $\log\lvert\det J\rvert = \ln 2 \approx 0.693$.
    \item $\mathbf{y}^B = (3,\; 4)$ \quad e \quad $\log\lvert\det J\rvert = 0$.
    \item $\mathbf{y}^B = (7,\; 3)$ \quad e \quad $\log\lvert\det J\rvert = 0$.
    \item $\mathbf{y}^B = (7,\; 3)$ \quad e \quad $\log\lvert\det J\rvert = 0.5$.
\end{enumerate}
...
\item (1.0) Em um modelo difusor com \textit{classifier-free guidance}, a estimativa de ruído guiada é $\tilde{\mathbf{g}}_t = (1 + w)\,\mathbf{g}_t[\mathbf{z}_t, c] - w\,\mathbf{g}_t[\mathbf{z}_t, \varnothing]$, com peso de guiamento $w \ge 0$, onde $\mathbf{g}_t[\mathbf{z}_t, c]$ é a predição \emph{condicionada} (ao \textit{prompt} $c$) e $\mathbf{g}_t[\mathbf{z}_t, \varnothing]$ a predição \emph{sem condição}. Para $\mathbf{g}_t[\mathbf{z}_t, c] = 2$, $\mathbf{g}_t[\mathbf{z}_t, \varnothing] = 0.5$ e $w = 3$, qual o valor de $\tilde{\mathbf{g}}_t$?
