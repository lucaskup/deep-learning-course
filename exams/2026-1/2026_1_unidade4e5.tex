...
\[
  D_{KL}\!\left[\mathcal{N}(\boldsymbol{\mu},\boldsymbol{\Sigma})\,\|\,\mathcal{N}(0,\mathbf{I})\right] = \tfrac{1}{2}\!\left(\mathrm{Tr}[\boldsymbol{\Sigma}] + \boldsymbol{\mu}^{\top}\boldsymbol{\mu} - D_z - \log\det\boldsymbol{\Sigma}\right),
\]
...
\textbf{II.} O \textit{reparametrization trick} ($\mathbf{z} = \boldsymbol{\mu} + \boldsymbol{\Sigma}^{1/2}\boldsymbol{\epsilon}$, com $\boldsymbol{\epsilon} \sim \mathcal{N}(0,\mathbf{I})$) é necessário para permitir a retropropagação \emph{através} da amostragem do latente.
...
\textbf{IV.} Um autoencoder determinístico comum pode gerar novas amostras simplesmente amostrando $\mathbf{z} \sim \mathcal{N}(0,\mathbf{I})$ e passando pelo \textit{decoder}, pois seu espaço latente já é gaussiano.
