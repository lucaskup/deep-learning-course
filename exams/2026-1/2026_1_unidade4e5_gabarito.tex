...
\textbf{II (V):} sem o \textit{reparametrization trick} não se retropropaga através do nó aleatório de amostragem; movê-lo para $\boldsymbol{\epsilon} \sim \mathcal{N}(\mathbf{0},\mathbf{I})$ mantém o caminho por $\boldsymbol{\mu}, \boldsymbol{\Sigma}^{1/2}$ diferenciável.
...
\textbf{IV (F):} no AE determinístico o espaço latente \emph{não} tem distribuição conhecida; amostrar $\mathbf{z}\sim\mathcal{N}(\mathbf{0},\mathbf{I})$ cai em regiões sem suporte e o \textit{decoder} produz lixo. É justamente o que o VAE corrige ao regularizar o posterior.
