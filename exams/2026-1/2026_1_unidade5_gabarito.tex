...
\textbf{II é incorreta:} a composição de transformações afins é afim: $\mathbf{\Theta}_2(\mathbf{\Theta}_1\mathbf{x} + \mathbf{b}_1) + \mathbf{b}_2 = (\mathbf{\Theta}_2\mathbf{\Theta}_1)\mathbf{x} + (\mathbf{\Theta}_2\mathbf{b}_1 + \mathbf{b}_2)$. Empilhar \textit{linear flows} não ganha expressividade alguma; a expressividade dos \textit{flows} vem das camadas não lineares (como as \textit{coupling layers}).
...
\textbf{(c)} o \textit{embedding} de $t$ é determinístico; a estocasticidade da amostragem vem do termo $\sigma_t\,\boldsymbol{\epsilon}$, somado fora da rede.
...
\begin{enumerate}[a)]
    \item \textbf{F.} O \textit{WGAN} original impõe a restrição de Lipschitz por \textit{weight clipping}: são os \textbf{pesos} do crítico que são truncados, $\mathbf{w} \leftarrow \mathrm{clip}(\mathbf{w}, -c, c)$, após cada atualização (não confundir com \textit{gradient clipping}, que trunca gradientes). \textit{Modificação:} substituir ``\textit{gradient clipping}: os gradientes do crítico são truncados'' por ``\textit{weight clipping}: os pesos do crítico são truncados para $[-c, c]$''. (Também aceito: descrever a alternativa \textit{gradient penalty} do WGAN-GP, que adiciona à \emph{loss} o termo $\lambda(\|\nabla_{\hat{\mathbf{x}}} f[\hat{\mathbf{x}},\boldsymbol{\phi}]\|_2 - 1)^2$ em pontos interpolados.)
    ...
    \item \textbf{V.} Descrição correta do \textit{reparametrization trick}: $\boldsymbol{\epsilon}^{*}$ é amostrado de uma distribuição fixa, fora do caminho do gradiente; $\boldsymbol{\mu}(\boldsymbol{\phi})$ e $\boldsymbol{\Sigma}^{1/2}(\boldsymbol{\phi})$ permanecem diferenciáveis, permitindo o \textit{backpropagation} até o \textit{encoder}.
...
Dados: $\mathbf{x} = (2.4,\, -1.0)$, $\boldsymbol{\mu} = (2,\, -1)$, $\boldsymbol{\Sigma} = \mathrm{diag}(0.25,\, 4)$, $\boldsymbol{\sigma} = (0.5,\, 2)$, $D_z = 2$.

\textbf{(a)} \textit{Termo KL:}
\begin{align*}
    \mathrm{Tr}[\boldsymbol{\Sigma}] &= 0.25 + 4 = 4.25 \\
    \boldsymbol{\mu}^{\top}\boldsymbol{\mu} &= 2^{2} + (-1)^{2} = 5 \\
    \log\det\boldsymbol{\Sigma} &= \log(0.25 \cdot 4) = \log 1 = 0 \\
    D_z &= 2 \\
...
\textbf{(b)} \textit{Reparametrização} com $\boldsymbol{\epsilon}^{*} = (0.2,\, -0.3)$:
\[
...
\textbf{(c)} \textit{Termo de reconstrução:} com $f[\mathbf{z}^{*},\thetav] = (2.0,\, -1.3)$,
\[
    \mathbf{x} - f[\mathbf{z}^{*},\thetav] = (0.4,\; 0.3), \qquad
    \tfrac{1}{2}\lVert \mathbf{x} - f \rVert^{2} = \tfrac{1}{2}\left(0.4^{2} + 0.3^{2}\right) = \tfrac{1}{2}\,(0.25) = 0.125.
\]
Montando o objetivo:
\[
...
\textbf{(d)} Com $\boldsymbol{\Sigma} = \mathrm{diag}(10^{-6}, 10^{-6})$, o termo
\[
    -\log\det\boldsymbol{\Sigma} = -\log\!\left(10^{-12}\right) = 12\ln 10 \approx 27.6
\]
\textbf{explode}, dominando a KL. Essa penalização evita que o \textit{encoder} colapse $q(\mathbf{z} \mid \mathbf{x}, \boldsymbol{\phi})$ para (quase) uma massa pontual, isto é, que ele se torne determinístico. Sem ela, o VAE degeneraria em um \textit{autoencoder} clássico: o espaço latente perderia a suavidade garantida pela sobreposição das distribuições $q$, e a amostragem ancestral a partir de $p(\mathbf{z})$ deixaria de produzir reconstruções plausíveis entre os pontos de treino.
