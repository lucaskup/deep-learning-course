...
\textbf{II.} Empilhar várias camadas de \textit{linear flow} $f(\mathbf{x}) = \boldsymbol{\theta}\mathbf{x} + \mathbf{b}$ com matrizes $\boldsymbol{\theta}$ distintas aumenta progressivamente a expressividade do modelo, permitindo representar densidades que uma única camada linear não consegue representar.
...
\item (1.2) Você treina uma GAN com a \emph{loss} original do gerador, $L[\thetav] = \sum_{j} \ln\!\left[1 - \sigma\!\left(f\!\left[g[z^{(j)},\thetav],\phi\right]\right)\right]$. Nas primeiras épocas, a acurácia do discriminador sobe para perto de $100\%$ e $\sigma(f[x^{*},\phi]) \approx 0$ em todas as amostras geradas; a \emph{loss} do gerador fica praticamente constante e $\|\nabla_{\thetav} L\| \approx 0$. Qual é o diagnóstico mais consistente com esses sintomas e a mitigação mais direta?
\begin{enumerate}[a)]
    ...
    \item Os \textit{manifolds} de $P$ e $P^{*}$ estão sobrepostos demais desde o início; reduzir a dimensão do latente $z$ separa as distribuições e recria o sinal de gradiente.
...
\item (1.2) Em um modelo difusor, a U-Net que estima o ruído recebe como entrada não apenas o latente $z_t$, mas também um \textit{time embedding} construído a partir do passo $t$. Por que essa informação de $t$ é necessária?
\begin{enumerate}[a)]
    ...
    \item Porque o \textit{embedding} de $t$ injeta a estocasticidade necessária na rede, fazendo o papel do termo de ruído $\sigma_t\,\epsilon$ usado na amostragem reversa.
    ...
    \item Porque sem o \textit{embedding} de $t$ a \emph{loss} MSE $|g_t[z_t,\thetav_t] - \epsilon|^{2}$ deixa de ser diferenciável em relação aos parâmetros da U-Net.
\end{enumerate}
...
\begin{enumerate}[a)]
    ...
    \item ( )\,V \quad ( )\,F \quad O \textit{reparametrization trick} reescreve a amostragem do latente como $z^{*} = \mu + \Sigma^{1/2}\epsilon^{*}$, com $\epsilon^{*} \sim \mathcal{N}(\mathbf{0},\mathbf{I})$, de modo que a estocasticidade fica concentrada em $\epsilon^{*}$ e o caminho por $\mu$ e $\Sigma^{1/2}$ permanece diferenciável em relação a $\phi$.\\[0.2em]
    \rule{\linewidth}{0.4pt}\\[0.2em]
    \rule{\linewidth}{0.4pt}
...
\item (2.0) Considere um VAE com latente bidimensional ($D_z = 2$) e \textit{decoder} $p(x \mid z, \thetav) = \mathcal{N}_{x}\!\left(f[z,\thetav],\, \mathbf{I}\right)$. Para a entrada $x = (2.4,\; -1.0)$, o \textit{encoder} $g[x,\phi]$ produz
\[
  \mu = (2,\; -1), \qquad \Sigma = \mathrm{diag}(0.25,\; 4) \quad (\text{logo } \sigma = (0.5,\; 2)).
\]
Use a forma fechada $D_{KL}\!\left[\mathcal{N}(\mu,\Sigma)\,\|\,\mathcal{N}(\mathbf{0},\mathbf{I})\right] = \tfrac{1}{2}\!\left(\mathrm{Tr}[\Sigma] + \mu^{\top}\mu - D_z - \log\det\Sigma\right)$.

\begin{enumerate}[a)]
    \item (0.5) Calcule $D_{KL}\!\left[q(z \mid x, \phi)\,\|\,p(z)\right]$, mostrando o valor de cada um dos quatro termos.
    \vspace{2.5cm}

    \item (0.5) Foi amostrado $\epsilon^{*} = (0.2,\; -0.3)$. Calcule $z^{*}$ pelo \textit{reparametrization trick}.
    \vspace{2cm}

    \item (0.5) O \textit{decoder} devolve $f[z^{*},\thetav] = (2.0,\; -1.3)$. Calcule o termo de reconstrução $\tfrac{1}{2}\lVert x - f[z^{*},\thetav]\rVert^{2}$ e monte o valor aproximado de $-\mathrm{ELBO} \approx \text{reconstrução} + D_{KL}$.
    \vspace{2.5cm}

    \item (0.5) Suponha que, para todas as entradas, o \textit{encoder} passasse a produzir $\Sigma = \mathrm{diag}(10^{-6},\, 10^{-6})$. O que acontece com o termo $-\log\det\Sigma$ da KL e qual comportamento indesejado do \textit{encoder} essa penalização evita?
    \vspace{2.5cm}
\end{enumerate}
