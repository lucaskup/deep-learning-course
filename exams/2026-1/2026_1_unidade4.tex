...
\begin{enumerate}[a)]
    ...
    \item A máscara causal não está sendo aplicada a $\mathbf{Q}\mathbf{K}^\top$, então o modelo enxerga tokens futuros e aprende a copiá-los; aplicar $-\infty$ às entradas $(i, j)$ com $j > i$ antes do \textit{softmax} resolve.
...
\begin{enumerate}[a)]
    \item ( )\,V \quad ( )\,F \quad Em \textit{Multi-Head Attention} com $H$ cabeças e dimensão $d_\text{model}$, cada cabeça opera com dimensão $d_k = d_\text{model}/H$ e a saída final é a concatenação das $H$ projeções, seguida por uma projeção linear $\mathbf{W}^O$ que recompõe a dimensão $d_\text{model}$.\\[0.2em]
...
\item (2.0) Considere uma camada de \textit{self-attention} causal de \textbf{cabeça única} aplicada a uma sequência de 3 tokens $(\mathbf{x}_1, \mathbf{x}_2, \mathbf{x}_3)$. As matrizes $\mathbf{Q}$, $\mathbf{K}$, $\mathbf{V}$ já foram calculadas e têm dimensão $3 \times 1$ (sequência de comprimento 3, dimensão de chave $d_k = 1$):
\[
    \mathbf{Q} = \begin{bmatrix} 1 \\ 1 \\ 1 \end{bmatrix},\quad
    \mathbf{K} = \begin{bmatrix} 1 \\ 1 \\ 1 \end{bmatrix},\quad
    \mathbf{V} = \begin{bmatrix} 2 \\ 4 \\ 6 \end{bmatrix}.
\]
A operação aplicada é $\mathbf{O} = \mathrm{softmax}\!\left(\mathrm{mask}(\mathbf{Q}\mathbf{K}^\top)/\sqrt{d_k}\right) \mathbf{V}$, onde $\mathrm{mask}(\cdot)$ aplica a máscara causal.

\begin{enumerate}[a)]
    \item (0.5) Compute a matriz de pontuações $\mathbf{S} = \mathbf{Q}\mathbf{K}^\top$.
    \vspace{1.5cm}

    \item (0.5) Aplique a máscara causal a $\mathbf{S}$ e indique a matriz $\mathbf{S}_\text{mask}$ resultante.
    \vspace{1.5cm}

    \item (0.5) Aplique \textit{softmax} linha a linha em $\mathbf{S}_\text{mask}$ e indique a matriz de pesos de atenção $\mathbf{A}$.
    \vspace{2.5cm}

    \item (0.5) Compute a saída $\mathbf{O} = \mathbf{A} \cdot \mathbf{V}$. Apresente os três valores $O_1$, $O_2$, $O_3$.
    \vspace{2cm}
\end{enumerate}
