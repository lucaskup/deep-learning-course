...
Com $H = 8$ e $d_{\text{model}} = 512$, cada cabeça tem $d_k = 512/8 = 64$. Logo $Q_i = X W_Q^i \in \mathbb{R}^{10 \times 64}$; os \textit{scores} $Q_i K_i^\top \in \mathbb{R}^{10 \times 10}$ (uma entrada por par de \textit{tokens}); após concatenar as $8$ cabeças ($10 \times 512$) e aplicar $W_O$ ($512\times 512$), a saída é $10 \times 512$.\\
\textbf{(a)} usa $Q_i$ com $d_{\text{model}}$ inteiro (não dividiu por $H$).
...
\textbf{(h)} saída $512\times 512$ confunde a matriz $W_O$ com a saída.

\subsection*{Questão 2 \textemdash{} Resposta: (c)}
Cada projeção de atenção é $64\times 64 + 64 = 4160$; com $W_Q, W_K, W_V, W_O$: $4 \times 4160 = 16\,640$ (o número de cabeças $H$ \emph{não} altera o total, pois as cabeças particionam $d_{\text{model}}$). FFN: $W_1 = 64\times 256 + 256 = 16\,640$ e $W_2 = 256\times 64 + 64 = 16\,448$, total $33\,088$. Dois \textit{LayerNorm}: $2 \times (64 + 64) = 256$. Soma: $16\,640 + 33\,088 + 256 = 49\,984$.\\
\textbf{(a)} $49\,408$ ignora os vieses das projeções e da FFN (mantém os \textit{LayerNorm}).
\textbf{(b)} $45\,824$ esquece a projeção de saída $W_O$.
...
\textbf{II (V):} sem as projeções $W_Q, W_K, W_V, W_O$, a atenção ``crua'' é só \textit{softmax} e somas ponderadas, sem parâmetros.
...
\textbf{I (V):} o termo $QK^\top$ é $O(n^2 d)$; dobrar $n$ quadruplica a parcela $n^2$.
\textbf{II (V):} com \textit{KV-cache}, cada novo \textit{token} reaproveita $K, V$ do prefixo, custando $O(n)$, mas a memória do \textit{cache} cresce $\propto n$.
...
$\mathrm{Tr}[\Sigma] = 0.5 + 2 = 2.5$; \quad $\mu^{\top}\mu = 2^2 + 0^2 = 4$; \quad $\det\Sigma = 0.5\cdot 2 = 1 \Rightarrow \log\det\Sigma = 0$; \quad $D_z = 2$. Logo
\[
D_{KL} = \tfrac{1}{2}(2.5 + 4 - 2 - 0) = \tfrac{1}{2}(4.5) = 2.25.
\]
\textbf{(a)} $3.25$ esquece o termo $-D_z$.
\textbf{(c)} $4.50$ esquece o fator $\tfrac{1}{2}$.
\textbf{(d)} $0.25$ esquece o termo $\mu^{\top}\mu$.
\textbf{(e)} $1.50$ usa $\det\Sigma$ no lugar de $\mathrm{Tr}[\Sigma]$.
\textbf{(f)} $4.25$ soma $+D_z$ em vez de subtrair.
\textbf{(g)/(h)} demais erros.

\subsection*{Questão 7 \textemdash{} Resposta: (h)}
\textbf{I (V):} decomposição reconstrução $-$ KL; a ELBO é limite \emph{inferior} de $\log p(x)$.
\textbf{II (V):} sem o \textit{reparametrization trick} não se retropropaga através do nó aleatório de amostragem; movê-lo para $\epsilon \sim \mathcal{N}(\mathbf{0},\mathbf{I})$ mantém o caminho por $\mu, \Sigma^{1/2}$ diferenciável.
\textbf{III (V):} $-\log\det\Sigma \to +\infty$ quando $\det\Sigma \to 0$, penalizando variância nula (colapso a massa pontual).
\textbf{IV (F):} no AE determinístico o espaço latente \emph{não} tem distribuição conhecida; amostrar $z\sim\mathcal{N}(\mathbf{0},\mathbf{I})$ cai em regiões sem suporte e o \textit{decoder} produz lixo. É justamente o que o VAE corrige ao regularizar o posterior.
\textbf{V (F):} a ELBO é limite \emph{inferior} (não superior); a diferença para $\log p(x)$ é $D_{KL}[q(z|x,\phi)\,\|\,p(z|x,\thetav)]$ (KL ao posterior \emph{verdadeiro}, não ao \textit{prior}).
Verdadeiras: I, II e III.\\
...
\textbf{(c)} $y^B = (6,4)$ esquece o deslocamento $\mathbf{t}$.
\textbf{(d)} $y^B = (7,4)$ aplica $\mathbf{t}$ só na primeira componente.
\textbf{(f)} não transforma $h^B$.
...
\[
\tilde{g}_t = (1 + w)\,g_t[z_t, c] - w\,g_t[z_t, \varnothing] = (1+3)\cdot 2 - 3\cdot 0.5 = 8 - 1.5 = 6.5.
\]
(Equivalentemente, $\tilde{g}_t = g_t[z_t,c] + w\,(g_t[z_t,c] - g_t[z_t,\varnothing]) = 2 + 3\cdot 1.5 = 6.5$: extrapola-se a partir da predição \emph{condicionada}.)\\
\textbf{(b)} $5.0$ usa a predição \emph{sem condição} como base: $g_\varnothing + w(g_c - g_\varnothing)$.
\textbf{(c)} $4.0$ inverte os papéis: $w\,g_c - (1+w)g_\varnothing$.
\textbf{(d)} $8.0$ esquece o termo $-w\,g_\varnothing$.
\textbf{(e)} $6.0$ calcula $(1+w)(g_c - g_\varnothing)$.
\textbf{(f)} $4.5$ calcula apenas $w\,(g_c - g_\varnothing)$.
