...
\begin{enumerate}[a)]
    ...
    \item O gradiente explode: a retropropagação no tempo multiplica fatores $\theta_{hh}\,(1-\tanh^2(z_k))$ que podem ter módulo maior que $1$; aplicar \textit{clipping} por norma ($g \leftarrow c\,g/\|g\|$ quando $\|g\| > c$) limita o passo preservando a direção.
...
\item (1{,}0) Em um modelo \textit{seq2seq} com atenção de produto escalar, o estado do decodificador no passo $t$ é $h_t = (1;\ 1)$ e os estados do codificador são $s_1 = (2;\ 2)$, $s_2 = (1;\ 1)$ e $s_3 = (0;\ -1)$. Os \textit{scores} são $\mathrm{score}(h_t, s_k) = h_t^{\top} s_k$ e os pesos de atenção são
\[
a_k^{(t)} = \frac{\exp(\mathrm{score}(h_t, s_k))}{\sum_{i=1}^{3}\exp(\mathrm{score}(h_t, s_i))}.
\]
...
\[
f_t = \sigma(\theta_f\,[h_{t-1}, x_t] + b_f), \qquad
i_t = \sigma(\theta_i\,[h_{t-1}, x_t] + b_i), \qquad
\tilde{c}_t = \tanh(\theta_c\,[h_{t-1}, x_t] + b_c),
\]
\[
c_t = f_t \odot c_{t-1} + i_t \odot \tilde{c}_t, \qquad
o_t = \sigma(\theta_o\,[h_{t-1}, x_t] + b_o), \qquad
h_t = o_t \odot \tanh(c_t).
\]
O estado inicial é $c_0 = -0{,}2$ e $h_0 = 0{,}4$, e a entrada no passo $1$ é $x_1 = 0{,}5$, de modo que $[h_0, x_1] = [0{,}4,\ 0{,}5]$. Os parâmetros são:
\[
\theta_f = [0{,}5,\ -0{,}4],\ b_f = 0{,}2; \qquad
\theta_i = [-0{,}3,\ 0{,}6],\ b_i = -0{,}1; \qquad
\]
\[
\theta_c = [0{,}2,\ 0{,}8],\ b_c = 0; \qquad
\theta_o = [0{,}7,\ 0{,}1],\ b_o = -0{,}3.
\]
