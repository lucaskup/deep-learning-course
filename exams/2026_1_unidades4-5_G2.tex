...
\item (1{,}0) No LoRA, uma matriz congelada $W_0 \in \mathbb{R}^{d \times k}$ recebe a correção \textit{low rank} $h = W_0 x + \frac{\alpha}{r} B A x$, com $B \in \mathbb{R}^{d \times r}$ e $A \in \mathbb{R}^{r \times k}$; apenas $B$ e $A$ são treináveis. Você aplica LoRA com $r = 8$ às projeções $W_Q$ e $W_V$ (ambas $1024 \times 1024$) de \textbf{cada uma} das $12$ camadas de um \textit{transformer}. Quantos parâmetros treináveis o ajuste introduz no total?
...
\textbf{4.} ALiBi & \textbf{(iv)} rotaciona os vetores $q$ e $k$ dentro da atenção, fazendo o \textit{score} depender apenas do deslocamento relativo $j - i$. \\
\end{tabular}
\end{center}
...
\textbf{II.} Desse modo uma única rede aprende tanto a predição condicional $g_t[z_t, c]$ quanto a incondicional $g_t[z_t, \varnothing]$, que são combinadas na amostragem como $\tilde{g}_t = (1+w)\,g_t[z_t, c] - w\,g_t[z_t, \varnothing]$, com $w \geq 0$.
...
\begin{itemize}
    \item[\textbf{I.}] Amostrar uma instância $x$ do conjunto de treinamento.
    \item[\textbf{II.}] Amostrar $t \sim \mathrm{Uniforme}\{1, \dots, T\}$ e $\epsilon \sim \mathcal{N}(0, \mathbf{I})$.
    \item[\textbf{III.}] Computar $z_t = \sqrt{\alpha_t}\,x + \sqrt{1 - \alpha_t}\,\epsilon$ (kernel de difusão).
    \item[\textbf{IV.}] Computar a perda $\lVert g_t[z_t, \thetav_t] - \epsilon\rVert^{2}$.
