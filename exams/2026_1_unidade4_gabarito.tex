...
Dados: $Q = K = [1, 1, 1]^\top$, $V = [2, 4, 6]^\top$, $d_k = 1$.

\textbf{(a)} \textit{Matriz de pontuações.} Cada entrada é $S_{ij} = Q_i \cdot K_j = 1 \cdot 1 = 1$:
\[
...
\textbf{(d)} \textit{Saída.} $O = A \cdot V$:
\begin{align*}
...
\end{align*}
\[
O = \begin{bmatrix} 2 \\ 3 \\ 4 \end{bmatrix}
\]
Cada $O_i$ é a média (com pesos uniformes pela máscara causal e $Q = K = $ constantes) dos valores $V_j$ até a posição $i$, capturando a interpretação intuitiva da \textit{self-attention} como uma soma ponderada do passado.
