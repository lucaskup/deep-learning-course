...
Cada matriz adaptada recebe $B \in \mathbb{R}^{1024 \times 8}$ e $A \in \mathbb{R}^{8 \times 1024}$:
\[
r(d + k) = 8 \times (1024 + 1024) = 16\,384 \text{ parâmetros por matriz adaptada}.
\]
São $2$ projeções ($W_Q$ e $W_V$) em cada uma das $12$ camadas:
\[
...
\textbf{(f)} $589\,824$ aplica a três projeções ($Q$, $K$ e $V$), não às duas do enunciado.
\textbf{(g)} $786\,432$ aplica às quatro projeções ($Q$, $K$, $V$ e $O$).
\textbf{(h)} $25\,165\,824$ treina o $\Delta W$ denso $1024 \times 1024$ em vez dos fatores \textit{low rank}, exatamente o que o LoRA evita.
...
O quadro mostrado é sintoma do \textbf{esquecimento catastrófico}: a acurácia na tarefa alvo sobe enquanto capacidades adquiridas no pré-treino se degradam, porque o \textit{fine-tuning} completo com taxa alta e muitas épocas sobrescreve os pesos que codificavam o conhecimento geral. Mitigações padrão: reduzir taxa/épocas, misturar dados gerais ao corpus de ajuste (\textit{replay}) ou usar PEFT (LoRA), que congela $W_0$ e limita a atualização a poucos parâmetros.\\
...
Ordem válida: \textbf{I, II, III, IV, V}. As dependências: o kernel (III) precisa de $x$ (I) e de $t, \epsilon$ (II); a perda (IV) precisa de $z_t$ (III) e de $\epsilon$ para comparar; a atualização (V) precisa do gradiente da perda (IV).\\
\textbf{(a)} computa o kernel antes de amostrar $t$ e $\epsilon$, dos quais ele depende.
\textbf{(b)} computa o kernel antes de amostrar a instância $x$.
\textbf{(c)} computa a perda antes de construir $z_t$.
\textbf{(d)} inicia pelo kernel, sem $x$, $t$ ou $\epsilon$ amostrados.
...
\textbf{(g)} computa a perda antes do kernel, com $z_t$ ainda inexistente.
